...
def main():
    ...
    parser.add_argument("--libroot", required=True, help="Path to the root of the library to fuzz")
    ...
    fuzzers = discover_fuzzers(args.libroot)
    ...
    for duration_seconds in durations_seconds:
        ...
        for fuzzfn, path in fuzzers.items():
            ...
            run_fuzzer(fuzzfn, path, next_duration_seconds, args.libroot)
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            ...
            for fuzzfn in re.findall(r"func\s+(Fuzz\w+)", text):
                ...
                fuzzers[fuzzfn] = os.path.relpath(root, path)
    return fuzzers

def run_fuzzer(fuzzfn, dir, duration_seconds, libroot):
