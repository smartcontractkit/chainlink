...
func onAptosWriteReadRoundtripTrigger(cfg config.Config, runtime sdk.Runtime, payload *cron.Payload) (_ any, err error) {
	...
	viewFunction := fmt.Sprintf("%s::registry::get_feeds", normalizeHex(cfg.ReceiverHex))
	viewReply, err := client.View(runtime, &aptos.ViewRequest{
		Function:  viewFunction,
		Arguments: nil,
	}).Await()
	if err != nil {
		return nil, fmt.Errorf("Aptos View(%s): %w", viewFunction, err)
	}
	if viewReply == nil || len(viewReply.Data) == 0 {
		return nil, fmt.Errorf("empty view reply for %s", viewFunction)
	}
