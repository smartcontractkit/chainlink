...
func onAptosWriteTrigger(cfg config.Config, runtime sdk.Runtime, payload *cron.Payload) (_ any, err error) {
	...
	if cfg.ExpectFailure {
		if reply.TxStatus != aptos.TxStatus_TX_STATUS_FAILED {
			...
			runtime.Logger().Info(
				"Aptos write failed: expected TX_STATUS_FAILED",
				"workflow", cfg.WorkflowName,
...
func normalizeTxHash(raw []byte) (string, error) {
	// Capabilities may return either:
	// 1) raw hash bytes (32 bytes), or
	// 2) string bytes for "0x...." / "....".
	if len(raw) == 32 {
	...
	if !aptosHashRe.MatchString(s) {
		return "", fmt.Errorf("expected 32-byte tx hash, got %q", string(raw))
	}
	return "0x" + s, nil
}
