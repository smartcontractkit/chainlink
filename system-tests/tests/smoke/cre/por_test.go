...
func validateTronPrices(t *testing.T, testEnv *ttypes.TestEnvironment, blockchain *cre.Blockchain, feedID string, priceProvider PriceProvider, startTime time.Time, waitFor time.Duration, tick time.Duration) error {
...
func validatePoRPrices(t *testing.T, testEnv *ttypes.TestEnvironment, priceProvider PriceProvider, config *WorkflowTestConfig, additionalPrice big.Int) {
	...
	for idx, bcOutput := range testEnv.Blockchains {
		if bcOutput.CtfOutput.Type == blockchain.FamilySolana {
			continue
		}
...
func validateEVMPrices(t *testing.T, testEnv *ttypes.TestEnvironment, blockchain *cre.Blockchain, feedID string, priceProvider PriceProvider, startTime time.Time, waitFor time.Duration, tick time.Duration) error {
