...
func allowlistRequest(t *testing.T, owner string, request jsonrpc.Request[json.RawMessage], sethClient *seth.Client, wfRegistryContract *workflow_registry_v2_wrapper.WorkflowRegistry) {
	...
	for attempt := range maxRetries {
		...
		if attempt == 0 && txOpts.GasPrice != nil {
			baseGasPrice = new(big.Int).Set(txOpts.GasPrice)
		}
		if attempt > 0 && baseGasPrice != nil {
			txOpts.GasPrice = new(big.Int).Mul(baseGasPrice, big.NewInt(int64(attempt+1)))
		}
		_, err = wfRegistryContract.AllowlistRequest(txOpts, reqDigestBytes, uint32(time.Now().Add(1*time.Hour).Unix())) //nolint:gosec // disable G115
		if err == nil {
			break
		}
		lastErr = err
		if attempt < maxRetries-1 {
			backoff := time.Duration(50*int(1<<uint(attempt))) * time.Millisecond
			time.Sleep(backoff)
		}
	}
	require.NoError(t, lastErr, "failed to allowlist request after %d attempts", maxRetries)
