...
func fundAptosAccountBestEffort(
	...
	t.Helper()
	// Fast path: account exists and already has enough balance for local CRE write retries.
	if _, err := client.Account(account); err == nil {
		balance, balErr := client.AccountAPTBalance(account)
		if balErr == nil && balance >= aptosWorkerMinBalanceOctas {
			return
		}
	}
	...
	if err == nil {
		if faucetClient, cErr := aptoslib.NewFaucetClient(client, faucetURL); cErr != nil {
			...
			fundErr := faucetClient.Fund(account, aptosWorkerFundingAmountOctas)
			if fundErr != nil {
				framework.L.Warn().
					Err(fundErr).
					Str("faucet_url", faucetURL).
