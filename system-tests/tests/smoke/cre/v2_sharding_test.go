...
func validateShardingScaleScenario(t *testing.T, testEnv *ttypes.TestEnvironment, rpcHost string, workflowIDs []string) {
	...
	resp, err := arbiterClient.GetDesiredReplicas(ctxShort, &ringpb.ShardStatusRequest{})
	cancel()
	require.NoError(t, err)
	require.Equal(t, uint32(contractCount), resp.WantShards, "Arbiter WantShards must equal contract getDesiredShardCount()")
