...
func WorkerGateway(nodeAddress common.Address, homeChainID uint64, donID uint32, gatewayConnectorData types.GatewayConnectorOutput) string {
	gatewayURL := fmt.Sprintf("ws://%s:%d%s", gatewayConnectorData.OutgoingGatewayConnectorOutput.Host, gatewayConnectorData.OutgoingGatewayConnectorOutput.Port, gatewayConnectorData.OutgoingGatewayConnectorOutput.Path)
