...
func generateNodeTomlConfig(input cre.GenerateConfigsInput, nodeConfigTransformers []cre.NodeConfigTransformerFn) (cre.NodeIndexToConfigOverride, error) {
	...
	for nodeIdx, nodeMetadata := range input.DonMetadata.NodesMetadata {
		...
		for _, role := range nodeMetadata.Roles {
			...
			case cre.WorkerNode:
				var cErr error
				nodeConfig, cErr = addWorkerNodeConfig(nodeConfig, input.Topology, input.OCRPeeringData, input.OCRBootstrapPlacement, input.OCRBootstrapAnnouncePort, commonInputs, input.DonMetadata, nodeMetadata)
				if cErr != nil {
...
func baseNodeConfig(commonInputs *commonInputs, donMetadata *cre.DonMetadata, nodeMetadata *cre.NodeMetadata) corechainlink.Config {
	...
	if commonInputs.provider.IsDocker() {
		...
		c.Tracing.Enabled = new(false) // Set to true to enable tracing
		c.Tracing.CollectorTarget = new(strings.TrimPrefix(framework.HostDockerInternal(), "http://") + ":4317")
		c.Tracing.SamplingRatio = new(1.0)
		c.Tracing.Mode = new("unencrypted")
		c.Tracing.NodeID = new(donMetadata.Name + "-node-" + strconv.Itoa(nodeMetadata.Index))
	}
...
func addBootstrapNodeConfig(
	...
	existingConfig.P2P = coretoml.P2P{
		V2: coretoml.P2PV2{
			Enabled:              new(true),
			ListenAddresses:      new([]string{"0.0.0.0:" + strconv.Itoa(ocrPeeringData.Port)}),
	...
	if existingConfig.Capabilities.ExternalRegistry.Address == nil {
		existingConfig.Capabilities.ExternalRegistry = coretoml.ExternalRegistry{
			Address:         new(commonInputs.capabilityRegistry.address),
			NetworkID:       new("evm"),
...
func addWorkerNodeConfig(
	...
	bootstrapAddress, bootstrapAddressErr := resolveNodeFacingBootstrapAddress(
		donMetadata.MustNodeSet().Placement,
	...
	if donMetadata.HasFlag(cre.WorkflowDON) && existingConfig.Capabilities.WorkflowRegistry.Address == nil {
		...
		existingConfig.Capabilities.WorkflowRegistry = coretoml.WorkflowRegistry{
			Address:                 new(commonInputs.workflowRegistry.address),
			NetworkID:               new("evm"),
			ChainID:                 new(strconv.FormatUint(commonInputs.registryChainID, 10)),
			ContractVersion:         new(commonInputs.workflowRegistry.version.String()),
	...
	if donMetadata.IsShardDON() {
		existingConfig.Sharding.ShardingEnabled = new(true)
		existingConfig.Sharding.ShardIndex = new(uint16(donMetadata.ShardIndex)) //nolint:gosec // disable G115 overflow is unrealistic
		existingConfig.Sharding.ArbiterPort = new(cre.DefaultArbiterPort)
		existingConfig.Sharding.ShardOrchestratorPort = new(cre.DefaultShardOrchestratorPort)
	...
	if donMetadata.HasFlag(cre.WorkflowDON) {
		...
		if topology != nil && topology.GatewayConnectors != nil && len(topology.GatewayConnectors.Configurations) > 0 {
			...
			existingConfig.Capabilities.GatewayConnector = coretoml.GatewayConnector{
				DonID:             new(donMetadata.Name),
				ChainIDForNodeKey: new(strconv.FormatUint(commonInputs.registryChainID, 10)),
...
func addGatewayNodeConfig(
	...
	if existingConfig.P2P.V2.Enabled == nil {
		existingConfig.P2P.V2.Enabled = new(true)
	...
	if len(existingConfig.Capabilities.GatewayConnector.Gateways) == 0 {
		existingConfig.Capabilities.GatewayConnector = coretoml.GatewayConnector{
			DonID:             new("doesn't-matter-for-gateway-node"),
			ChainIDForNodeKey: new(strconv.FormatUint(commonInputs.registryChainID, 10)),
		}
	}
...
func findEVMChains(input cre.GenerateConfigsInput) ([]*evmChain, error) {
	...
	for chainSelector, bcOut := range input.Blockchains {
		...
		targetPlacementRaw, ok := input.BlockchainPlacementBySelector[chainSelector]
		if !ok || strings.TrimSpace(targetPlacementRaw) == "" {
			targetPlacementRaw = string(connectivity.PlacementLocal)
		}
		targetPlacement, err := connectivity.PlacementFromTarget(targetPlacementRaw)
		if err != nil {
			return nil, err
		}
		resolvedHTTP, err := connectivity.Resolve(callerPlacement, targetPlacement, connectivity.EndpointPair{
...
func findOneSolanaChain(input cre.GenerateConfigsInput) (*solanaChain, error) {
	...
	for _, bcOut := range input.Blockchains {
		...
		targetPlacementRaw, ok := input.BlockchainPlacementBySelector[solBc.ChainSelector()]
		if !ok || strings.TrimSpace(targetPlacementRaw) == "" {
			targetPlacementRaw = string(connectivity.PlacementLocal)
		}
		targetPlacement, err := connectivity.PlacementFromTarget(targetPlacementRaw)
		...
		resolvedNodeURL, err := connectivity.Resolve(callerPlacement, targetPlacement, connectivity.EndpointPair{
			Name:     "solana-rpc",
			Internal: bcOut.CtfOutput().Nodes[0].InternalHTTPUrl,
			External: bcOut.CtfOutput().Nodes[0].ExternalHTTPUrl,
		})
...
func resolveNodeFacingBootstrapAddress(callerPlacement, bootstrapPlacement, bootstrapHost string, internalPort, externalPort int, remoteHostIP string) (string, error) {
	...
	target, err := connectivity.PlacementFromTarget(bootstrapPlacement)
	if err != nil {
		return "", err
	}
