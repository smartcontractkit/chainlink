...
func LinkToJobDistributor(ctx context.Context, input *cre.LinkDonsToJDInput) (*cldf.Environment, []*devenv.DON, error) {
	...
	for idx, nodeOutput := range input.NodeSetOutput {
		...
		supportedChains, schErr := findSupportedChainsForDON(input.Topology.DonsMetadata.List()[idx], input.Blockchains)
		if schErr != nil {
...
func findSupportedChainsForDON(donMetadata *cre.DonMetadata, blockchainOutputs []*cre.Blockchain) ([]devenv.ChainConfig, error) {
	...
	for chainSelector, bcOut := range blockchainOutputs {
		...
		chainIsSolana := strings.EqualFold(bcOut.CtfOutput.Family, chainselectors.FamilySolana)
