...
func GenerateJobSpecs(donTopology *types.DonTopology, addressBook cldf.AddressBook, chainID uint64) (types.DonsToJobSpecs, error) {
	...
	for _, donWithMetadata := range donTopology.DonsWithMetadata {
		...
		for _, workerNode := range workflowNodeSet {
			...
			donToJobSpecs[donWithMetadata.ID] = append(donToJobSpecs[donWithMetadata.ID], jobs.DonTimeJob(nodeID, oCR3CapabilityAddress, nodeEthAddr, ocr2KeyBundleID, ocrPeeringData, chainID))
		}
	}
