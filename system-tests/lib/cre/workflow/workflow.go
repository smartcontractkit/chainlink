...
var NoCompilationArtifact *CompilationArtifact = nil
...
const (
	SourceOfBinary = "binary"
	SourceOfConfig = "config"
)
...
) (*libcrecli.DataSource, error) {
	dataSource := libcrecli.NewDataSource()

	if compilationArtifact != nil {
		if compilationArtifact.BinaryFilePath != "" {
			dataSource.AddObject(SourceOfBinary, ReadFile, compilationArtifact.BinaryFilePath)
		} else {
			return nil, errors.New("CompilationArtifact is invalid, binary file path is empty")
		}
		if compilationArtifact.ConfigFilePath != "" {
			dataSource.AddObject(SourceOfConfig, ReadFile, compilationArtifact.ConfigFilePath)
		} else {
			return nil, errors.New("CompilationArtifact is invalid, config file path is empty")
		}
	} else {
		dataSource.AddObject(SourceOfBinary, libnet.DownloadAndDecodeBase64, binaryURL)
		dataSource.AddObject(SourceOfConfig, libnet.Download, libcrecli.Deref(configURL))
	}
...
func RegisterWithContract(
	...
	donID uint32,
	workflowName, binaryURL string,
	configURL, secretsURL *string,
	compilationArtifact *CompilationArtifact,
) error {
	dataSource, err := NewWorkflowDataSource(binaryURL, configURL, compilationArtifact)
	if err != nil {
		return errors.Wrap(err, "failed to create workflow data source")
	}

	workFlowData, err := dataSource.GetData(ctx, SourceOfBinary)
	if err != nil {
	...
	configURLToUse := ""
	if configURL != nil {
		configData, err = dataSource.GetData(ctx, SourceOfConfig)
		if err != nil {
