...
// 	switch w.(type) {
// 	case *evm.Blockchain:
// 		asEvm := w.(*evm.Blockchain)
// 		cfg.ChainID = strconv.FormatUint(w.ChainID(), 10)
// 		cfg.ChainName = asEvm.SethClient.Cfg.Network.Name
// 		// ensure nonce fetched from chain at use time
// 		cfg.DeployerKey = asEvm.SethClient.NewTXOpts(seth.WithNonce(nil))
// 	case *solana.Blockchain:
// 		asSolana := w.(*solana.Blockchain)
// 		cfg.ChainID = asSolana.SolanaChainID
// 		cfg.SolDeployerKey = asSolana.PrivateKey
// 		cfg.SolArtifactDir = asSolana.ArtifactsDir
// 	case *tron.Blockchain:
// 		asTron := w.(*tron.Blockchain)
// 		cfg.ChainID = strconv.FormatUint(w.ChainID(), 10)
// 		privateKey, err := crypto.HexToECDSA(asTron.DeployerPrivateKey)
// 		if err != nil {
