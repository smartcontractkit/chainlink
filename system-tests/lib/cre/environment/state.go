...
func BuildFromSavedState(ctx context.Context, cldLogger logger.Logger, cachedInput *envconfig.Config, envArtifact EnvArtifact) (*cre.FullCLDEnvironmentOutput, []*cre.WrappedBlockchainOutput, error) {
	...
	for _, bc := range cachedInput.Blockchains {
		...
		if bc.Out != nil && bc.Out.Family == "tron" {
	...
	for _, output := range wrappedBlockchainOutputs {
		if output.TronChain != nil {
			...
			// For EVM chains, we still need to create proper chain objects
			// but we'll use the deployment_devenv approach for EVM chains
			chainConfigs := []deployment_devenv.ChainConfig{{
				...
				DeployerKey: output.SethClient.NewTXOpts(seth.WithNonce(nil)),
			}}

			evmChains, evmErr := deployment_devenv.NewChains(cldLogger, chainConfigs)
			if evmErr != nil {
				return nil, nil, errors.Wrapf(evmErr, "failed to create EVM chain %d", output.ChainSelector)
			}

			// Extract the single EVM chain and add it to our map
			for selector, evmChain := range evmChains.All() {
				blockChainsMap[selector] = evmChain
			}
		}
	}
