...
func PrepareNodeTOMLConfigurations(
	...
	for _, blockchain := range blockchains {
		if blockchain.SolChain != nil {
			sel := blockchain.SolChain.ChainSelector
			chainPerSelector[sel] = blockchain
			chainPerSelector[sel].ChainSelector = sel
			chainPerSelector[sel].SolChain = blockchain.SolChain
			chainPerSelector[sel].SolChain.ArtifactsDir = blockchain.SolChain.ArtifactsDir
			continue
		}
		chainPerSelector[blockchain.ChainSelector] = blockchain
	}
