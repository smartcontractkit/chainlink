...
func SetupTestEnvironment(
	...
	if pairingSummary := topology.DonFamilyGatewayPairingSummary(); pairingSummary != "" {
		testLogger.Info().Msg(pairingSummary) // confirms env-start pairing, e.g. feeds-zone-a → gateway-zone-a
	}
	...
	workflowRegistryConfigurationOutput, wfErr := workflow.ConfigureWorkflowRegistry(
		...
		&cre.WorkflowRegistryInput{
			...
			DONFamilies:     topology.WorkflowDONFamilies(),                                                                             // nil on non-gateway topologies → registry setup uses DefaultDONFamily; else SetDONLimit per family
