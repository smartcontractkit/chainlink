...
func (o *Mock) PostEnvStartup(
	...
	workerInput := cre_jobs.ProposeJobSpecInput{
		...
		Template: job_types.Cron, // use cron job type to avoid adding mock job type to jobs package, because it is only for testing
	...
	if workerVerErr != nil {
		return fmt.Errorf("precondition verification failed for Mock worker job: %w", workerVerErr)
	}

	workerReport, workerErr := cre_jobs.ProposeJobSpec{}.Apply(*creEnv.CldfEnvironment, workerInput)
	if workerErr != nil {
		return fmt.Errorf("failed to propose Mock worker job spec: %w", workerErr)
	}
	...
	if approveErr != nil {
		return fmt.Errorf("failed to approve Mock jobs: %w", approveErr)
	}
