...
const flag = cre.RingOCRCapability

type Ring struct{}

func (c *Ring) Flag() cre.CapabilityFlag {
	return flag
}

func (c *Ring) PreEnvStartup(
	ctx context.Context,
...
const ContractQualifier = "ring_consensus"

func (c *Ring) PostEnvStartup(
	ctx context.Context,
	testLogger zerolog.Logger,
	don *cre.Don,
	dons *cre.Dons,
	...
	strategy, err := strategies.CreateStrategy(
		...
		*creEnv.CldfEnvironment,
		...
		nil,
		*ocr3ContractAddr,
		"PostEnvStartup - Configure OCR3 Contract - v2 Ring",
	)
	if err != nil {
...
func createJobs(
	...
	if bootVerErr != nil {
		return fmt.Errorf("precondition verification failed for Ring v2 bootstrap job for chainID %d: %w", chainID, bootVerErr)
	}

	bootReport, bootErr := cre_jobs.ProposeJobSpec{}.Apply(*creEnv.CldfEnvironment, bootInput)
	if bootErr != nil {
		return fmt.Errorf("failed to propose Ring v2 bootstrap job spec for chainID %d: %w", chainID, bootErr)
	}
	...
	if workerVerErr != nil {
		return fmt.Errorf("precondition verification failed for Ring v2 worker job: %w", workerVerErr)
	}

	workerReport, workerErr := cre_jobs.ProposeJobSpec{}.Apply(*creEnv.CldfEnvironment, workerInput)
	if workerErr != nil {
		return fmt.Errorf("failed to propose Ring v2 worker job spec: %w", workerErr)
	}
	...
	if approveErr != nil {
		return fmt.Errorf("failed to approve Ring v2 jobs: %w", approveErr)
	}
