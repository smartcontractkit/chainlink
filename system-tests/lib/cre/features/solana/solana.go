...
func (o *Solana) PreEnvStartup(
	...
	// 		version, ks_sol.DefaultForwarderQualifier, bcOut.ChainSelector)
	// 	if errp != nil {
	...
	// 		ks_sol_op.Deps{
	// 			Env:       *cldfEnv,
	// 			Chain:     cldfEnv.BlockChains.SolanaChains()[bcOut.ChainSelector],
	// 			Datastore: memoryDatastore.Seal(),
	// 		},
	// 		ks_sol_seq.DeployForwarderSeqInput{
	// 			ChainSel:     bcOut.ChainSelector,
	...
	// 		ChainSelector: bcOut.ChainSelector,
	...
	// 	testLogger.Info().Msgf("Deployed Forwarder %s contract on Solana chain chain %d programID: %s state: %s", creEnv.ContractVersions[ks_sol.ForwarderContract.String()], bcOut.ChainSelector, out.Output.ProgramID.String(), out.Output.State.String())
	// }
	...
	// if err := deployForwarder(testLogger, creEnv, solChain); err != nil {
	// 	return nil, errors.Wrap(err, "failed to deploy solana forwarder")
	// }
	...
	// 	forwarders := creEnv.CldfEnvironment.DataStore.Addresses().Filter(datastore.AddressRefByChainSelector(data.ChainSelector))
