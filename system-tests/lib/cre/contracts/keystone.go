...
func (d *dons) embedOCR3Config(capConfig *capabilitiespb.CapabilityConfig, don donConfig, chainSelector uint64) error {
	...
	ocrConfig, err := ocr3.GenerateOCR3ConfigFromNodes(*oracleConfig, nodes, chainSelector, d.env.OCRSecrets, nil)
	if err != nil {
	...
	capConfig.Ocr3Configs[ocr3.OCR3ConfigDefaultKey] = ocr3Proto
...
func (d *dons) mustToV2ConfigureInput(chainSelector uint64, contractAddress string) cap_reg_v2_seq.ConfigureCapabilitiesRegistryInput {
	...
	for _, don := range d.donsOrderedByID() {
		...
		for _, cap := range don.Capabilities {
			...
			configBytes := []byte("{}")
			if cap.Config != nil {
				// For ConsensusCapabilityV2 DONs, compute and embed the OCR3 config
				if flags.HasFlag(don.flags, cre.ConsensusCapabilityV2) && cap.Capability.CapabilityType == 2 {
