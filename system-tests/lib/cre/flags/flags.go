...
func RequiresForwarderContract(values []string, chainID uint64) bool {
	return HasFlagForChain(values, cre.EVMCapability, chainID) || HasFlagForChain(values, cre.WriteEVMCapability, chainID) || HasFlagForAnyChain(values, cre.WriteSolanaCapability) || HasFlagForAnyChain(values, cre.SolanaCapability)
}
