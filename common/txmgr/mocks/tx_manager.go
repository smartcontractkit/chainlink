...
func (_m *TxManager[CHAIN_ID, HEAD, ADDR, TX_HASH, BLOCK_HASH, SEQ, FEE]) GetTransactionStatus(ctx context.Context, transactionID uuid.UUID) (txmgrtypes.TxState, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, uuid.UUID) (txmgrtypes.TxState, error)); ok {
		return rf(ctx, transactionID)
	}
	if rf, ok := ret.Get(0).(func(context.Context, uuid.UUID) txmgrtypes.TxState); ok {
