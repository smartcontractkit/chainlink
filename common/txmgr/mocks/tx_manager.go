...
func (_m *TxManager[CHAIN_ID, HEAD, ADDR, TX_HASH, BLOCK_HASH, SEQ, FEE]) TxStatusByIdempotencyKey(ctx context.Context, idempotency string) (txmgrtypes.TxState, client.TxError, error) {
	...
	var r1 client.TxError
	var r2 error
	if rf, ok := ret.Get(0).(func(context.Context, string) (txmgrtypes.TxState, client.TxError, error)); ok {
	...
	if rf, ok := ret.Get(1).(func(context.Context, string) client.TxError); ok {
		r1 = rf(ctx, idempotency)
	} else {
		if ret.Get(1) != nil {
			r1 = ret.Get(1).(client.TxError)
		}
	}
