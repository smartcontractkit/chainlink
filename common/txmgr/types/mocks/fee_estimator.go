// Code generated by mockery v2.22.1. DO NOT EDIT.

package mocks

import (
	context "context"

	commontypes "github.com/smartcontractkit/chainlink/common/types"

	mock "github.com/stretchr/testify/mock"

	types "github.com/smartcontractkit/chainlink/common/txmgr/types"
)

// FeeEstimator is an autogenerated mock type for the FeeEstimator type
<<<<<<< HEAD
type FeeEstimator[H types.Head, FEE interface{}, MAXPRICE interface{}, TX_HASH commontypes.Hashable] struct {
=======
type FeeEstimator[H types.Head, F types.Fee, MAXPRICE interface{}, HASH interface{}] struct {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	mock.Mock
}

// BumpFee provides a mock function with given fields: ctx, originalFee, feeLimit, maxFeePrice, attempts
<<<<<<< HEAD
func (_m *FeeEstimator[H, FEE, MAXPRICE, TX_HASH]) BumpFee(ctx context.Context, originalFee FEE, feeLimit uint32, maxFeePrice MAXPRICE, attempts []types.PriorAttempt[FEE, TX_HASH]) (FEE, uint32, error) {
=======
func (_m *FeeEstimator[H, F, MAXPRICE, HASH]) BumpFee(ctx context.Context, originalFee F, feeLimit uint32, maxFeePrice MAXPRICE, attempts []types.PriorAttempt[F, HASH]) (F, uint32, error) {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	ret := _m.Called(ctx, originalFee, feeLimit, maxFeePrice, attempts)

	var r0 F
	var r1 uint32
	var r2 error
<<<<<<< HEAD
	if rf, ok := ret.Get(0).(func(context.Context, FEE, uint32, MAXPRICE, []types.PriorAttempt[FEE, TX_HASH]) (FEE, uint32, error)); ok {
		return rf(ctx, originalFee, feeLimit, maxFeePrice, attempts)
	}
	if rf, ok := ret.Get(0).(func(context.Context, FEE, uint32, MAXPRICE, []types.PriorAttempt[FEE, TX_HASH]) FEE); ok {
=======
	if rf, ok := ret.Get(0).(func(context.Context, F, uint32, MAXPRICE, []types.PriorAttempt[F, HASH]) (F, uint32, error)); ok {
		return rf(ctx, originalFee, feeLimit, maxFeePrice, attempts)
	}
	if rf, ok := ret.Get(0).(func(context.Context, F, uint32, MAXPRICE, []types.PriorAttempt[F, HASH]) F); ok {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
		r0 = rf(ctx, originalFee, feeLimit, maxFeePrice, attempts)
	} else {
		r0 = ret.Get(0).(F)
	}

<<<<<<< HEAD
	if rf, ok := ret.Get(1).(func(context.Context, FEE, uint32, MAXPRICE, []types.PriorAttempt[FEE, TX_HASH]) uint32); ok {
=======
	if rf, ok := ret.Get(1).(func(context.Context, F, uint32, MAXPRICE, []types.PriorAttempt[F, HASH]) uint32); ok {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
		r1 = rf(ctx, originalFee, feeLimit, maxFeePrice, attempts)
	} else {
		r1 = ret.Get(1).(uint32)
	}

<<<<<<< HEAD
	if rf, ok := ret.Get(2).(func(context.Context, FEE, uint32, MAXPRICE, []types.PriorAttempt[FEE, TX_HASH]) error); ok {
=======
	if rf, ok := ret.Get(2).(func(context.Context, F, uint32, MAXPRICE, []types.PriorAttempt[F, HASH]) error); ok {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
		r2 = rf(ctx, originalFee, feeLimit, maxFeePrice, attempts)
	} else {
		r2 = ret.Error(2)
	}

	return r0, r1, r2
}

// Close provides a mock function with given fields:
<<<<<<< HEAD
func (_m *FeeEstimator[H, FEE, MAXPRICE, TX_HASH]) Close() error {
=======
func (_m *FeeEstimator[H, F, MAXPRICE, HASH]) Close() error {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	ret := _m.Called()

	var r0 error
	if rf, ok := ret.Get(0).(func() error); ok {
		r0 = rf()
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// GetFee provides a mock function with given fields: ctx, calldata, feeLimit, maxFeePrice, opts
<<<<<<< HEAD
func (_m *FeeEstimator[H, FEE, MAXPRICE, TX_HASH]) GetFee(ctx context.Context, calldata []byte, feeLimit uint32, maxFeePrice MAXPRICE, opts ...types.Opt) (FEE, uint32, error) {
=======
func (_m *FeeEstimator[H, F, MAXPRICE, HASH]) GetFee(ctx context.Context, calldata []byte, feeLimit uint32, maxFeePrice MAXPRICE, opts ...types.Opt) (F, uint32, error) {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	_va := make([]interface{}, len(opts))
	for _i := range opts {
		_va[_i] = opts[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, ctx, calldata, feeLimit, maxFeePrice)
	_ca = append(_ca, _va...)
	ret := _m.Called(_ca...)

	var r0 F
	var r1 uint32
	var r2 error
	if rf, ok := ret.Get(0).(func(context.Context, []byte, uint32, MAXPRICE, ...types.Opt) (F, uint32, error)); ok {
		return rf(ctx, calldata, feeLimit, maxFeePrice, opts...)
	}
	if rf, ok := ret.Get(0).(func(context.Context, []byte, uint32, MAXPRICE, ...types.Opt) F); ok {
		r0 = rf(ctx, calldata, feeLimit, maxFeePrice, opts...)
	} else {
		r0 = ret.Get(0).(F)
	}

	if rf, ok := ret.Get(1).(func(context.Context, []byte, uint32, MAXPRICE, ...types.Opt) uint32); ok {
		r1 = rf(ctx, calldata, feeLimit, maxFeePrice, opts...)
	} else {
		r1 = ret.Get(1).(uint32)
	}

	if rf, ok := ret.Get(2).(func(context.Context, []byte, uint32, MAXPRICE, ...types.Opt) error); ok {
		r2 = rf(ctx, calldata, feeLimit, maxFeePrice, opts...)
	} else {
		r2 = ret.Error(2)
	}

	return r0, r1, r2
}

// HealthReport provides a mock function with given fields:
<<<<<<< HEAD
func (_m *FeeEstimator[H, FEE, MAXPRICE, TX_HASH]) HealthReport() map[string]error {
=======
func (_m *FeeEstimator[H, F, MAXPRICE, HASH]) HealthReport() map[string]error {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	ret := _m.Called()

	var r0 map[string]error
	if rf, ok := ret.Get(0).(func() map[string]error); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(map[string]error)
		}
	}

	return r0
}

// Name provides a mock function with given fields:
<<<<<<< HEAD
func (_m *FeeEstimator[H, FEE, MAXPRICE, TX_HASH]) Name() string {
=======
func (_m *FeeEstimator[H, F, MAXPRICE, HASH]) Name() string {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	ret := _m.Called()

	var r0 string
	if rf, ok := ret.Get(0).(func() string); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(string)
	}

	return r0
}

// OnNewLongestChain provides a mock function with given fields: ctx, head
<<<<<<< HEAD
func (_m *FeeEstimator[H, FEE, MAXPRICE, TX_HASH]) OnNewLongestChain(ctx context.Context, head H) {
=======
func (_m *FeeEstimator[H, F, MAXPRICE, HASH]) OnNewLongestChain(ctx context.Context, head H) {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	_m.Called(ctx, head)
}

// Ready provides a mock function with given fields:
<<<<<<< HEAD
func (_m *FeeEstimator[H, FEE, MAXPRICE, TX_HASH]) Ready() error {
=======
func (_m *FeeEstimator[H, F, MAXPRICE, HASH]) Ready() error {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	ret := _m.Called()

	var r0 error
	if rf, ok := ret.Get(0).(func() error); ok {
		r0 = rf()
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Start provides a mock function with given fields: _a0
<<<<<<< HEAD
func (_m *FeeEstimator[H, FEE, MAXPRICE, TX_HASH]) Start(_a0 context.Context) error {
=======
func (_m *FeeEstimator[H, F, MAXPRICE, HASH]) Start(_a0 context.Context) error {
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	ret := _m.Called(_a0)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

type mockConstructorTestingTNewFeeEstimator interface {
	mock.TestingT
	Cleanup(func())
}

// NewFeeEstimator creates a new instance of FeeEstimator. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
<<<<<<< HEAD
func NewFeeEstimator[H types.Head, FEE interface{}, MAXPRICE interface{}, TX_HASH commontypes.Hashable](t mockConstructorTestingTNewFeeEstimator) *FeeEstimator[H, FEE, MAXPRICE, TX_HASH] {
	mock := &FeeEstimator[H, FEE, MAXPRICE, TX_HASH]{}
=======
func NewFeeEstimator[H types.Head, F types.Fee, MAXPRICE interface{}, HASH interface{}](t mockConstructorTestingTNewFeeEstimator) *FeeEstimator[H, F, MAXPRICE, HASH] {
	mock := &FeeEstimator[H, F, MAXPRICE, HASH]{}
>>>>>>> bc2f026a1b8b7d7f8472efddde9eae2eabe2d499
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
