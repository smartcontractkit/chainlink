...
type FeeEstimator[H types.Head[TX_HASH], F txmgrtypes.Fee, MAXPRICE interface{}, TX_HASH types.Hashable] struct {
	mock.Mock
}

// BumpFee provides a mock function with given fields: ctx, originalFee, feeLimit, maxFeePrice, attempts
func (_m *FeeEstimator[H, F, MAXPRICE, TX_HASH]) BumpFee(ctx context.Context, originalFee F, feeLimit uint32, maxFeePrice MAXPRICE, attempts []txmgrtypes.PriorAttempt[F, TX_HASH]) (F, uint32, error) {
	ret := _m.Called(ctx, originalFee, feeLimit, maxFeePrice, attempts)

	var r0 F
	var r1 uint32
	var r2 error
	if rf, ok := ret.Get(0).(func(context.Context, F, uint32, MAXPRICE, []txmgrtypes.PriorAttempt[F, TX_HASH]) (F, uint32, error)); ok {
		return rf(ctx, originalFee, feeLimit, maxFeePrice, attempts)
	}
	if rf, ok := ret.Get(0).(func(context.Context, F, uint32, MAXPRICE, []txmgrtypes.PriorAttempt[F, TX_HASH]) F); ok {
		r0 = rf(ctx, originalFee, feeLimit, maxFeePrice, attempts)
	} else {
		r0 = ret.Get(0).(F)
	}

	if rf, ok := ret.Get(1).(func(context.Context, F, uint32, MAXPRICE, []txmgrtypes.PriorAttempt[F, TX_HASH]) uint32); ok {
		r1 = rf(ctx, originalFee, feeLimit, maxFeePrice, attempts)
	} else {
		r1 = ret.Get(1).(uint32)
	}

	if rf, ok := ret.Get(2).(func(context.Context, F, uint32, MAXPRICE, []txmgrtypes.PriorAttempt[F, TX_HASH]) error); ok {
...
func (_m *FeeEstimator[H, F, MAXPRICE, TX_HASH]) GetFee(ctx context.Context, calldata []byte, feeLimit uint32, maxFeePrice MAXPRICE, opts ...txmgrtypes.Opt) (F, uint32, error) {
...
func NewFeeEstimator[H types.Head[TX_HASH], F txmgrtypes.Fee, MAXPRICE interface{}, TX_HASH types.Hashable](t mockConstructorTestingTNewFeeEstimator) *FeeEstimator[H, F, MAXPRICE, TX_HASH] {
