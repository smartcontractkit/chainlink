...
type KeyStore[ADDR types.Hashable[ADDR], ID interface{}, S interface{}] struct {
	mock.Mock
}

// CheckEnabled provides a mock function with given fields: address, chainID
func (_m *KeyStore[ADDR, ID, S]) CheckEnabled(address ADDR, chainID ID) error {
...
func (_m *KeyStore[ADDR, ID, S]) EnabledAddressesForChain(chainId ID) ([]ADDR, error) {
...
func (_m *KeyStore[ADDR, ID, S]) NextSequence(address ADDR, chainID ID, qopts ...pg.QOpt) (S, error) {
	_va := make([]interface{}, len(qopts))
	for _i := range qopts {
		_va[_i] = qopts[_i]
	}
	...
	var r0 S
	var r1 error
	if rf, ok := ret.Get(0).(func(ADDR, ID, ...pg.QOpt) (S, error)); ok {
		return rf(address, chainID, qopts...)
	}
	if rf, ok := ret.Get(0).(func(ADDR, ID, ...pg.QOpt) S); ok {
...
func (_m *KeyStore[ADDR, ID, S]) SubscribeToKeyChanges() (chan struct{}, func()) {
	ret := _m.Called()
...
func NewKeyStore[ADDR types.Hashable[ADDR], ID interface{}, S interface{}](t mockConstructorTestingTNewKeyStore) *KeyStore[ADDR, ID, S] {
	mock := &KeyStore[ADDR, ID, S]{}
	mock.Mock.Test(t)
