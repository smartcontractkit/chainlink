...
func (a *TxAttempt[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) String() string {
...
type Tx[
	...
	CreatedAt          time.Time
	State              TxState
	TxAttempts         []TxAttempt[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE] `json:"-"`
	// Marshalled TxMeta
...
func (e *Tx[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) GetError() error {
	if e.Error.Valid {
...
func (e *Tx[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) GetID() string {
	return fmt.Sprintf("%d", e.ID)
}

// GetMeta returns an Tx's meta in struct form, unmarshalling it from JSON first.
func (e *Tx[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) GetMeta() (*TxMeta[ADDR, TX_HASH], error) {
	if e.Meta == nil {
		return nil, nil
	}
...
func (e *Tx[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) GetLogger(lgr logger.Logger) logger.Logger {
	lgr = lgr.With(
		"txID", e.ID,
		"sequence", e.Sequence,
...
// first.
func (e *Tx[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, R, SEQ, FEE]) GetChecker() (TransmitCheckerSpec[ADDR], error) {
