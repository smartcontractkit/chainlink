...
func (n *node[CHAIN_ID, HEAD, RPC]) aliveLoop() {
	...
	for {
		...
		case <-pollCh:
			...
			lggr.Tracew("Pinging RPC", "nodeState", n.getCachedState(), "pollFailures", pollFailures)
			...
			if err != nil {
				...
				lggr.Debugw("Ping successful", "nodeState", n.getCachedState())
