...
contract UpkeepResetter {
    ...
    function _ResetConsumerBenchmark(address upkeepAddress, uint256 testRange, uint256 averageEligibilityCadence, uint256 firstEligibleBuffer, uint256 checkGasToBurn, uint256 performGasToBurn) private {
    ...
    function ResetManyConsumerBenchmark(address[] memory upkeepAddresses, uint256 testRange, uint256 averageEligibilityCadence, uint256 firstEligibleBuffer, uint256 checkGasToBurn, uint256 performGasToBurn) external {
        for (uint i=0; i<upkeepAddresses.length; i++) {
            _ResetConsumerBenchmark(upkeepAddresses[i], testRange, averageEligibilityCadence, firstEligibleBuffer, checkGasToBurn, performGasToBurn);
        }
    }
