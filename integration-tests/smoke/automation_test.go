...
func TestAutomatedCheckPerformGasLimit(t *testing.T) {
	...
	ocrConfig := actions.BuildAutomationOCR2Config(t, nodesWithoutBootstrap, highCheckGasLimit, registrar.Address(), 5*time.Second)
...
func setupAutomationTest(
	...
	if !onlyStartRunner {
		...
		nodesWithoutBootstrap := chainlinkNodes[1:]
		ocrConfig := actions.BuildAutomationOCR2Config(t, nodesWithoutBootstrap, registryConfig, registrar.Address(), 5*time.Second)
