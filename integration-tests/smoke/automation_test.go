...
var (
	automationConfigTable = map[string]bool{
		"using-env-var-config": true,
		"using-toml-config":    false,
	}
...
func TestAutomatedBasic(t *testing.T) {
	...
	for name, u := range automationConfigTable {
		useEnvVars := u
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			chainClient, _, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "basic-upkeep", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
...
func TestAutomatedAddFunds(t *testing.T) {
	...
	for name, u := range automationConfigTable {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			chainClient, _, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "add-funds", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
...
func TestAutomatedPauseUnPause(t *testing.T) {
	...
	for name, u := range automationConfigTable {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			chainClient, _, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "pause-unpause", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
...
func TestAutomatedRegisterUpkeep(t *testing.T) {
	...
	for name, u := range automationConfigTable {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			chainClient, _, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "register-upkeep", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
...
func TestAutomatedPauseRegistry(t *testing.T) {
	...
	for name, u := range automationConfigTable {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			chainClient, _, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "pause-registry", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
	...
	for name, u := range automationConfigTable {
		...
		t.Run(name, func(t *testing.T) {
			...
			chainClient, chainlinkNodes, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "keeper-nodes-down", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
...
func TestAutomatedPerformSimulation(t *testing.T) {
	...
	for name, u := range automationConfigTable {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			chainClient, _, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "perform-simulation", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
...
func TestAutomatedCheckPerformGasLimit(t *testing.T) {
	...
	for name, u := range automationConfigTable {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			chainClient, chainlinkNodes, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "gas-limit", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
...
func TestUpdateCheckData(t *testing.T) {
	...
	for name, u := range automationConfigTable {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			chainClient, _, contractDeployer, linkToken, registry, registrar, onlyStartRunner := setupAutomationTest(
				t, "update-check-data", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, useEnvVars,
			)
			if onlyStartRunner {
				return
			}
