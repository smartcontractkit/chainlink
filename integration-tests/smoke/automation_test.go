...
ListenAddresses = ["0.0.0.0:6690"]`
	...
	defaultRegistryConfig = contracts.KeeperRegistrySettings{
		PaymentPremiumPPB:    uint32(200000000),
		FlatFeeMicroLINK:     uint32(0),
		BlockCountPerTurn:    big.NewInt(10),
		CheckGasLimit:        uint32(2500000),
...
func setupAutomationTestEnvironment(
	t *testing.T,
	testName string,
	...
	testEnvironment := environment.New(&environment.Config{
		...
		AddHelm(chainlink.New(0, map[string]interface{}{
			"replicas": "5",
			"toml":     client.AddNetworksConfig(automationTOML, network),
		}))
	...
	t.Cleanup(func() { // Register cleanup for any test
...
func TestAutomatedBasic(t *testing.T) {
	t.Parallel()
	chainClient, _, contractDeployer, linkToken, registry, registrar := setupAutomationTestEnvironment(
...
// 			latestCounter, err := consumers[i].Counter(context.Background())
// 			g.Expect(err).ShouldNot(gomega.HaveOccurred(), "Failed to retrieve consumer counter for upkeep at index "+strconv.Itoa(i))
...
// 			x Entry("v2.0 Basic smoke test @simulated", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, BasicCounter, BasicSmokeTest, big.NewInt(defaultLinkFunds), numberOfUpkeeps),
// 			x Entry("v2.0 Add funds to upkeep test @simulated", ethereum.RegistryVersion_2_0, defaultOCRRegistryConfig, BasicCounter, AddFundsToUpkeepTest, big.NewInt(1), numberOfUpkeeps),
