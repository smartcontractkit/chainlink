...
env DOCKER_DEFAULT_PLATFORM=linux/amd64 docker buildx build --platform linux/amd64 -f ./core/chainlink.Dockerfile --build-arg ENVIRONMENT=release --build-arg COMMIT_SHA=$(git rev-parse HEAD) --build-arg VERSION_TAG=$(git describe --always) --build-arg DOCKER_TAG=develop-$(git rev-parse HEAD) -t smartcontract/chainlink:develop-$(git rev-parse HEAD) .
