...
func Test_CCIP_TokenTransfer_LnR_EVM2Aptos(t *testing.T) {
	...
	evmToken, _, aptosToken, aptosTokenPool, err := testhelpers.DeployLnRTokenAptos(t, lggr, e.Env, sourceChain, destChain, "TOKEN", nil)
	require.NoError(t, err)

	// Provide liquidity to the Aptos LnR token pool
	payload, err := aptos.FungibleAssetTransferPayload(&aptosToken, deployerAddressDestChain, aptosTokenPool.Address(), 100e8)
	require.NoError(t, err)
