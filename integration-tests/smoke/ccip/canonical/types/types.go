...
type Adapter interface {
	...
	ValidateCommit(t *testing.T, sourceSelector uint64, seqNumRange ccipocr3.SeqNumRange)

	// ValidateExec validates that the message specified by the given send event was executed.
	ValidateExec(t *testing.T, sourceSelector uint64, seqNrs []uint64)
}
...
type ExecContext interface {
	...
	// OnchainState returns the onchain state of the environment.
	...
	ReplayLogs(t *testing.T, selectorToBlockMap map[uint64]uint64)
}
