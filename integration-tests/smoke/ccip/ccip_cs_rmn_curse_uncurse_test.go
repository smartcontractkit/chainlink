...
import (
	...
	cldchangesetsMCMS "github.com/smartcontractkit/cld-changesets/legacy/mcms/changesets"
...
func transferRMNContractToMCMS(t *testing.T, e *testhelpers.DeployedEnv, state stateview.CCIPOnChainState) {
	...
	cfgAmounts := cldchangesetsMCMS.AmountsToTransfer{
	...
	amountsPerChain := make(map[uint64]cldchangesetsMCMS.AmountsToTransfer)
	...
	config := cldchangesetsMCMS.FundMCMSignerConfig{
		AmountsPerChain: amountsPerChain,
	}

	changesetInstance := cldchangesetsMCMS.FundMCMSignersChangeset{}
