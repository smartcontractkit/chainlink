...
func Test_PricingForTokenTransfers(t *testing.T) {
	...
	t.Run("Send Token Pay with Link token home chain -> remote", func(t *testing.T) {
		...
		seqNum := changeset.TestSendRequest(t, e, state, src, dest, false, ccipMessage).SequenceNumber
		expectedSeqNum[sourceDestPair] = seqNum
	...
	t.Run("Send Token pay with wrapped native home chain -> remote", func(t *testing.T) {
		...
		wrappedNative, err := state.Chains[src].Router.GetWrappedNative(nil)
		require.NoError(t, err)
		require.Equal(t, WETH.Address(), wrappedNative)
		...
		depositOps.Value = new(big.Int).Mul(twoCoins, big.NewInt(40))
		tx, err := WETH.Deposit(&depositOps)
		require.NoError(t, err)
		_, err = e.Chains[tenv.HomeChainSel].Confirm(tx)
		require.NoError(t, err)

		// Approve to spend 100 WETH token as feeToken
		tx, err = WETH.Approve(e.Chains[tenv.HomeChainSel].DeployerKey, state.Chains[src].Router.Address(), new(big.Int).Mul(twoCoins, big.NewInt(50)))
		require.NoError(t, err)
		_, err = e.Chains[tenv.HomeChainSel].Confirm(tx)
		require.NoError(t, err)
	...
	t.Run("Send Token but revert not enough tokens", func(t *testing.T) {
		...
		tx, _, err = changeset.CCIPSendRequest(
			e,
			state,
			src, dest,
			false,
...
func Test_PricingForMessages(t *testing.T) {
	...
	emptyTokenArray := []router.ClientEVMTokenAmount{}
	...
	t.Run("Send programmable token transfer", func(t *testing.T) {
