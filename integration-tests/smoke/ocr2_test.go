...
func TestOCRv2Basic(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"legacy", map[string]string{string(env.MedianPluginCmd): ""}, false},
		{"plugins", map[string]string{string(env.MedianPluginCmd): "chainlink-feeds"}, false},
		{"chain-reader", map[string]string{string(env.MedianPluginCmd): "chainlink-feeds"}, true},
		{"plugins-chain-reader", map[string]string{string(env.MedianPluginCmd): "chainlink-feeds"}, true},
	} {
		test := test
