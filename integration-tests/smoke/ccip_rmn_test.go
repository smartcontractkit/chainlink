...
func runRmnTestCase(t *testing.T, tc rmnTestCase) {
	...
	expectedSeqNumExec := make(map[ccipdeployment.SourceDestPair][]uint64)
	for _, msg := range tc.messagesToSend {
		...
		for i := 0; i < msg.count; i++ {
			...
			expectedSeqNumExec[ccipdeployment.SourceDestPair{
				SourceChainSelector: fromChain,
				DestChainSelector:   toChain,
	...
	if tc.waitForExec {
		...
		ccipdeployment.ConfirmExecWithSeqNrsForAll(t, envWithRMN.Env, onChainState, expectedSeqNumExec, startBlocks)
		t.Logf("✅ Exec report")
	}
}
