...
func ExecuteBasicLogPollerTest(t *testing.T, cfg *Config) {
	...
	chainClient, _, contractDeployer, linkToken, registry, registrar, testEnv := setupLogPollerTestDocker(
		t, testName, ethereum.RegistryVersion_2_1, defaultOCRRegistryConfig, false, time.Duration(500*time.Millisecond), 10, false,
	)
...
func ExecuteBackupLogPollerTest(t *testing.T, cfg *Config, backupPollerTimeout string) {
	l := logging.GetTestLogger(t)
	coreLogger := core_logger.TestLogger(t) //needed by ORM ¯\_(ツ)_/¯
	...
	chainClient, _, contractDeployer, linkToken, registry, registrar, testEnv := setupLogPollerTestDocker(
		t, testName, ethereum.RegistryVersion_2_1, defaultOCRRegistryConfig, false, time.Duration(1000*time.Millisecond), 10, false,
	)
	...
	// let's make sure that main log poller process has advanced beyond block in which we stopped emitting logs
	// so that we can be sure these logs are not picked up by regular process
	endBlockWaitDuration := "1m"
	l.Warn().Str("Duration", endBlockWaitDuration).Msg("Waiting log poller to process end block")

	gom := gomega.NewGomegaWithT(t)
	gom.Eventually(func(g gomega.Gomega) {
		isProcessed, err := waitForEndBlockInLogPoller(endBlock, testEnv.EVMClient.GetChainID(), l, coreLogger, testEnv.ClCluster)
		if err != nil {
			l.Warn().Err(err).Msg("Error checking if log poller has processed end block. Retrying...")
		}
		g.Expect(isProcessed).To(gomega.BeTrue(), "Log poller has not processed end block")
	}, endBlockWaitDuration, "5s").Should(gomega.Succeed())
	...
	require.NoError(t, err, "Error checking if CL nodes have no logs yet count")
	require.True(t, logCountMatches, "At least one CL node already had logs in DB")
	l.Info().Msg("No logs were saved by CL nodes yet, as expected. Registering filters.")
	...
	l.Info().Int("End block", int(endBlock)).Int("Start block", int(startBlock)).Msg("Block range that backup poller has to process")
	l.Warn().Str("Duration", backupPollerTimeout).Msg("Waiting for logs to be processed by all nodes' backup poller processes")
	...
	gom.Eventually(func(g gomega.Gomega) {
		...
		g.Expect(logCountMatches).To(gomega.BeTrue(), "Not all CL nodes have expected log count")
	}, backupPollerTimeout, "10s").Should(gomega.Succeed())
...
func ExecuteBackupLogPollerReplay(t *testing.T, cfg *Config, consistencyTimeout string) {
	...
	chainClient, _, contractDeployer, linkToken, registry, registrar, testEnv := setupLogPollerTestDocker(
		t, testName, ethereum.RegistryVersion_2_1, defaultOCRRegistryConfig, false, time.Duration(200*time.Millisecond), 10, false,
	)
