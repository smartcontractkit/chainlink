...
%s`
	...
	if sethCfg.IsSimulatedNetwork() {
		require.GreaterOrEqual(t, int(*sethCfg.EphemeralAddrs), 50, "You need to use at least 50 ephemeral addresses on a simulated network, but %s was used. Please update '[Seth] ephemeral_addresses_number' field in the TOML config file", *sethCfg.EphemeralAddrs)
		...
		ephmeralMsg := `You must not use any ephemeral addresses on a live network. All funds send to them will be lost. You prefund some addresses instead manually and pass their private keys in network configuration or
		use Seth key CLI to do that. But if you really know what you are doing remove this check from the test.`
		require.Equal(t, 0, *sethCfg.EphemeralAddrs, ephmeralMsg)
		addressesMsg := `You need to use at least 50 addresses on a live network. Otherwise load generation will surely fail. Please update '[Network.NETWORK_NAME] private_keys_secret=[]' field in the TOML config file
		or set SETH_KEYFILE_PATH env var with path to keyfile.toml generated by Seth key CLI.`
		require.GreaterOrEqual(t, len(sethCfg.Network.PrivateKeys), 50, addressesMsg)
	}
