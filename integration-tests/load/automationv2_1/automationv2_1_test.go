...
%s`
	...
	if sethCfg.IsSimulatedNetwork() {
		require.GreaterOrEqual(t, int(*sethCfg.EphemeralAddrs), 10, "You need to use at least 10 ephemeral addresses on a simulated network, but %s was used. Please update '[Seth] ephemeral_addresses_number' field in the TOML config file", *sethCfg.EphemeralAddrs)
	} else {
		...
		addressesMsg := `You need to use at least 10 addresses on a live network. Otherwise load generation will surely fail. Please update '[Network.NETWORK_NAME] private_keys_secret=[]' field in the TOML config file
		or set SETH_KEYFILE_PATH env var with path to keyfile.toml generated by Seth key CLI.`
		require.GreaterOrEqual(t, len(sethCfg.Network.PrivateKeys), 10, addressesMsg)
	}
