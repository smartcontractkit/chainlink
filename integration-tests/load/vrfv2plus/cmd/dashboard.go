			...
			dashboard.Row("LoadContractMetrics",
				row.WithTimeSeries(
					...
					timeseries.WithPrometheusTarget(
						`
					last_over_time({type="vrfv2plus_contracts_load_summary", network=~"${network:pipe}", test_type=~"${test_type:pipe}", run_id=~"${run_id:pipe}", go_test_name=~"${go_test_name:pipe}", branch=~"${branch:pipe}", commit=~"${commit:pipe}", gen_name=~"${gen_name:pipe}"}
					| json
