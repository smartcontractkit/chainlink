...
func ValidateAddressesTypeAndNumber(sethCfg *seth.Config, minKeyNumber int) error {
	if sethCfg.IsSimulatedNetwork() {
		...
		if int(*sethCfg.EphemeralAddrs) > 0 {
			msg := `You must not use any ephemeral addresses on a live network, but %d were set. All funds send to them will be lost. You prefund some addresses instead manually and pass their private keys in network configuration or
		...
		if len(sethCfg.Network.PrivateKeys) < minKeyNumber {
			msg := `You need to use at least %d addresses on a live network, but %d were passed. With current setup load generation will surely fail. Please update '[Network.NETWORK_NAME] private_keys_secret=[]' field in the TOML config file
