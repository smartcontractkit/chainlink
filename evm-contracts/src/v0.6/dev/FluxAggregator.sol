...
contract FluxAggregator is AggregatorInterface, Owned {
  ...
  uint32 public maxResponseCount;
  uint32 public minResponseCount;
  ...
  event RoundDetailsUpdated(
    ...
    uint32 indexed minResponseCount,
    uint32 indexed maxResponseCount,
    uint32 restartDelay,
   ...
   * @param _roundId is the ID of the round this response pertains to
   * @param _response is the updated data that the oracle is submitting
   */
  function updateAnswer(uint256 _roundId, int256 _response)
    external
  {
    ...
    recordSubmission(_response, uint32(_roundId));
   ...
   * @param _minResponses is the new minimum response count for each round
   * @param _maxResponses is the new maximum response count for each round
    ...
    onlyOwner()
  {
    ...
    updateFutureRounds(paymentAmount, _minResponses, _maxResponses, _restartDelay, timeout);
  }
    ...
    onlyOwner()
  {
    ...
    require(_maxResponses >= _minResponses, "max must equal/exceed min");
    require(oracleNum >= _maxResponses, "max cannot exceed total");
    ...
    paymentAmount = _paymentAmount;
    minResponseCount = _minResponses;
    maxResponseCount = _maxResponses;
  ...
  {
    return (oracles[_oracle].latestResponse, oracles[_oracle].lastReportedRound);
  }
    ...
    private
  {
    ...
    rounds[_roundId].details.maxResponses = maxResponseCount;
    rounds[_roundId].details.minResponses = minResponseCount;
  ...
  function updateRoundAnswer(uint32 _roundId)
    private
  {
    if (rounds[_roundId].details.responses.length < rounds[_roundId].details.minResponses) return;

    int256 newAnswer = Median.calculateInplace(rounds[_roundId].details.responses);
  ...
  function recordSubmission(int256 _response, uint32 _roundId)
    private
  {
    require(acceptingSubmissions(_roundId), "round not accepting anwers");

    rounds[_roundId].details.responses.push(_response);
    oracles[msg.sender].lastReportedRound = _roundId;
    oracles[msg.sender].latestResponse = _response;

    emit SubmissionReceived(_response, _roundId, msg.sender);
  }

  function deleteRoundDetails(uint32 _roundId)
    private
  {
    if (rounds[_roundId].details.responses.length < rounds[_roundId].details.maxResponses) return;
    ...
    )
  {
    ...
    return (
      _eligibleToSubmit,
      _roundId,
      oracles[_oracle].latestResponse,
  ...
  function acceptingSubmissions(uint32 _roundId)
    ...
    returns (bool)
  {
    return rounds[_roundId].details.maxResponses != 0;
  }
