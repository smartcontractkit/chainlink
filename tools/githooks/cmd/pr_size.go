...
func newPRSizeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := prsize.Run(ctx, cfg); err != nil {
				if errors.Is(err, prsize.ErrLargePR) {
	...
	cmd.Flags().BoolVar(&allowLargePR, "allow-large-pr", false, "Allow large PRs without failing (or set ALLOW_LARGE_PR=true)")
