...
func TestBuildCCIPSystemMatrix_DefaultQuarantine(t *testing.T) {
	t.Parallel()
...
func TestBuildCCIPSystemMatrix_RunMixedVersionTests(t *testing.T) {
	t.Parallel()
...
func TestBuildCCIPSystemMatrix_MixedVersionOnly_Precedence(t *testing.T) {
	t.Parallel()

	// Release rollout callers set MixedVersionOnly=true without RunMixedVersionTests;
	// it must still return mixed-version tests (preventing PR #23790 bash bug).
	res, err := matrix.BuildCCIPSystemMatrix(context.Background(), matrix.CCIPSystemOptions{
		RunID:                "112233",
		RunAttempt:           "1",
		SpotFlag:             "spot=co",
		MixedVersionOnly:     true,
		RunMixedVersionTests: false,
	})
	require.NoError(t, err)
	require.Len(t, res, 2)
	assert.Equal(t, "Test_CCIPMixedVersionDON", res[0].TestName)
