...
import "testing"
func Test_CRE_Example(t *testing.T) {}
`
	...
	err := rootCmd.Execute()
	require.NoError(t, err)
	require.Contains(t, stdout.String(), `"test_name":"Test_CRE_Example"`)
...
func TestChangelogSubcommand(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(pkgPath, []byte(`{"version": "1.0.0"}`), 0o600))
