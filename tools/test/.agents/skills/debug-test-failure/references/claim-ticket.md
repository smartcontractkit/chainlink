...
<pre-requisites>
1. Ticket status check: either in `Open` status or in any other status AND assigned to current user. If not,
    do not claim the ticket in `skip_reason` say that someone else is already working on it.
...
3. Follow [transition-ticket.md](./transition-ticket.md) with `jira_key` and `target = "In Progress"`.
   - If the transition fails: unasign the ticket.
