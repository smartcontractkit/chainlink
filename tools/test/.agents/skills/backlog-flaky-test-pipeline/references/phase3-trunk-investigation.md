...
<purpose>
Retrieve observational evidence from Trunk and optionally from a specific CI run. Writes `actionable_facts`, `ci_run_evidence`, `trunk_investigation_status`, and `trunk_analysis_url` to the calling context.
</purpose>
...
**If response has `summary` or `facts`**: set `trunk_investigation_status = "existing"`. If `ci_run_url` is non-null, proceed to `<step id="investigate-ci-failure">`. Otherwise done.

**If response is empty** (no investigation yet) — branch on `ci_run_url`:

**Branch A: `ci_run_url` is non-null** → skip polling; rely on `investigate-ci-failure`. Set `trunk_investigation_status = "ci_run_only"`. Proceed to `<step id="investigate-ci-failure">`.
...
- Options:
  - "Attach a CI run URL" — user provides URL via the Other field → set `ci_run_url`, proceed to `investigate-ci-failure`; set `trunk_investigation_status = "ci_run_only"`.
  - "Trigger a new investigation (2–5 min)" → fall through to Branch C.
  - "Skip this ticket" → set `trunk_investigation_status = "uninvestigated"`; return.
...
- Non-empty result → set `trunk_investigation_status = "triggered"`. Done.
- Still empty after 5 minutes → set `trunk_investigation_status = "uninvestigated"`. Ask using `AskUserQuestion`:
...
- `trunk_investigation_status: "existing" | "triggered" | "ci_run_only" | "uninvestigated"`
