...
<initialization>
1. Verify target: test, package, JIRA issue, or N JIRA flaky-test tickets. Prompt user if unknown.
2. Read JIRA skip_reason. Ask user for guidance.
3. Open non-Trunk CI links (e.g. GitHub Actions). Find stack trace and logs.
4. Run bounded diagnosis (`--fail-fast-on=(timeout|slow)` or low `--iterations`) if no failure details.
5. Define hypothesis: flake, timeout, slow, panic, deadlock, race.
</initialization>
...
<loop>
1. Plan run using parallel profile. Execute.
2. Escalate iterations if no issues. Keep parallelism. Deep profile requires user approval if slow. Standard profile required minimum after fix.
3. Focus targeted issues.
4. Read `diagnose-attempted-fixes-[test/package]-[flake/broken/timeout/slow].jsonl`.
5. Form hypothesis.
6. Implement fix.
7. Output hypothesis, fix, rationale.
8. Run diagnose (Standard minimum). Read `report.json`. Append JSONL:
```json
...
```
9. GOTO 2.
10. Run `golangci-lint`. Fix lint errors.
Resume loop on user interrupt.
</loop>
