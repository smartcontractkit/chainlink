...
For repeated runs with Postgres setup, `go test -json` capture, and machine-readable reports under `diagnose-*` directories, use the harness from the **repository root** (`go -C tools/test`, declared in the root `go.mod`):

```sh
go -C tools/test diagnose --iterations 50 -- --failfast ./path/to/package
```
