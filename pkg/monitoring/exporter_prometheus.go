...
// old value != new value && old timestamp == new timestap => This is probably and error since any new transmission updates the timestamp as well, but, to record the observation, we return true.
// old value != new value && old timestamp != new timestap => return true
// old value == new value && old timestamp != new timestap => An unlikely case given the high precision of observations but still a valid update. Return true
