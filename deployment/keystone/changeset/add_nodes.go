...
func AddNodes(env deployment.Environment, req *AddNodesRequest) (deployment.ChangesetOutput, error) {
	...
	if useMCMS {
		...
		out.MCMSTimelockProposals = []mcms.TimelockProposal{*proposal} //nolint:staticcheck //SA1019 ignoring deprecated field for compatibility; we don't have tools to generate the new field
	}
	return out, nil
}
