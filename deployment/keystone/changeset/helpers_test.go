...
// TODO: seperate the config into different types; wf should expand to types of ocr keybundles; writer to target chains; ...
...
func Setup(t *testing.T, c TestConfig) TestEnv {
	...
	gotWfNodes, err := gotRegistry.GetNodesByP2PIds(nil, p2pIDs(t, maps.Keys(wfNodes)))
	require.NoError(t, err)
	require.Len(t, gotWfNodes, len(wfNodes))
	gotCwNodes, err := gotRegistry.GetNodesByP2PIds(nil, p2pIDs(t, maps.Keys(cwNodes)))
	require.NoError(t, err)
	require.Len(t, gotCwNodes, len(cwNodes))
	gotAssetNodes, err := gotRegistry.GetNodesByP2PIds(nil, p2pIDs(t, maps.Keys(assetNodes)))
	require.NoError(t, err)
	require.Len(t, gotAssetNodes, len(assetNodes))

	// check the dons
	gotDons, err := gotRegistry.GetDONs(nil)
	require.NoError(t, err)
	require.Len(t, gotDons, len(allDons))
	...
	if c.EnableMCMS {
		...
		require.NotNil(t, gotRegistry)
		// check the MCMS
		gotMCMS := regChainContracts.MCMSWithTimelockState
...
func TestSetup(t *testing.T) {
	...
	for _, useMCMS := range []bool{true, false} {
		...
		t.Run(fmt.Sprintf("use MCMS %T", useMCMS), func(t *testing.T) {
