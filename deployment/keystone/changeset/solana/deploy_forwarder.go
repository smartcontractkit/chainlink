...
// TODO move everything below to helpers
// Configuration
const (
...
func generateVanityKeys(e cldf.Environment, keys map[cldf.ContractType]string) error {
	...
	for program, prefix := range programToVanityKey {
		...
		destination := filepath.Join(cloneDir, deployDir, "keystone_forwarder"+"-keypair.json")
...
func buildProject(e cldf.Environment) error {
	...
	args := []string{"build_contracts_local"}
...
func BuildSolana(e cldf.Environment, config *BuildSolanaConfig) error {
	//TODO add build in CLD environment handle here
	e.Logger.Debug("Building Solana Keystone program artifacts locally...")
	err := buildLocally(e, config)
	if err != nil {
		return fmt.Errorf("error building solana keystone program artifacts: %w", err)
	}
