...
func generateOracleFactory(cldEnv cldf.Environment, nodeInfo deployment.Node, job pkg.StandardCapabilityJob) (*pkg.OracleFactory, error) {
	...
	if job.UseCapRegOCRConfig {
		capRegVersion, vErr := semver.NewVersion(job.CapRegVersion)
		if vErr != nil {
			return &pkg.OracleFactory{}, fmt.Errorf("failed to parse capRegVersion %q: %w", job.CapRegVersion, vErr)
		}
		addrRefKey = pkg.GetCapRegAddressRefKey(uint64(contractChainSelector), capRegVersion, job.CapRegQualifier)
	} else {
