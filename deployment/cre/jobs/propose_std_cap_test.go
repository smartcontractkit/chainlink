...
func TestProposeStandardCapabilityJob_Apply_HTTPTrigger_WithOracleFactory(t *testing.T) {
	testEnv := test.SetupEnvV2(t, false)
	env := testEnv.Env

	input := jobs.ProposeStandardCapabilityJobInput{
		JobName:       "http-trigger-with-oracle",
		Command:       "http_trigger",
		Config:        `{}`,
		ExternalJobID: "http-trigger-oracle-id",
		DONName:       "test-don",
		DONFilters: []offchain.TargetDONFilter{
			{Key: offchain.FilterKeyDONName, Value: "don-" + test.DONName},
			{Key: "environment", Value: "test"},
	...
	// Verify the job specs contain expected configuration
	found := false
	for _, req := range reqs {
		if strings.Contains(req.Spec, `name = "http-trigger-with-oracle"`) {
			found = true
			t.Logf("HTTP Trigger with Oracle Job Spec:\n%s", req.Spec)
			assert.Contains(t, req.Spec, `command = "http_trigger"`)
			assert.Contains(t, req.Spec, `config = """{}"""`)
			assert.Contains(t, req.Spec, `externalJobID = "http-trigger-oracle-id"`)
		}
	}
	assert.True(t, found, "Expected to find http-trigger-with-oracle job spec")
}

func TestProposeStandardCapabilityJob_Apply_HTTPAction_MinimalConfig(t *testing.T) {
	testEnv := test.SetupEnvV2(t, false)
	env := testEnv.Env

	input := jobs.ProposeStandardCapabilityJobInput{
		JobName: "http-action-minimal",
		Command: "http_action",
		Config:  `{"proxyMode": "direct"}`,
		DONName: "test-don",
		DONFilters: []offchain.TargetDONFilter{
	...
	found := false
	for _, req := range reqs {
		if strings.Contains(req.Spec, `name = "http-action-minimal"`) {
			found = true
			t.Logf("HTTP Action Minimal Job Spec:\n%s", req.Spec)
			assert.Contains(t, req.Spec, `command = "http_action"`)
			assert.Contains(t, req.Spec, `config = {"proxyMode": "direct"}`)
			// ExternalJobID should be empty/not present for minimal config
		}
	}
	assert.True(t, found, "Expected to find http-action-minimal job spec")
}
