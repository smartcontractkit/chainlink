...
func TestUserWorkflowUpsert(t *testing.T) {
	t.Parallel()
	...
	t.Run("link-owner upsert pause activate delete unlink-owner", func(t *testing.T) {
		...
		deployerKey := fixture.rt.Environment().BlockChains.EVMChains()[fixture.selector].DeployerKey
...
func generateAndSignOwnershipProof(
	...
	testAddress, signerPrivateKey, signerAddress, orgID, nonce, version string, requestType uint8,
	...
	privKey, err := crypto.HexToECDSA(signerPrivateKey)
	require.NoError(t, err, "failed to convert private key from hex")
