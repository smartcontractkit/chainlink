...
func setupRegistryForUpdateDON(t *testing.T, isWorkflow, useMCMS bool) *updFixture {
	...
	aptosWriteChain := capabilities_registry_v2.CapabilitiesRegistryCapability{
		CapabilityId:          "aptos:ChainSelector:13456@1.0.0",
		ConfigurationContract: common.Address{},
		Metadata:              []byte(`{"capabilityType": 3, "responseType": 1}`),
	}
	var aptosWriteChainMeta map[string]any
	require.NoError(t, json.Unmarshal(aptosWriteChain.Metadata, &aptosWriteChainMeta))
	...
	nodes := []changeset.CapabilitiesRegistryNodeParams{
		{
			...
			CsaKey:              csaKey,
			CapabilityIDs:       []string{writeChain.CapabilityId, trigger.CapabilityId, aptosWriteChain.CapabilityId},
		},
		{
			NOP:                 nop2,
			Signer:              signer2,
	...
	_, err = changeset.ConfigureCapabilitiesRegistry{}.Apply(rt.Environment(), changeset.ConfigureCapabilitiesRegistryInput{
		...
		Capabilities: []changeset.CapabilitiesRegistryCapability{
			...
			{CapabilityID: aptosWriteChain.CapabilityId, Metadata: aptosWriteChainMeta},
		},
		Nodes: nodes,
		DONs: []changeset.CapabilitiesRegistryNewDONParams{
			{
				...
				CapabilityConfigurations: []changeset.CapabilitiesRegistryCapabilityConfiguration{
					...
					{CapabilityID: aptosWriteChain.CapabilityId, Config: cfg},
				},
				Nodes:            nodeSet,
				F:                1,
				IsPublic:         true,
