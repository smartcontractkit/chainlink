...
func TestBuildProposalFromBatchesV2(t *testing.T) {
	...
	timelocksPerChain := map[uint64]common.Address{
		chainSelector: timelockAddress,
	}

	sig, err := gethwrappers.NewManyChainMultiSig(mcmsState.ProposerMcm.Address(), env.Chains[chainSelector].Client)
	require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			proposal, err := proposalutils.BuildProposalFromBatchesV2(timelocksPerChain, proposerMcmsesPerChain, tt.batches, description, minDelay)
			if tt.wantErr {
				require.Error(t, err)
