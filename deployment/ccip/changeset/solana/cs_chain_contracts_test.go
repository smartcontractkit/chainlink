...
func TestSetOcr3(t *testing.T) {
	...
	_, _ = testhelpers.TransferOwnershipSolana(t, &tenv.Env, solChainSelectors[0], true, true, true, true, false, nil, nil)
...
func TestPoolLookupTable(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.Msg, func(t *testing.T) {
			...
			if test.Mcms {
				_, _ = testhelpers.TransferOwnershipSolana(t, &tenv.Env, solChain, true, true, true, true, false, nil, nil)
				mcmsConfig = &ccipChangesetSolana.MCMSConfigSolana{
					MCMS: &ccipChangeset.MCMSConfig{
						MinDelay: 1 * time.Second,
					},
