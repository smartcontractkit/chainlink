...
// use this to build locally and test
func TestDeployAndUpgradeLocally(t *testing.T) {
	ci := os.Getenv("CI") == "true"
	if ci {
		t.Skip("Local only")
	}
	if os.Getenv("TEST_SOLANA_UPGRADE_LOCALLY") != "true" {
		t.Skip("User enforced only")
	}
	t.Parallel()
	lggr := logger.TestLogger(t)
	e := memory.NewMemoryEnvironment(t, lggr, zapcore.InfoLevel, memory.MemoryEnvironmentConfig{
