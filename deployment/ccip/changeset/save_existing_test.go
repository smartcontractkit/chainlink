...
func TestSaveExistingCCIP(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, state.EVMState.Chains[chain1].LinkToken.Address(), common.BigToAddress(big.NewInt(1)))
	require.Equal(t, state.EVMState.Chains[chain1].Weth9.Address(), common.BigToAddress(big.NewInt(2)))
	require.Equal(t, state.EVMState.Chains[chain1].TokenAdminRegistry.Address(), common.BigToAddress(big.NewInt(3)))
	require.Equal(t, state.EVMState.Chains[chain2].RegistryModule.Address(), common.BigToAddress(big.NewInt(4)))
	require.Equal(t, state.EVMState.Chains[chain2].Router.Address(), common.BigToAddress(big.NewInt(5)))
}
