...
func TestValidateSyncUSDCDomainsWithChainsConfig(t *testing.T) {
	...
	tests := []struct {
		Msg        string
		Input      func(selector uint64) changeset.SyncUSDCDomainsWithChainsConfig
		ErrStr     string
		DeployUSDC bool
	}{
		{
			...
			Input: func(selector uint64) changeset.SyncUSDCDomainsWithChainsConfig {
				return changeset.SyncUSDCDomainsWithChainsConfig{}
			},
...
func TestSyncUSDCDomainsWithChainsChangeset(t *testing.T) {
	...
	for _, mcmsConfig := range []*changeset.MCMSConfig{nil, &changeset.MCMSConfig{MinDelay: 0 * time.Second}} {
		...
		t.Run(msg, func(t *testing.T) {
			...
			e, err = commoncs.Apply(t, e, timelockContracts,
				commonchangeset.Configure(
					deployment.CreateLegacyChangeSet(changeset.SyncUSDCDomainsWithChainsChangeset),
					changeset.SyncUSDCDomainsWithChainsConfig{
						MCMS: mcmsConfig,
						USDCVersionByChain: map[uint64]semver.Version{
