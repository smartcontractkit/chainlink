...
func createBufferIdlInstruction(e cldf.Environment, programID, buffer, authority solana.PublicKey) (solana.GenericInstruction, error) {
	accounts, instruction, err2 := getAccountsForCreateBufferIdlInstruction(e, programID, buffer, authority)
	if err2 != nil {
		return instruction, err2
	}
	return buildIdlInstruction(programID, accounts, IdlInstructionCreateBuffer)
}
...
func upgradeIDLIx(e cldf.Environment, programsPath, programID, programName string, c IDLConfig) (*mcmsTypes.Transaction, error) {
	...
	instruction, err := createBufferIdlInstruction(e, solana.MustPublicKeyFromBase58(programID), buffer, authority)
	if err != nil {
...
func SetAuthorityIDL(e cldf.Environment, c IDLConfig) (cldf.ChangesetOutput, error) {
	...
	if c.Router {
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, chainState.Router.String(), deployment.RouterProgramName, "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
	...
	if c.FeeQuoter {
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, chainState.FeeQuoter.String(), deployment.FeeQuoterProgramName, "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
	...
	if c.OffRamp {
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, chainState.OffRamp.String(), deployment.OffRampProgramName, "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
	...
	if c.RMNRemote {
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, chainState.RMNRemote.String(), deployment.RMNRemoteProgramName, "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
	...
	for _, bnmMetadata := range c.BurnMintTokenPoolMetadata {
		tokenPool := chainState.GetActiveTokenPool(shared.BurnMintTokenPool, bnmMetadata)
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, tokenPool.String(), deployment.BurnMintTokenPoolProgramName, "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
	...
	for _, lrMetadata := range c.LockReleaseTokenPoolMetadata {
		tokenPool := chainState.GetActiveTokenPool(shared.LockReleaseTokenPool, lrMetadata)
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, tokenPool.String(), deployment.LockReleaseTokenPoolProgramName, "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
	...
	if c.CCTPTokenPool {
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, chainState.CCTPTokenPool.String(), deployment.CCTPTokenPoolProgramName, "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
	...
	if c.AccessController {
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, mcmState.AccessControllerProgram.String(), types.AccessControllerProgram.String(), "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
	...
	if c.Timelock {
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, mcmState.TimelockProgram.String(), types.RBACTimelockProgram.String(), "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
	...
	if c.MCM {
		setAuthorityTx, err := setAuthorityIDLIx(e, timelockSignerPDA.String(), chain.ProgramsPath, mcmState.McmProgram.String(), types.ManyChainMultisigProgram.String(), "")
		if err != nil {
			return cldf.ChangesetOutput{}, err
		}
		if setAuthorityTx != nil {
