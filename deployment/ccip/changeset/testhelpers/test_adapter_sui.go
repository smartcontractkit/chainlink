...
// suiEventTypeMatches compares a Sui event type reported by the RPC (fully-qualified, e.g.
// "0xABC::offramp::CommitReportAccepted") against the expected fully-qualified handle, ignoring
// the 0x prefix. The gRPC Event.event_type is already the full Move type string, so it must be
// compared directly — NOT re-prefixed with package/module, which would double the handle and
// never match (the bug fixed by CCIP #23272).
