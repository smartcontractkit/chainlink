...
func TestAddLanesWithTestRouter(t *testing.T) {
	...
	latesthdr, err := e.Env.Chains[chain2].EVMChain.Client.HeaderByNumber(testcontext.Get(t), nil)
	require.NoError(t, err)
	block := latesthdr.Number.Uint64()
	startBlocks[chain2] = &block
	msgSentEvent := TestSendRequest(t, e.Env, state, chain1, chain2, true, router.ClientEVM2AnyMessage{
		Receiver:     common.LeftPadBytes(state.EVMState.Chains[chain2].Receiver.Address().Bytes(), 32),
		Data:         []byte("hello"),
		TokenAmounts: nil,
...
func TestAddLane(t *testing.T) {
	...
	for _, sel := range []uint64{chain1, chain2} {
		chain := state.EVMState.Chains[sel]
	...
	disableRampTx, err := state.EVMState.Chains[chain2].OffRamp.ApplySourceChainConfigUpdates(e.Env.Chains[chain2].EVMChain.DeployerKey, []offramp.OffRampSourceChainConfigArgs{
		{
			Router:              state.EVMState.Chains[chain2].Router.Address(),
			SourceChainSelector: chain1,
			IsEnabled:           false,
			OnRamp:              common.LeftPadBytes(state.EVMState.Chains[chain1].OnRamp.Address().Bytes(), 32),
		},
	})
	...
	for _, sel := range []uint64{chain1, chain2} {
		...
		if sel == chain2 {
			...
			require.NoError(t, err)
			require.Equal(t, common.LeftPadBytes(state.EVMState.Chains[chain1].OnRamp.Address().Bytes(), 32), srcCfg.OnRamp)
	...
	latesthdr, err = e.Env.Chains[chain1].EVMChain.Client.HeaderByNumber(testcontext.Get(t), nil)
	require.NoError(t, err)
	...
	require.NoError(t,
		commonutils.JustError(
			ConfirmExecWithSeqNrs(
				...
				e.Env.Chains[chain2],
				e.Env.Chains[chain1],
				state.EVMState.Chains[chain1].OffRamp,
				&startBlock2,
	...
	ConfirmNoExecConsistentlyWithSeqNr(t, e.Env.Chains[chain1], e.Env.Chains[chain2], state.EVMState.Chains[chain2].OffRamp, msgSentEvent1.SequenceNumber, 30*time.Second)

	// enable the onRamp on OffRamp
	enableRampTx, err := state.EVMState.Chains[chain2].OffRamp.ApplySourceChainConfigUpdates(e.Env.Chains[chain2].EVMChain.DeployerKey, []offramp.OffRampSourceChainConfigArgs{
		{
			Router:              state.EVMState.Chains[chain2].Router.Address(),
			SourceChainSelector: chain1,
			IsEnabled:           true,
	...
	srcCfg, err := state.EVMState.Chains[chain2].OffRamp.GetSourceChainConfig(nil, chain1)
	require.NoError(t, err)
	require.Equal(t, common.LeftPadBytes(state.EVMState.Chains[chain1].OnRamp.Address().Bytes(), 32), srcCfg.OnRamp)
	...
	require.NoError(t,
		commonutils.JustError(
			ConfirmExecWithSeqNrs(
				...
				e.Env.Chains[chain1],
				e.Env.Chains[chain2],
				state.EVMState.Chains[chain2].OffRamp,
				&startBlock,
