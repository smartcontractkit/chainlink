...
func TestAddLanesWithSolana(t *testing.T) {
	...
	_, _, _, err = testhelpers.DeployTransferableTokenSolana(e.Env.Logger, e.Env, chain1, solChain, e.Env.Chains[chain1].DeployerKey, e.Env.ExistingAddresses, "MY_TOKEN")
	require.NoError(t, err)
}
