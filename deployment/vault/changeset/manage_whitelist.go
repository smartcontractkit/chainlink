...
func setWhitelistLogic(e cldf.Environment, cfg types.SetWhitelistConfig) (cldf.ChangesetOutput, error) {
	...
	for chainSelector, addresses := range cfg.WhitelistByChain {
		...
		if len(addresses) == 0 {
			...
			if err != nil {
				return cldf.ChangesetOutput{}, fmt.Errorf("failed to clear whitelist for chain %d: %w", chainSelector, err)
			}
			continue
		}
		...
		existingMetadata, err := GetChainWhitelistMutable(ds, chainSelector)
		if err != nil {
			return cldf.ChangesetOutput{}, fmt.Errorf("failed to load existing whitelist for chain %d: %w", chainSelector, err)
		}

		merged := mergeWhitelistEntries(existingMetadata.Addresses, addresses)
		...
		if err != nil {
			return cldf.ChangesetOutput{}, fmt.Errorf("failed to upsert whitelist chain metadata for chain %d: %w", chainSelector, err)
		}
	}
...
func mergeWhitelistEntries(existing, incoming []types.WhitelistAddress) []types.WhitelistAddress {
	...
	merged := make([]types.WhitelistAddress, 0, len(existing)+len(incoming))

	for idx, addr := range existing {
		index[addr.Address] = idx
		merged = append(merged, addr)
	}

	for _, addr := range incoming {
		if existingIdx, ok := index[addr.Address]; ok {
			merged[existingIdx].Description = addr.Description
			if len(addr.Labels) > 0 {
				merged[existingIdx].Labels = addr.Labels
			}
			continue
		}

		index[addr.Address] = len(merged)
		merged = append(merged, addr)
	}
