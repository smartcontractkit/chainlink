...
func GetTimelockBalances(e cldf.Environment, chainSelectors []uint64, timelockByChain map[uint64]string) (map[uint64]*types.TimelockNativeBalanceInfo, error) {
	...
	for _, chainSelector := range chainSelectors {
		...
		timelockAddr := ""
		var err error
		if timelockByChain != nil {
			timelockAddr = timelockByChain[chainSelector]
		}

		if timelockAddr != "" {
			_, err = GetContractRecordByAddress(e.DataStore, chainSelector, commontypes.RBACTimelock, timelockAddr)
			if err != nil {
				return nil, fmt.Errorf("timelock %s not found for chain %d: %w", timelockAddr, chainSelector, err)
			}
		} else {
...
func CalculateFundingRequirements(e cldf.Environment, cfg types.BatchNativeTransferConfig) (map[uint64]*FundingRequirement, error) {
	...
	currentBalances, err := GetTimelockBalances(e, chainSelectors, cfg.TimelockByChain)
	if err != nil {
