...
func ValidateBatchNativeTransferConfig(ctx context.Context, e cldf.Environment, cfg types.BatchNativeTransferConfig) error {
	...
	for chainSelector, transfers := range cfg.TransfersByChain {
		...
		timelockAddr, ok := cfg.TimelockByChain[chainSelector]
		if !ok || timelockAddr == "" {
			return fmt.Errorf("timelock address missing for chain %d", chainSelector)
		}
		if common.HexToAddress(timelockAddr) == (common.Address{}) {
			return fmt.Errorf("invalid timelock address %s for chain %d", timelockAddr, chainSelector)
		}
		...
		if err := validateNativeTransfers(ctx, e, chainSelector, transfers, timelockAddr); err != nil {
			return fmt.Errorf("validation failed for chain %d: %w", chainSelector, err)
		}
	}

	if cfg.MCMSConfig != nil {
		if err := validateMCMSConfig(e, cfg.MCMSConfig, cfg.TransfersByChain, cfg.TimelockByChain); err != nil {
...
func validateNativeTransfers(_ context.Context, e cldf.Environment, chainSelector uint64, transfers []types.NativeTransfer, timelockAddr string) error {
	...
	if err := validateTimelockBalance(e, chainSelector, timelockAddr, totalAmount); err != nil {
...
func validateTimelockBalance(e cldf.Environment, chainSelector uint64, timelockAddr string, requiredAmount *big.Int) error {
	balances, err := GetTimelockBalances(e, []uint64{chainSelector}, map[uint64]string{chainSelector: timelockAddr})
	if err != nil {
...
func validateMCMSConfig(e cldf.Environment, mcmsConfig *proposalutils.TimelockConfig, transfersByChain map[uint64][]types.NativeTransfer, timelockByChain map[uint64]string) error {
	if mcmsConfig != nil {
	...
	for chainSelector := range transfersByChain {
		timelockAddr := timelockByChain[chainSelector]
		timelockRecord, err := GetContractRecordByAddress(e.DataStore, chainSelector, commontypes.RBACTimelock, timelockAddr)
		if err != nil {
			return fmt.Errorf("timelock %s not found for chain %d: %w", timelockAddr, chainSelector, err)
		}
		...
		var label string
		if lbls := timelockRecord.Labels.List(); len(lbls) > 0 {
			label = lbls[0]
		}

		if label != "" {
			if _, err := GetContractAddressByLabel(e.DataStore, chainSelector, commontypes.ProposerManyChainMultisig, label); err != nil {
				return fmt.Errorf("proposer not found for chain %d with label %s: %w", chainSelector, label, err)
			}
		} else {
			if _, err := GetContractAddress(e.DataStore, chainSelector, commontypes.ProposerManyChainMultisig); err != nil {
