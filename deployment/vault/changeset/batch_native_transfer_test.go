...
func TestBatchNativeTransferValidation(t *testing.T) {
	...
	configured := commonchangeset.Configure(
	...
	updatedEnv, err := commonchangeset.Apply(t, env, nil, configured)
	require.NoError(t, err)
	env = updatedEnv

	timelockAddr, err := GetContractAddress(env.DataStore, chainSelector, commontypes.RBACTimelock)
	require.NoError(t, err)

	whitelistOutput, err := SetWhitelistChangeset.Apply(env, types.SetWhitelistConfig{
	...
	require.NoError(t, err)
	env.DataStore = whitelistOutput.DataStore.Seal()
	...
	tests := []struct {
	...
	}{
		{
			...
			config: types.BatchNativeTransferConfig{
				...
				TimelockByChain: map[uint64]string{chainSelector: timelockAddr},
		...
		},
		{
			...
			config: types.BatchNativeTransferConfig{
				TransfersByChain: map[uint64][]types.NativeTransfer{
					chainSelector: {
						{
							To:     common.HexToAddress(testAddr1).Hex(),
							Amount: big.NewInt(0),
						},
					},
				},
...
func fundTimelockContracts(t *testing.T, env cldf.Environment, chainSelectors ...uint64) cldf.Environment {
	...
	timelockByChain := make(map[uint64]string, len(chainSelectors))
	for _, chainSel := range chainSelectors {
		addr, err := GetContractAddress(env.DataStore, chainSel, commontypes.RBACTimelock)
		require.NoError(t, err)
		timelockByChain[chainSel] = addr
	}

	timelockBalances, err := GetTimelockBalances(env, chainSelectors, timelockByChain)
...
func executeBatchTransfersWithMCMS(t *testing.T, env cldf.Environment, chainSelectors ...uint64) {
	...
	transferConfig.TimelockByChain = timelockByChain
...
func executeDirectTransfers(t *testing.T, env cldf.Environment, chainSelector uint64) {
	...
	timelockAddr, err := GetContractAddress(env.DataStore, chainSelector, commontypes.RBACTimelock)
	require.NoError(t, err)
	transferConfig.TimelockByChain = map[uint64]string{chainSelector: timelockAddr}
