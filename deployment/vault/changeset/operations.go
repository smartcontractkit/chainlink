...
type BatchNativeTransferSequenceInput struct {
	...
	TimelockByChain  map[uint64]string                 `json:"timelock_by_chain"`
...
func generateMCMSProposals(b operations.Bundle, deps VaultDeps, input BatchNativeTransferSequenceInput, output BatchNativeTransferSequenceOutput) (BatchNativeTransferSequenceOutput, error) {
	...
	for chainSelector, transfers := range input.TransfersByChain {
		...
		timelockAddr := input.TimelockByChain[chainSelector]
		if timelockAddr == "" {
			return BatchNativeTransferSequenceOutput{}, fmt.Errorf("timelock address missing for chain %d", chainSelector)
		}

		timelockRecord, err := GetContractRecordByAddress(deps.DataStore, chainSelector, commontypes.RBACTimelock, timelockAddr)
		if err != nil {
			return BatchNativeTransferSequenceOutput{}, fmt.Errorf("timelock %s not found for chain %d: %w", timelockAddr, chainSelector, err)
		}
		...
		if input.MCMSConfig.MCMSAction == mcmstypes.TimelockActionBypass {
			if label != "" {
				mcmAddr, err = GetContractAddressByLabel(deps.DataStore, chainSelector, commontypes.BypasserManyChainMultisig, label)
			} else {
			...
			if label != "" {
				mcmAddr, err = GetContractAddressByLabel(deps.DataStore, chainSelector, commontypes.ProposerManyChainMultisig, label)
			} else {
		...
		timelockAddressByChain[chainSelector] = timelockRecord.Address
