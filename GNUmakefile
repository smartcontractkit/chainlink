...
# Use rg if present, fallback to pruned find + grep
RM_MOCKED_CMD ?= $(shell if command -v rg >/dev/null 2>&1; then \
	echo 'rg --files-with-matches --glob "*.go" "^// Code generated by mockery" .'; \
else \
	echo 'find . \( -name .git -o -name node_modules -o -name .yarn -o -name vendor \) -prune -o -name "*.go" -exec grep -l "^// Code generated by mockery" {} +'; \
fi)

GENERATE_PKGS_CMD ?= $(shell if command -v rg >/dev/null 2>&1; then \
	echo 'rg --files-with-matches --glob "*.go" "^//go:generate" .'; \
else \
	echo 'find . \( -name .git -o -name node_modules -o -name .yarn -o -name vendor \) -prune -o -name "*.go" -exec grep -l "^//go:generate" {} +'; \
fi)
...
generate: codecgen mockery protoc gomods ## Execute all go:generate commands.
	...
	export PATH="$(HOME)/.local/bin:$(PATH)"; $(GENERATE_PKGS_CMD) | xargs -n1 dirname | sort -u | xargs go generate
	find . \( -name .git -o -name node_modules -o -name .yarn -o -name vendor \) -prune -o -name .mockery.yaml -print | xargs -n1 -P0 -I{} sh -c 'cd "$$(dirname "{}")" && $(MOCKERY_BIN)'
...
gomods: ## Install gomods
	@if ! command -v gomods >/dev/null 2>&1; then \
...
mockery: $(mockery) ## Install mockery.
	@if ! command -v $(MOCKERY_BIN) >/dev/null 2>&1 || ! $(MOCKERY_BIN) --version 2>&1 | grep -q "$(MOCKERY_VERSION)"; then \
		go install github.com/vektra/mockery/v2@v$(MOCKERY_VERSION); \
	fi

.PHONY: codecgen
codecgen: $(codecgen) ## Install codecgen
	@if ! command -v codecgen >/dev/null 2>&1; then \
