...
gomods: ## Install gomods
	@if ! command -v gomods >/dev/null 2>&1 || ! go version -m "$$(command -v gomods)" 2>/dev/null | grep -q "v0.1.7"; then \
...
codecgen: $(codecgen) ## Install codecgen
	@if ! command -v codecgen >/dev/null 2>&1 || ! go version -m "$$(command -v codecgen)" 2>/dev/null | grep -q "v1.2.10"; then \
...
.PHONY: modgraph
modgraph:
	@if ! command -v modgraph >/dev/null 2>&1 || ! go version -m "$$(command -v modgraph)" 2>/dev/null | grep -q "v0.1.4"; then \
