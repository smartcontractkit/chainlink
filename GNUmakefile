...
golangci-lint: ## Run golangci-lint for all issues.
	...
	docker run --rm -v $(shell pwd):/app -w /app golangci/golangci-lint:v1.53.2 golangci-lint run --max-issues-per-linter 0 --max-same-issues 0 > ./golangci-lint/$(shell date +%Y-%mm-%dd_%s).txt
