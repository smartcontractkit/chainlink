...
func TestRegistrySyncerORM_InsertAndRetrieval(t *testing.T) {
	...
	lggr := logger.Test(t)
	orm := registrysyncer_v2.NewORM(db, lggr)

	var states []registrysyncer_v2.LocalRegistry
	for i := 0; i < 11; i++ {
...
func generateState(t *testing.T) registrysyncer_v2.LocalRegistry {
	dID := uint32(1)
	...
	return registrysyncer_v2.LocalRegistry{
		IDsToDONs: map[registrysyncer_v2.DonID]registrysyncer_v2.DON{
			registrysyncer_v2.DonID(dID): {
				...
				CapabilityConfigurations: map[string]registrysyncer_v2.CapabilityConfiguration{
					capabilityIDStr: {
						Config: configb,
					},
					capabilityID2Str: {
		...
		IDsToCapabilities: map[string]registrysyncer_v2.Capability{
			capabilityIDStr: {
				ID:             capabilityIDStr,
