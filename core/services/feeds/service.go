...
// ProposeJob creates a job proposal if it does not exist. If a pending proposal
// already exists, update the existing pending job proposal.
//
// We check that the job proposal belongs to the feeds manager who is making
// the proposal because in anticipation for multiple feeds manager support. On
// the highly unlikely event of a UUID collision, or a potential malicious
// attack by another feeds manager, we return an error.
func (s *service) ProposeJob(jp *JobProposal) (int64, error) {
	...
	// Find an existing pending job proposal
	existing, err := s.orm.GetJobProposalByRemoteUUIDAndStatus(jp.RemoteUUID, JobProposalStatusPending)
	if err != nil {
