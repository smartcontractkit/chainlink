	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedToPoll {
				...
				tm.pipelineRunner.
					On("ExecuteAndInsertFinishedRun", context.Background(), pipelineSpec, nil, pipeline.JSONSerializable{
...
func TestPollingDeviationChecker_BuffersLogs(t *testing.T) {
	...
	tm.pipelineRunner.
		On("ExecuteAndInsertFinishedRun", context.Background(), pipelineSpec, nil, pipeline.JSONSerializable{Val: map[string]interface{}(nil), Null: false}, defaultLogger, false).
		Return(int64(1), pipeline.FinalResult{
...
func TestFluxMonitor_DoesNotDoubleSubmit(t *testing.T) {
	t.Run("when NewRound log arrives, then poll ticker fires", func(t *testing.T) {
		...
		tm.pipelineRunner.
			On("ExecuteAndInsertFinishedRun", context.Background(), pipelineSpec, nil, mock.Anything, defaultLogger, false).
			Return(int64(1), pipeline.FinalResult{
