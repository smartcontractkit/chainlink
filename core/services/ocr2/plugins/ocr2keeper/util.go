...
func EVMDependencies(
...
) (evmrelay.OCR2KeeperProvider, *kevm.EvmRegistry, ktypes.ReportEncoder, *kevm.LogProvider, *kevm.RegistryPoller, error) {
	var (
		lookbackBlocks int64 = 250
		keeperProvider evmrelay.OCR2KeeperProvider
		encoder        ktypes.ReportEncoder
		registry       *kevm.EvmRegistry
		chain          evm.Chain
		err            error
	)
	...
	rp, err := kevm.NewRegistryPoller(chain.LogPoller(), lggr, chain.Client(), rAddr)
	if err != nil {
		return nil, nil, nil, nil, nil, err
	}

	if registry, err = kevm.NewEVMRegistryServiceV2_0(rAddr, chain, mc, lggr, rp); err != nil {
		return nil, nil, nil, nil, nil, err
	}
	...
	logProvider, err := kevm.NewLogProvider(lggr, rp, chain.Client(), lookbackBlocks)

	return keeperProvider, registry, encoder, logProvider, rp, err
}
