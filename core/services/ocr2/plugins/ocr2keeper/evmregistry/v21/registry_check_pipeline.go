...
import (
	...
	core2 "github.com/smartcontractkit/chainlink/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/core"
	encoding2 "github.com/smartcontractkit/chainlink/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/encoding"
)
...
func (r *EvmRegistry) verifyCheckBlock(_ context.Context, checkBlock, upkeepId *big.Int, checkHash common.Hash) (state encoding2.PipelineExecutionState, retryable bool) {
	...
	if ok && h == checkHash.Hex() {
		...
		return encoding2.NoPipelineError, false
	}
	...
	if err != nil {
		...
		return encoding2.RpcFlakyFailure, true
	}
	if checkHash.Hex() != b.Hex() {
		...
		return encoding2.CheckBlockInvalid, false
	}
	return encoding2.NoPipelineError, false
}

// verifyLogExists checks that the log still exists on chain, returns failure reason, pipeline error, and retryable
func (r *EvmRegistry) verifyLogExists(upkeepId *big.Int, p ocr2keepers.UpkeepPayload) (encoding2.UpkeepFailureReason, encoding2.PipelineExecutionState, bool) {
	...
	if checkBlockHash.String() == logBlockHash.String() {
		...
		// for a very new log
		return encoding2.UpkeepFailureReasonNone, encoding2.NoPipelineError, false
	}
	...
	bn, bh, err := core2.GetTxBlock(r.ctx, r.client, p.Trigger.LogTriggerExtension.TxHash)
	if err != nil {
		...
		if strings.Contains(err.Error(), "missing required field") || strings.Contains(err.Error(), "not found") {
			return encoding2.UpkeepFailureReasonTxHashNoLongerExists, encoding2.NoPipelineError, false
		}
		...
		return encoding2.UpkeepFailureReasonNone, encoding2.RpcFlakyFailure, true
	}
	if bn == nil {
	...
	if bh.Hex() != logBlockHash.Hex() {
		...
		return encoding2.UpkeepFailureReasonTxHashReorged, encoding2.NoPipelineError, false
	}
...
func (r *EvmRegistry) checkUpkeeps(ctx context.Context, payloads []ocr2keepers.UpkeepPayload) ([]ocr2keepers.CheckResult, error) {
	...
	for i, p := range payloads {
		...
		if state != encoding2.NoPipelineError {
			results[i] = encoding2.GetIneligibleCheckResultWithoutPerformData(p, encoding2.UpkeepFailureReasonNone, state, retryable)
			continue
		}
		...
		switch core2.GetUpkeepType(*uid) {
		case ocr2keepers.LogTrigger:
			reason, state, retryable := r.verifyLogExists(upkeepId, p)
			if reason != encoding2.UpkeepFailureReasonNone || state != encoding2.NoPipelineError {
				results[i] = encoding2.GetIneligibleCheckResultWithoutPerformData(p, reason, state, retryable)
				continue
			}
			...
			if err != nil {
				...
				results[i] = encoding2.GetIneligibleCheckResultWithoutPerformData(p, encoding2.UpkeepFailureReasonNone, encoding2.PackUnpackDecodeFailed, false)
				continue
			}
		default:
		...
		results[i] = encoding2.GetIneligibleCheckResultWithoutPerformData(p, encoding2.UpkeepFailureReasonNone, encoding2.NoPipelineError, false)
	...
	for i, req := range checkReqs {
		...
		if req.Error != nil {
			...
			if blockNotFound && latestBlockNumber-checkBlock.Int64() > checkBlockTooOldRange {
				...
				results[index].PipelineExecutionState = uint8(encoding2.CheckBlockTooOld)
			} else {
				...
				results[index].PipelineExecutionState = uint8(encoding2.RpcFlakyFailure)
			}
		} else {
			var err error
...
func (r *EvmRegistry) simulatePerformUpkeeps(ctx context.Context, checkResults []ocr2keepers.CheckResult) ([]ocr2keepers.CheckResult, error) {
	...
	for i, cr := range checkResults {
		...
		if err != nil {
			...
			checkResults[i].PipelineExecutionState = uint8(encoding2.PackUnpackDecodeFailed)
			continue
		}
	...
	for i, req := range performReqs {
		...
		if req.Error != nil {
			...
			checkResults[idx].PipelineExecutionState = uint8(encoding2.RpcFlakyFailure)
			continue
		}
		...
		if !simulatePerformSuccess {
			...
			checkResults[performToKeyIdx[i]].IneligibilityReason = uint8(encoding2.UpkeepFailureReasonSimulationFailed)
		}
	}
