...
func (c *EventProvider) processLogs(latestBlock int64, logs ...logpoller.Log) ([]ocr2keepers.TransmitEvent, error) {
	...
	for _, log := range logs {
		...
		if !ok {
			...
			if !ok {
				return nil, core2.ErrInvalidUpkeepID
			}
			triggerW, err := core2.UnpackTrigger(id, l.Trigger())
			if err != nil {
			...
			switch core2.GetUpkeepType(*upkeepId) {
			...
			workID := core2.UpkeepWorkID(*upkeepId, trigger)
