...
type logRecoverer struct {
	...
	states            core2.UpkeepStateReader
...
func NewLogRecoverer(lggr logger.Logger, poller logpoller.LogPoller, client client.Client, stateStore core2.UpkeepStateReader, packer LogDataPacker, filterStore UpkeepFilterStore, opts LogTriggersOptions) *logRecoverer {
	rec := &logRecoverer{
...
func (r *logRecoverer) GetProposalData(ctx context.Context, proposal ocr2keepers.CoordinatedBlockProposal) ([]byte, error) {
	switch core2.GetUpkeepType(proposal.UpkeepID) {
...
func (r *logRecoverer) getLogTriggerCheckData(ctx context.Context, proposal ocr2keepers.CoordinatedBlockProposal) ([]byte, error) {
	...
	bn, bh, err := core2.GetTxBlock(ctx, r.client, proposal.Trigger.LogTriggerExtension.TxHash)
	if err != nil {
		return nil, err
	}
	if bn == nil {
	...
	for _, log := range logs {
		...
		wid := core2.UpkeepWorkID(proposal.UpkeepID, trigger)
...
func (r *logRecoverer) recoverFilter(ctx context.Context, f upkeepFilter, startBlock, offsetBlock int64) error {
	...
	for _, log := range logs {
		...
		workIDs = append(workIDs, core2.UpkeepWorkID(*upkeepId, trigger))
	}
...
func (r *logRecoverer) populatePending(f upkeepFilter, filteredLogs []logpoller.Log) (int, int, bool) {
	...
	for _, log := range filteredLogs {
		...
		wid := core2.UpkeepWorkID(*upkeepId, trigger)
		...
		payload, err := core2.NewUpkeepPayload(f.upkeepID, trigger, checkData)
		if err != nil {
