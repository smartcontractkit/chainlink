...
import (
	...
	logprovider2 "github.com/smartcontractkit/chainlink/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/logprovider"
...
func TestIntegration_LogEventProvider(t *testing.T) {
	...
	t.Run("Restart", func(t *testing.T) {
		...
		filterStore := logprovider2.NewUpkeepFilterStore()
		logProvider2 := logprovider2.NewLogProvider(logger.TestLogger(t), lp, logprovider2.NewLogEventsPacker(), filterStore, opts)
		...
		// re-register filters
		for i, id := range ids {
			err := logProvider2.RegisterFilter(ctx, logprovider2.FilterOptions{
				UpkeepID:      id,
...
func TestIntegration_LogEventProvider_UpdateConfig(t *testing.T) {
	...
	opts := &logprovider2.LogTriggersOptions{
	...
	t.Run("update filter config", func(t *testing.T) {
		upkeepID := core2.GenUpkeepID(ocr2keepers.LogTrigger, "111")
		id := upkeepID.BigInt()
		cfg := newPlainLogTriggerConfig(addrs[0])
		b, err := ethClient.BlockByHash(ctx, backend.Commit())
		require.NoError(t, err)
		bn := b.Number()
		err = logProvider.RegisterFilter(ctx, logprovider2.FilterOptions{
			UpkeepID:      id,
			TriggerConfig: cfg,
	...
	t.Run("register same log filter", func(t *testing.T) {
		upkeepID := core2.GenUpkeepID(ocr2keepers.LogTrigger, "222")
		id := upkeepID.BigInt()
		cfg := newPlainLogTriggerConfig(addrs[0])
		b, err := ethClient.BlockByHash(ctx, backend.Commit())
		require.NoError(t, err)
...
func TestIntegration_LogEventProvider_RateLimit(t *testing.T) {
	setupTest := func(
		...
		*backends.SimulatedBackend,
		...
		logprovider2.LogEventProviderTest,
		[]*big.Int,
		func(),
	) {
		ctx, cancel := context.WithCancel(testutils.Context(t))
	...
	t.Run("should allow constant polls within the rate and burst limit", func(t *testing.T) {
		ctx, backend, poll, logProvider, ids, deferFunc := setupTest(t, &logprovider2.LogTriggersOptions{
			LookbackBlocks: 200,
		...
		for i := 0; i < 4; i++ {
			...
			if err != nil {
				assert.False(t, errors.Is(err, logprovider2.ErrBlockLimitExceeded), "error should not contain block limit exceeded")
			}
	...
	t.Run("should produce a rate limit error for over burst limit", func(t *testing.T) {
		...
		for i := 0; i < 4; i++ {
			...
			err := logProvider.ReadLogs(ctx, ids...)
			if err != nil {
				assert.True(t, errors.Is(err, logprovider2.ErrBlockLimitExceeded), "error should not contain block limit exceeded")
			}
...
func TestIntegration_LogRecoverer_Backfill(t *testing.T) {
	...
	lp, ethClient := setupDependencies(t, db, backend)
	filterStore := logprovider2.NewUpkeepFilterStore()
	origDefaultRecoveryInterval := logprovider2.RecoveryInterval
	logprovider2.RecoveryInterval = time.Millisecond * 200
	defer func() {
		logprovider2.RecoveryInterval = origDefaultRecoveryInterval
	}()
...
func collectPayloads(ctx context.Context, t *testing.T, logProvider logprovider2.LogEventProvider, n, rounds int) []ocr2keepers.UpkeepPayload {
	...
	for ctx.Err() == nil && len(allPayloads) < n && rounds > 0 {
		...
		require.NoError(t, err)
		require.LessOrEqual(t, len(logs), logprovider2.AllowedLogsPerUpkeep, "failed to get all logs")
...
func waitLogProvider(ctx context.Context, t *testing.T, logProvider logprovider2.LogEventProviderTest, partition int) {
...
func deployUpkeepCounter(
	...
	backend *backends.SimulatedBackend,
	account *bind.TransactOpts,
	logProvider logprovider2.LogEventProvider,
...
func newPlainLogTriggerConfig(upkeepAddr common.Address) logprovider2.LogTriggerConfig {
	return logprovider2.LogTriggerConfig{
...
func setup(lggr logger.Logger, poller logpoller.LogPoller, c client.Client, stateStore core2.UpkeepStateReader, filterStore logprovider2.UpkeepFilterStore, opts *logprovider2.LogTriggersOptions) (logprovider2.LogEventProvider, logprovider2.LogRecoverer) {
	packer := logprovider2.NewLogEventsPacker()
	if opts == nil {
		o := logprovider2.NewOptions(200)
		opts = &o
	}
	provider := logprovider2.NewLogProvider(lggr, poller, packer, filterStore, *opts)
	recoverer := logprovider2.NewLogRecoverer(lggr, poller, c, stateStore, packer, filterStore, *opts)
