...
import (
	...
	upkeepstate2 "github.com/smartcontractkit/chainlink/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/upkeepstate"
...
type AutomationServices interface {
	...
	UpkeepStateStore() upkeepstate2.UpkeepStateStore
	LogEventProvider() logprovider2.LogEventProvider
	LogRecoverer() logprovider2.LogRecoverer
...
func New(addr common.Address, client evm.Chain, mc *models.MercuryCredentials, keyring ocrtypes.OnchainKeyring, lggr logger.Logger, db *sqlx.DB, dbCfg pg.QConfig) (AutomationServices, error) {
	...
	packer := encoding2.NewAbiPacker()
	services.encoder = encoding2.NewReportEncoder(packer)

	finalityDepth := client.Config().EVM().FinalityDepth()

	orm := upkeepstate2.NewORM(client.ID(), db, lggr, dbCfg)
	scanner := upkeepstate2.NewPerformedEventsScanner(lggr, client.LogPoller(), addr, finalityDepth)
	services.upkeepState = upkeepstate2.NewUpkeepStateStore(orm, lggr, scanner)

	logProvider, logRecoverer := logprovider2.New(lggr, client.LogPoller(), client.Client(), services.upkeepState, finalityDepth)
...
type automationServices struct {
	...
	payloadBuilder        ocr2keepers.PayloadBuilder
	upkeepState           upkeepstate2.UpkeepStateStore
...
func (f *automationServices) UpkeepStateStore() upkeepstate2.UpkeepStateStore {
	return f.upkeepState
}

func (f *automationServices) LogEventProvider() logprovider2.LogEventProvider {
	return f.logProvider
}

func (f *automationServices) LogRecoverer() logprovider2.LogRecoverer {
