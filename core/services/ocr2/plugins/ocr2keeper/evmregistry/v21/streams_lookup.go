...
type MercuryData struct {
	...
	Retryable bool
	Bytes     [][]byte
	State     encoding2.PipelineExecutionState
}
...
func (r *EvmRegistry) streamsLookup(ctx context.Context, checkResults []ocr2keepers.CheckResult) []ocr2keepers.CheckResult {
	...
	for i, res := range checkResults {
		if res.IneligibilityReason != uint8(encoding2.UpkeepFailureReasonTargetCheckReverted) {
		...
		if len(l.Feeds) == 0 {
			checkResults[i].IneligibilityReason = uint8(encoding2.UpkeepFailureReasonInvalidRevertDataInput)
		...
		if l.FeedParamKey == feedIdHex && l.TimeParamKey == blockNumber {
			...
			if !allowed {
				...
				checkResults[i].IneligibilityReason = uint8(encoding2.UpkeepFailureReasonMercuryAccessNotAllowed)
				continue
			}
...
func (r *EvmRegistry) doLookup(ctx context.Context, wg *sync.WaitGroup, lookup *StreamsLookup, i int, checkResults []ocr2keepers.CheckResult, lggr logger.Logger) {
	...
	if failureReason == uint8(encoding2.UpkeepFailureReasonMercuryCallbackReverted) {
		checkResults[i].IneligibilityReason = uint8(encoding2.UpkeepFailureReasonMercuryCallbackReverted)
		lggr.Debugf("at block %d upkeep %s mercury callback reverts", lookup.block, lookup.upkeepId)
		return
	}

	if !needed {
		checkResults[i].IneligibilityReason = uint8(encoding2.UpkeepFailureReasonUpkeepNotNeeded)
		lggr.Debugf("at block %d upkeep %s callback reports upkeep not needed", lookup.block, lookup.upkeepId)
		return
	}

	checkResults[i].IneligibilityReason = uint8(encoding2.UpkeepFailureReasonNone)
...
func (r *EvmRegistry) allowedToUseMercury(opts *bind.CallOpts, upkeepId *big.Int) (state encoding2.PipelineExecutionState, reason encoding2.UpkeepFailureReason, retryable bool, allow bool, err error) {
	allowed, ok := r.mercury.allowListCache.Get(upkeepId.String())
	if ok {
		return encoding2.NoPipelineError, encoding2.UpkeepFailureReasonNone, false, allowed.(bool), nil
	}
	...
	if err != nil {
		...
		return encoding2.PackUnpackDecodeFailed, encoding2.UpkeepFailureReasonNone, false, false, fmt.Errorf("failed to pack upkeepId: %w", err)
	}
	...
	if err != nil {
		return encoding2.RpcFlakyFailure, encoding2.UpkeepFailureReasonNone, true, false, fmt.Errorf("failed to get upkeep privilege config: %v", err)
	}

	cfg, err := r.packer.UnpackGetUpkeepPrivilegeConfig(resultBytes)
	if err != nil {
		return encoding2.PackUnpackDecodeFailed, encoding2.UpkeepFailureReasonNone, false, false, fmt.Errorf("failed to get upkeep privilege config: %v", err)
	}

	if len(cfg) == 0 {
		r.mercury.allowListCache.Set(upkeepId.String(), false, cache.DefaultExpiration)
		return encoding2.NoPipelineError, encoding2.UpkeepFailureReasonMercuryAccessNotAllowed, false, false, fmt.Errorf("upkeep privilege config is empty")
	}

	var privilegeConfig UpkeepPrivilegeConfig
	if err := json.Unmarshal(cfg, &privilegeConfig); err != nil {
		return encoding2.MercuryUnmarshalError, encoding2.UpkeepFailureReasonNone, false, false, fmt.Errorf("failed to unmarshal privilege config: %v", err)
	}

	r.mercury.allowListCache.Set(upkeepId.String(), privilegeConfig.MercuryEnabled, cache.DefaultExpiration)

	return encoding2.NoPipelineError, encoding2.UpkeepFailureReasonNone, false, privilegeConfig.MercuryEnabled, nil
}

func (r *EvmRegistry) checkCallback(ctx context.Context, values [][]byte, lookup *StreamsLookup) (encoding2.PipelineExecutionState, bool, hexutil.Bytes, error) {
	payload, err := r.abi.Pack("checkCallback", lookup.upkeepId, values, lookup.ExtraData)
	if err != nil {
		return encoding2.PackUnpackDecodeFailed, false, nil, err
	}
	...
	if err != nil {
		return encoding2.RpcFlakyFailure, true, nil, err
	}
	return encoding2.NoPipelineError, false, b, nil
}

// doMercuryRequest sends requests to Mercury API to retrieve mercury data.
func (r *EvmRegistry) doMercuryRequest(ctx context.Context, sl *StreamsLookup, prk string, lggr logger.Logger) (encoding2.PipelineExecutionState, encoding2.UpkeepFailureReason, [][]byte, bool, time.Duration, error) {
	var isMercuryV03 bool
	resultLen := len(sl.Feeds)
	ch := make(chan MercuryData, resultLen)
	if len(sl.Feeds) == 0 {
		return encoding2.NoPipelineError, encoding2.UpkeepFailureReasonInvalidRevertDataInput, [][]byte{}, false, 0 * time.Second, fmt.Errorf("invalid revert data input: feed param key %s, time param key %s, feeds %s", sl.FeedParamKey, sl.TimeParamKey, sl.Feeds)
	}
	...
	state := encoding2.NoPipelineError
	for i := 0; i < resultLen; i++ {
		...
		if m.Error != nil {
			...
			allSuccess = false
			if m.State != encoding2.NoPipelineError {
				state = m.State
			}
			continue
		}
	...
	return state, encoding2.UpkeepFailureReasonNone, results, retryable && !allSuccess, ri, reqErr
}
...
func (r *EvmRegistry) singleFeedRequest(ctx context.Context, ch chan<- MercuryData, index int, sl *StreamsLookup, lggr logger.Logger) {
	...
	if err != nil {
		ch <- MercuryData{Index: index, Error: err, Retryable: false, State: encoding2.InvalidMercuryRequest}
		return
	}
	...
	retryErr := retry.Do(
		func() error {
			...
			if err1 != nil {
				...
				retryable = true
				state = encoding2.MercuryFlakyFailure
				return err1
			}
			...
			if err1 != nil {
				retryable = false
				state = encoding2.InvalidMercuryResponse
				return err1
			}
			...
			if resp.StatusCode == http.StatusNotFound || resp.StatusCode == http.StatusInternalServerError || resp.StatusCode == http.StatusBadGateway || resp.StatusCode == http.StatusServiceUnavailable || resp.StatusCode == http.StatusGatewayTimeout {
				...
				retryable = false
				state = encoding2.InvalidMercuryRequest
			...
			if err1 != nil {
				...
				retryable = false
				state = encoding2.MercuryUnmarshalError
				return err1
			}
...
func (r *EvmRegistry) multiFeedsRequest(ctx context.Context, ch chan<- MercuryData, sl *StreamsLookup, lggr logger.Logger) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, reqUrl, nil)
	if err != nil {
		ch <- MercuryData{Index: 0, Error: err, Retryable: false, State: encoding2.InvalidMercuryRequest}
		return
	}
	...
	retryErr := retry.Do(
		func() error {
			...
			if resp.StatusCode == http.StatusUnauthorized {
				retryable = false
				state = encoding2.UpkeepNotAuthorized
