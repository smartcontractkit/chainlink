...
func (e reportEncoder) Encode(results ...ocr2keepers.CheckResult) ([]byte, error) {
	...
	for i, result := range results {
		...
		switch core2.GetUpkeepType(result.UpkeepID) {
		...
		trigger, err := core2.PackTrigger(id, triggerW)
		if err != nil {
...
func (e reportEncoder) Extract(raw []byte) ([]ocr2keepers.ReportedUpkeep, error) {
	...
	for i, upkeepId := range report.UpkeepIds {
		triggerW, err := core2.UnpackTrigger(upkeepId, report.Triggers[i])
		if err != nil {
		...
		workID := core2.UpkeepWorkID(*id, trigger)
