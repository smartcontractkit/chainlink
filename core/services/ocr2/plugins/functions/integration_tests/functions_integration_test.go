...
func TestIntegration_Functions_MultipleRequests_Success(t *testing.T) {
	...
	wg.Wait()
	fmt.Println("Done with basic tests")
}

func TestIntegration_Functions_MultipleRequests_SuccessWithThresholdDecryption(t *testing.T) {
	// a batch of 8 max-length results uses around 1M gas (assuming 70k gas per client callback - see FunctionsClientExample.sol)
	nOracleNodes := 4
	nClients := 50
	requestLenBytes := 1000
	maxGas := 1_300_000
	batchSize := 8
	// Below are the corresponding values for the threshold keys used in the test
	...
	var apps []*cltest.TestApplication
	for i := 0; i < nOracleNodes; i++ {
		oracleNode := utils.StartNewNode(t, owner, bootstrapNodePort+1+uint16(i), fmt.Sprintf("oracle%d", i), b, uint32(maxGas),
			[]commontypes.BootstrapperLocator{
				{PeerID: bootstrapNode.PeerID, Addrs: []string{fmt.Sprintf("127.0.0.1:%d", bootstrapNodePort)}},
			}, exportedOcr2Keystores[i], mockThresholdKeyShares[i],
		)
		apps = append(apps, oracleNode.App)
		oracles = append(oracles, oracleNode.OracleIdentity)
	...
	wg.Wait()
	fmt.Println("Done with threshold tests")
}
