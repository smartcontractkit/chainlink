...
func StartNewMockEA(t *testing.T) *httptest.Server {
	return httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
		...
		assert.NoError(t, json.Unmarshal(b, &jsonMap))
		...
		default:
			assert.Failf(t, "unknown external adapter endpoint", "endpoint: %s", jsonMap["endpoint"].(string))
		}
...
func CreateFunctionsNodes(
	...
	bootstrapNode = StartNewNode(t, owner, bootstrapPort, b, uint32(maxGas), nil, nil, "") //nolint:gosec // G115 maxGas is a test-controlled constant, no overflow
	...
	for i := range nOracleNodes {
		...
		oracleNode := StartNewNode(t, owner, ports[i], b, uint32(maxGas), []commontypes.BootstrapperLocator{ //nolint:gosec // G115 maxGas is a test-controlled constant, no overflow
