...
func TestNewConnector_Success(t *testing.T) {
	...
	_, err = functions.NewConnector(gwcCfg, ethKeystore, chainID, s4Storage, allowlist, rateLimiter, subscriptions, *assets.NewLinkFromJuels(0), nil, nil, logger.TestLogger(t))
	require.NoError(t, err)
}
