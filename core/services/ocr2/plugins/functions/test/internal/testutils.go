...
import (
	...
	"github.com/smartcontractkit/chainlink/core/gethwrappers/generated/functions_client_example"
	"github.com/smartcontractkit/chainlink/core/gethwrappers/generated/functions_don"
	"github.com/smartcontractkit/chainlink/core/gethwrappers/generated/functions_registry"
...
func SetOracleConfig(t *testing.T, owner *bind.TransactOpts, donContract *functions_don.FunctionsDON, oracles []confighelper2.OracleIdentityExtra) {
	...
	require.NoError(t, err)
	_, err = donContract.DeactivateAuthorizedReceiver(owner)
	require.NoError(t, err)
}

func SetRegistryConfig(t *testing.T, owner *bind.TransactOpts, registryContract *functions_registry.FunctionsBillingRegistry, donContractAddress common.Address) {
	...
	var senders = []common.Address{donContractAddress}
...
func CreateAndFundSubscriptions(t *testing.T, owner *bind.TransactOpts, linkToken *link_token_interface.LinkToken, registryContractAddress common.Address, registryContract *functions_registry.FunctionsBillingRegistry, clientContracts []deployedClientContract) (subscriptionId uint64) {
...
func StartNewChainWithContracts(t *testing.T, nClients int) (*bind.TransactOpts, *backends.SimulatedBackend, *time.Ticker, common.Address, *functions_don.FunctionsDON, []deployedClientContract, common.Address, *functions_registry.FunctionsBillingRegistry, *link_token_interface.LinkToken) {
	...
	registryAddress, _, registryContract, err := functions_registry.DeployFunctionsBillingRegistry(owner, b, linkAddr, linkEthFeedAddr)
	require.NoError(t, err)

	ocrContractAddress, _, ocrContract, err := functions_don.DeployFunctionsDON(owner, b)
	require.NoError(t, err)
	...
	for i := 0; i < nClients; i++ {
		clientContractAddress, _, clientContract, err := functions_client_example.DeployFunctionsClientExample(owner, b, ocrContractAddress)
		require.NoError(t, err)
