...
import (
	...
	functions_serv "github.com/smartcontractkit/chainlink/core/services/functions"
...
func newRequestID() functions_serv.RequestID {
	return testutils.Random32Byte()
}

func newRequest() functions_serv.Request {
	return functions_serv.Request{RequestID: newRequestID(), State: functions_serv.IN_PROGRESS}
}

func newRequestWithResult(result []byte) functions_serv.Request {
	return functions_serv.Request{RequestID: newRequestID(), State: functions_serv.RESULT_READY, Result: result}
}

func newRequestFinalized() functions_serv.Request {
	return functions_serv.Request{RequestID: newRequestID(), State: functions_serv.FINALIZED}
}

func newRequestTimedOut() functions_serv.Request {
	return functions_serv.Request{RequestID: newRequestID(), State: functions_serv.TIMED_OUT}
}

func newRequestConfirmed() functions_serv.Request {
	return functions_serv.Request{RequestID: newRequestID(), State: functions_serv.CONFIRMED}
}

func newMarshalledQuery(t *testing.T, reqIDs ...functions_serv.RequestID) []byte {
...
func newProcessedRequest(requestId functions_serv.RequestID, compResult []byte, compError []byte) *functions.ProcessedRequest {
...
func TestDRReporting_Query(t *testing.T) {
	...
	reqs := []functions_serv.Request{newRequest(), newRequest()}
	orm.On("FindOldestEntriesByState", functions_serv.RESULT_READY, uint32(batchSize)).Return(reqs, nil)
...
func getReportBytes(t *testing.T, codec *functions.ReportCodec, reqs ...functions_serv.Request) []byte {
