...
// donSettingsBoolFields is deliberately empty: vault_force_empty_ocr_rounds is
// an emergency per-node lever for stalled rounds and must NOT be
// consensus-governed.
...
var donFieldMaxIdentifierKeyLengthBytes = donSettingsField[uint64]{
	...
	local: func(ctx context.Context, r *ReportingPlugin) uint64 {
		return localSizeLimit(ctx, r.validator.MaxIdentifierKeyLengthLimiter)
	},
...
var donFieldMaxIdentifierOwnerLengthBytes = donSettingsField[uint64]{
	...
	local: func(ctx context.Context, r *ReportingPlugin) uint64 {
		return localSizeLimit(ctx, r.validator.MaxIdentifierOwnerLengthLimiter)
	},
...
var donFieldMaxIdentifierNamespaceLengthBytes = donSettingsField[uint64]{
	...
	local: func(ctx context.Context, r *ReportingPlugin) uint64 {
		return localSizeLimit(ctx, r.validator.MaxIdentifierNamespaceLengthLimiter)
	},
...
var donFieldMaxRequestBatchSize = donSettingsField[uint64]{
	...
	local: func(ctx context.Context, r *ReportingPlugin) uint64 {
		return localIntLimit(ctx, r.validator.MaxRequestBatchSizeLimiter)
	},
