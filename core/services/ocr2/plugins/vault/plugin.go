...
func (r *ReportingPlugin) validatePendingQueueObservations(
	...
	nextObs, err := r.observePendingQueueItem(ctx, 0, readKV, excluded)
	if err != nil {
...
func shaForObservation(o *vaultcommon.Observation, omitCiphertext bool) (string, error) {
...
func (r *ReportingPlugin) checkRequestBatchLimit(ctx context.Context, batchSize int) error {
	if err := r.validator.MaxRequestBatchSizeLimiter.Check(ctx, batchSize); err != nil {
...
func (r *ReportingPlugin) StateTransition(ctx context.Context, seqNr uint64, aq types.AttributedQuery, aos []types.AttributedObservation, keyValueReadWriter ocr3_1types.KeyValueStateReadWriter, blobFetcher ocr3_1types.BlobFetcher) (ocr3_1types.ReportsPlusPrecursor, error) {
	...
	for _, id := range idsToProcess {
		...
		for _, ob := range obs {
			sha, err := shaForObservation(ob, omitCiphertext)
			if err != nil {
...
func (r *ReportingPlugin) stateTransitionGetSecrets(ctx context.Context, reader ReadKVStore, preRoundCiphertexts map[string][]byte, chosen []*vaultcommon.Observation, pendingItem *vaultcommon.StoredPendingQueueItem, o *vaultcommon.Outcome) {
	...
	if r.ciphertextlessObservationsEnabled(ctx) {
		for _, resp := range sortedResponses {
			...
			if stored == nil {
				resp.Result = &vaultcommon.SecretResponse_Error{
					Error: userFacingError(vaulttypes.NewUserError("key does not exist"), "failed to read secret"),
				}
				continue
			}
