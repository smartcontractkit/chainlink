...
func TestIntegration_Mercury(t *testing.T) {
	...
	for req := range reqs {
		...
		t.Logf("oracle %s reported for feed %s (0x%x)", req.pk, feed.name, feed.id)
...
func (s *mercuryServer) LatestReport(ctx context.Context, lrr *pb.LatestReportRequest) (*pb.LatestReportResponse, error) {
	...
	s.t.Logf("mercury server got latest report from %s for feed id 0x%x", p.PublicKey, lrr.FeedId)
	return nil, nil
}
