...
func TestIntegration_Mercury(t *testing.T) {
	...
	t.Run("receives at least one report per feed from each oracle when EAs are at 50% reliability", func(t *testing.T) {
		pError.Store(50) // 50% chance of EA error
	...
	t.Run("if initialValidFromBlockNumber is set to non-zero value in the job spec", func(t *testing.T) {
		t.Fatal("TODO")
	})
}
