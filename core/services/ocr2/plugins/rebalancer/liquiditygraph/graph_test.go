...
func TestGraph(t *testing.T) {
	...
	for i := 0; i < numNetworks; i++ {
		assert.True(t, g.AddNetwork(models.NetworkID(i), big.NewInt(int64(i*100))))
	}
	...
	netID := models.NetworkID(numNetworks - 1)
	assert.False(t, g.AddNetwork(netID, big.NewInt(123)))

	// network does not exist
	liq, err := g.GetLiquidity(models.NetworkID(numNetworks + 1))
	assert.Empty(t, liq)
	...
	netID1 := rand.Intn(numNetworks)
	netID2 := rand.Intn(numNetworks)
	for netID2 == netID1 {
		netID2 = rand.Intn(numNetworks)
	}
	assert.True(t, g.AddConnection(models.NetworkID(netID1), models.NetworkID(netID2)))
	assert.False(t, g.AddConnection(models.NetworkID(numNetworks+1), models.NetworkID(numNetworks+2)))

	assert.True(t, g.SetLiquidity(models.NetworkID(netID2), big.NewInt(999)))
	liq, err = g.GetLiquidity(models.NetworkID(netID2))
	assert.NoError(t, err)
	assert.Equal(t, int64(999), liq.Int64())
	assert.False(t, g.SetLiquidity(models.NetworkID(999), big.NewInt(999)), "non-existent network")
...
func TestGraphThreadSafety(t *testing.T) {
	...
	g := liquiditygraph.NewGraph()
	for i := 0; i < numNetworks; i++ {
		g.AddNetwork(models.NetworkID(i), big.NewInt(int64(i*100)))
	}
...
func runGraphOperations(t *testing.T, numNetworks int, g liquiditygraph.LiquidityGraph) {
	g.GetNetworks()
	assert.True(t, g.HasNetwork(models.NetworkID(numNetworks-3)))
	assert.False(t, g.HasNetwork(models.NetworkID(numNetworks+1234)))
	assert.False(t, g.IsEmpty())
	newNetID := models.NetworkID(rand.Intn(numNetworks * 3))
	...
	_, err = g.GetLiquidity(newNetID)
	assert.NoError(t, err)
	g.AddConnection(models.NetworkID(1), models.NetworkID(2))
}
