...
func (e EvmReportCodec) Decode(networkID models.NetworkSelector, rebalancerAddress models.Address, binaryReport []byte) (models.Report, rebalancer_report_encoder.IRebalancerLiquidityInstructions, error) {
	unpacked, err := e.onchainReportArguments.Unpack(binaryReport)
	if err != nil {
		return models.Report{}, rebalancer_report_encoder.IRebalancerLiquidityInstructions{}, fmt.Errorf("failed to unpack report: %w", err)
	}
	if len(unpacked) != 1 {
		return models.Report{}, rebalancer_report_encoder.IRebalancerLiquidityInstructions{}, fmt.Errorf("unexpected number of arguments: %d", len(unpacked))
	}
