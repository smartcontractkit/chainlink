...
func (pr *promReporter) reportPendingEthTxes(ctx context.Context, evmChainID *big.Int) (err error) {
	...
	if err := pr.db.QueryRowContext(ctx, `SELECT count(*) FROM evm.eth_txes WHERE state = 'unconfirmed' AND evm_chain_id = $1`, evmChainID.String()).Scan(&unconfirmed); err != nil {
...
func (pr *promReporter) reportMaxUnconfirmedAge(ctx context.Context, evmChainID *big.Int) (err error) {
	...
	now := time.Now()
	if err := pr.db.QueryRowContext(ctx, `SELECT min(initial_broadcast_at) FROM evm.eth_txes WHERE state = 'unconfirmed' AND evm_chain_id = $1`, evmChainID.String()).Scan(&broadcastAt); err != nil {
		return errors.Wrap(err, "failed to query for unconfirmed eth_tx count")
	}
	var seconds float64
...
SELECT MIN(broadcast_before_block_num) FROM evm.eth_tx_attempts
JOIN evm.eth_txes ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id
WHERE evm.eth_txes.state = 'unconfirmed'
AND evm_chain_id = $1
AND evm.eth_txes.state = 'unconfirmed'`, head.EVMChainID.String()).Scan(&earliestUnconfirmedTxBlock)
	if err != nil {
