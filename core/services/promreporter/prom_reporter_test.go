...
func Test_PromReporter_OnNewLongestChain(t *testing.T) {
	...
	t.Run("with unconfirmed evm.eth_txes", func(t *testing.T) {
		...
		require.NoError(t, utils.JustError(db.Exec(`UPDATE evm.eth_tx_attempts SET broadcast_before_block_num = 7 WHERE eth_tx_id = $1`, etx.ID)))
