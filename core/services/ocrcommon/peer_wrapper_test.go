...
func Test_SingletonPeerWrapper_Start(t *testing.T) {
	...
	t.Run("with one p2p key and matching P2P_PEER_ID returns nil", func(t *testing.T) {
		...
		cfg = configtest.NewGeneralConfig(t, func(c *chainlink.Config, s *chainlink.Secrets) {
			c.P2P.V1.Enabled = testutils.Ptr(true)
			c.P2P.PeerID = testutils.Ptr(k.PeerID())
		})
		keyStore = cltest.NewKeyStore(t, db, cfg)
	...
	t.Run("with one p2p key and mismatching P2P_PEER_ID returns error", func(t *testing.T) {
		cfg := configtest.NewGeneralConfig(t, func(c *chainlink.Config, s *chainlink.Secrets) {
			c.P2P.V1.Enabled = testutils.Ptr(true)
			c.P2P.PeerID = testutils.Ptr(p2pkey.PeerID(peerID))
		})
		keyStore := cltest.NewKeyStore(t, db, cfg)
	...
	t.Run("with multiple p2p keys and valid P2P_PEER_ID returns nil", func(t *testing.T) {
		...
		cfg = configtest.NewGeneralConfig(t, func(c *chainlink.Config, s *chainlink.Secrets) {
			c.P2P.V1.Enabled = testutils.Ptr(true)
			c.P2P.PeerID = testutils.Ptr(k2.PeerID())
		})
		keyStore = cltest.NewKeyStore(t, db, cfg)
...
func Test_SingletonPeerWrapper_Close(t *testing.T) {
	...
	cfg = configtest.NewGeneralConfig(t, func(c *chainlink.Config, s *chainlink.Secrets) {
		...
		c.P2P.V1.ListenPort = testutils.Ptr[uint16](0)

		p2paddresses := []string{
			"127.0.0.1:17193",
		}
		c.P2P.V2.ListenAddresses = testutils.Ptr(p2paddresses)
		c.P2P.V2.AnnounceAddresses = testutils.Ptr(p2paddresses)
