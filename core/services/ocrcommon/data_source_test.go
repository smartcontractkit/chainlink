...
func Test_CachedInMemoryDataSourceErrHandling(t *testing.T) {
	...
	t.Run("test normal cache updater fail recovery", func(t *testing.T) {
		...
		dsCache, err := ocrcommon.NewInMemoryDataSourceCache(ds, &mockKVStore, &config.JuelsPerFeeCoinCache{UpdateInterval: commontypes.Interval(time.Second * 2)})
		require.NoError(t, err)
	...
	t.Run("test total updater fail with persisted value recovery", func(t *testing.T) {
		...
		dsCache, err := ocrcommon.NewInMemoryDataSourceCache(ds, &mockKVStore, &config.JuelsPerFeeCoinCache{UpdateInterval: commontypes.Interval(time.Hour * 100)})
		require.NoError(t, err)
