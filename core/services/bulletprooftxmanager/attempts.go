...
var Max256BitUInt *big.Int

func init() {
	var i, e = big.NewInt(2), big.NewInt(256)
	Max256BitUInt = i.Exp(i, e, nil)
}
...
func validateDynamicFeeGas(cfg Config, fee gas.DynamicFee, gasLimit uint64, etx EthTx) error {
	...
	minPrice := cfg.EvmMinGasPriceWei()
	if gasFeeCap.Cmp(minPrice) < 0 {
		return errors.Errorf("cannot create tx attempt: specified gas fee cap of %s is below min configured gas price of %s for key %s", gasFeeCap.String(), minPrice.String(), etx.FromAddress.Hex())
	}
