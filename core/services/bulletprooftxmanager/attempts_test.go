...
func TestBulletproofTxManager_NewDynamicFeeTx(t *testing.T) {
	...
	t.Run("verifies gas tip and fees", func(t *testing.T) {
		tests := []struct {
			...
			{"fee cap below min allowed", assets.GWei(5), assets.GWei(5), func(cfg *configtest.TestGeneralConfig) {
				cfg.Overrides.GlobalEvmMinGasPriceWei = assets.GWei(6)
			}, "specified gas fee cap of 5000000000 is below min configured gas price of 6000000000"},
