...
func TestEthConfirmer_FindEthTxsRequiringResubmissionDueToInsufficientEth(t *testing.T) {
	...
	t.Run("returns all eth_txes with at least one attempt that is in insufficient_eth state", func(t *testing.T) {
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringResubmissionDueToInsufficientEth(context.Background(), db, fromAddress, cltest.FixtureChainID)
		require.NoError(t, err)
	...
	t.Run("does not return eth_txes with different chain ID", func(t *testing.T) {
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringResubmissionDueToInsufficientEth(context.Background(), db, fromAddress, *big.NewInt(42))
		require.NoError(t, err)
...
func TestEthConfirmer_FindEthTxsRequiringRebroadcast(t *testing.T) {
	...
	t.Run("returns nothing when there are no transactions", func(t *testing.T) {
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, fromAddress, currentHead, gasBumpThreshold, 10, 0, lggr, cltest.FixtureChainID)
		require.NoError(t, err)
	...
	t.Run("does nothing if the transaction is from a different address than the one given", func(t *testing.T) {
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, otherAddress, currentHead, gasBumpThreshold, 10, 0, lggr, cltest.FixtureChainID)
		require.NoError(t, err)
	...
	t.Run("returns nothing for different chain id", func(t *testing.T) {
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, fromAddress, currentHead, gasBumpThreshold, 10, 0, lggr, *big.NewInt(42))
		require.NoError(t, err)
	...
	t.Run("returns nothing if threshold is zero", func(t *testing.T) {
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, fromAddress, currentHead, 0, 10, 0, lggr, cltest.FixtureChainID)
		require.NoError(t, err)
	...
	t.Run("does not return more transactions for gas bumping than gasBumpThreshold", func(t *testing.T) {
		...
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, fromAddress, currentHead, gasBumpThreshold, 4, 0, lggr, cltest.FixtureChainID)
		require.NoError(t, err)
		...
		etxs, err = bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, fromAddress, currentHead, gasBumpThreshold, 5, 0, lggr, cltest.FixtureChainID)
		require.NoError(t, err)
		...
		etxs, err = bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, fromAddress, currentHead, gasBumpThreshold, 0, 0, lggr, cltest.FixtureChainID)
		require.NoError(t, err)
	...
	t.Run("ignores pending transactions for another key", func(t *testing.T) {
		...
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, fromAddress, currentHead, gasBumpThreshold, 6, 0, lggr, cltest.FixtureChainID)
		require.NoError(t, err)
	...
	t.Run("applies limit", func(t *testing.T) {
		etxs, err := bulletprooftxmanager.FindEthTxsRequiringRebroadcast(context.Background(), db, fromAddress, currentHead, gasBumpThreshold, 10, 2, lggr, cltest.FixtureChainID)
		require.NoError(t, err)
