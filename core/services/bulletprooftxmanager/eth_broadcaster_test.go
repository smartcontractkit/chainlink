...
func assertEqualEthTxes(t *testing.T, expected, actual bulletprooftxmanager.EthTx) {
	assert.Equal(t, expected.Nonce, actual.Nonce)
	assert.Equal(t, expected.FromAddress, actual.FromAddress)
	assert.Equal(t, expected.EncodedPayload, actual.EncodedPayload)
	assert.Equal(t, expected.Value, actual.Value)
	assert.Equal(t, expected.GasLimit, actual.GasLimit)
	assert.Equal(t, expected.Error, actual.Error)
	assert.Equal(t, expected.BroadcastAt, actual.BroadcastAt)
	assert.Equal(t, expected.State, actual.State)
	assert.Equal(t, expected.Meta, actual.Meta)
	assert.Equal(t, expected.Subject, actual.Subject)
	assert.Equal(t, expected.EVMChainID, actual.EVMChainID)
	assert.Equal(t, expected.PipelineTaskRunID, actual.PipelineTaskRunID)
	assert.Equal(t, expected.MinConfirmations, actual.MinConfirmations)
	assert.Equal(t, expected.AccessList, actual.AccessList)
}
