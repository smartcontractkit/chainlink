...
func Test_EthResender_FindEthTxesRequiringResend(t *testing.T) {
	...
	etxs := []bulletprooftxmanager.EthTx{
		cltest.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, gdb, 0, fromAddress, time.Unix(1616509100, 0)),
		cltest.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, gdb, 1, fromAddress, time.Unix(1616509200, 0)),
		cltest.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, gdb, 2, fromAddress, time.Unix(1616509300, 0)),
	}
	attempt1_2 := newBroadcastLegacyEthTxAttempt(t, etxs[0].ID)
	attempt1_2.GasPrice = utils.NewBig(big.NewInt(10))
	require.NoError(t, gdb.Create(&attempt1_2).Error)

	attempt3_2 := newInProgressLegacyEthTxAttempt(t, etxs[2].ID)
	attempt3_2.GasPrice = utils.NewBig(big.NewInt(10))
	require.NoError(t, gdb.Create(&attempt3_2).Error)
...
func Test_EthResender_Start(t *testing.T) {
	...
	t.Run("resends transactions that have been languishing unconfirmed for too long", func(t *testing.T) {
		...
		etx := cltest.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, gdb, 0, fromAddress, originalBroadcastAt)
		etx2 := cltest.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, gdb, 1, fromAddress, originalBroadcastAt)
		cltest.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, gdb, 2, fromAddress, time.Now().Add(1*time.Hour))
		...
		err := gdb.First(&etx).Error
		require.NoError(t, err)
		err = gdb.First(&etx2).Error
		require.NoError(t, err)
