...
func bumpGasPrice(config Config, currentGasPrice, originalGasPrice *big.Int) (*big.Int, error) {
	...
	if currentGasPrice == nil {
		// Ignore
	} else if currentGasPrice.Cmp(maxGasPrice) > 0 {
	...
	if bumpedGasPrice.Cmp(maxGasPrice) > 0 {
		return maxGasPrice, errors.Errorf("bumped gas price of %s would exceed configured max gas price of %s (original price was %s). %s",
