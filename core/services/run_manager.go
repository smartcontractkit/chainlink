...
func (rm *runManager) MaybeDoubleMinIncomingConfs(run *models.JobRun) error {
	...
	type maxConfsByBlockHash struct {
		MaxConfs  uint32
		BlockHash common.Hash
	}
	var confsByBlockHash []maxConfsByBlockHash
	if err := rm.orm.DB.Raw(`
SELECT coalesce(max(task_runs.minimum_confirmations), 0) as max_confs, run_requests.block_hash as block_hash FROM job_runs 
	...
	GROUP BY run_requests.block_hash
`, run.RunRequest.RequestID, adapters.TaskTypeRandom).Scan(&confsByBlockHash).Error; err != nil {
		return logger.NewErrorw("RunManager: unable to check for duplicate requests", "err", err)
	}
	if len(confsByBlockHash) != 0 {
		var newConfs uint32
		for i := range confsByBlockHash {
			if confsByBlockHash[i].BlockHash.String() == run.RunRequest.BlockHash.String() {
				// If we've seen this blockhash before, then reuse the same incoming confs
				newConfs = confsByBlockHash[i].MaxConfs
				break
			}
			if newConfs < confsByBlockHash[i].MaxConfs*2 {
				newConfs = confsByBlockHash[i].MaxConfs * 2
			}
		}
		for i := range run.TaskRuns {
			if run.TaskRuns[i].TaskSpec.Type == adapters.TaskTypeRandom && run.TaskRuns[i].MinRequiredIncomingConfirmations.Valid {
				// Sanity cap must be less than 256 otherwise the blockhash will not be available.
				// We give it 16 blocks as a buffer to still be able to fulfill within that bound.
				if newConfs > 240 {
					newConfs = 240
				}
