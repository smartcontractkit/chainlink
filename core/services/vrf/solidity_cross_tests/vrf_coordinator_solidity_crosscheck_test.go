...
func TestRegisterProvingKey(t *testing.T) {
	...
	for log.Next() {
		...
		assert.Equal(t, 0, fee.Cmp(log.Event.Fee), "VRFCoordinator logged a different fee than was registered")
	}
	...
	assert.Equal(t, 0, fee.Cmp(serviceAgreement.Fee),
...
func TestRandomnessRequestLog(t *testing.T) {
	...
	for _, tc := range tt {
		...
		assert.Equal(t, 0, actualSeed.Cmp(log.Seed),
		...
		assert.Equal(t, 0, fee.Cmp((*big.Int)(log.Fee)), "VRFCoordinator logged different fee from randomness request!")
...
func TestFulfillRandomness(t *testing.T) {
	...
	assert.Equal(t, 0, proof.Output.Cmp(output), "VRF output from randomness "+
	...
	assert.Equal(t, 0, neilBalance.Cmp(fee), "neil's balance on VRFCoordinator "+
