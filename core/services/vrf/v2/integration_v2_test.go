...
const (
	ConfirmedEthTxesV2Query = `SELECT * FROM evm.eth_txes
		WHERE evm.eth_txes.state = 'confirmed'
		AND evm.eth_txes.meta->>'RequestID' = $1
		AND CAST(evm.eth_txes.meta->>'SubId' AS NUMERIC) = $2 LIMIT 1`
	ConfirmedEthTxesV2PlusQuery = `SELECT * FROM evm.eth_txes
		WHERE evm.eth_txes.state = 'confirmed'
		AND evm.eth_txes.meta->>'RequestID' = $1
		AND CAST(evm.eth_txes.meta->>'GlobalSubId' AS NUMERIC) = $2 LIMIT 1`
	ConfirmedEthTxesV2BatchQuery = `
		SELECT * FROM evm.eth_txes
		WHERE evm.eth_txes.state = 'confirmed'
		AND CAST(evm.eth_txes.meta->>'SubId' AS NUMERIC) = $1`
	ConfirmedEthTxesV2PlusBatchQuery = `
		SELECT * FROM evm.eth_txes
		WHERE evm.eth_txes.state = 'confirmed'
		AND CAST(evm.eth_txes.meta->>'GlobalSubId' AS NUMERIC) = $1`
)
...
func TestStartingCountsV1(t *testing.T) {
	...
	sql := `INSERT INTO evm.eth_txes (nonce, from_address, to_address, encoded_payload, value, gas_limit, state, created_at, broadcast_at, initial_broadcast_at, meta, subject, evm_chain_id, min_confirmations, pipeline_task_run_id)
VALUES (:nonce, :from_address, :to_address, :encoded_payload, :value, :gas_limit, :state, :created_at, :broadcast_at, :initial_broadcast_at, :meta, :subject, :evm_chain_id, :min_confirmations, :pipeline_task_run_id);`
	...
	// add evm.eth_tx_attempts for confirmed
	...
	// add evm.eth_tx_attempts for unconfirmed
	...
	sql = `INSERT INTO evm.eth_tx_attempts (eth_tx_id, gas_price, signed_raw_tx, hash, state, created_at, chain_specific_gas_limit)
	...
	sql = `INSERT INTO evm.eth_receipts (block_hash, tx_hash, block_number, transaction_index, receipt, created_at)
