...
func TestIntegration_SharedProvingKey(t *testing.T) {
	config, _, cfgCleanup := cltest.BootstrapThrowawayORM(t, "vrf_shared_proving_key_integration_test", true, true)
	defer cfgCleanup()
	config.Config.Dialect = dialects.PostgresWithoutLock
	config.Set("MIN_INCOMING_CONFIRMATIONS", 3) // allow space for another node to answer before us
