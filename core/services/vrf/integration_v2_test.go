...
func newVRFCoordinatorV2Universe(t *testing.T, key ethkey.KeyV2, numConsumers int) coordinatorV2Universe {
	...
	genesisData := core.GenesisAlloc{
		...
		neil.From:     {Balance: assets.ItoEther(1000).ToInt()},
		ned.From:      {Balance: assets.ItoEther(1000).ToInt()},
		nallory.From:  {Balance: assets.ItoEther(1000).ToInt()},
		evil.From:     {Balance: assets.ItoEther(1000).ToInt()},
		reverter.From: {Balance: assets.ItoEther(1000).ToInt()},
	}
	for _, consumer := range vrfConsumers {
		genesisData[consumer.From] = core.GenesisAccount{
			Balance: assets.ItoEther(1000).ToInt(),
		}
	}
	...
	for _, author := range vrfConsumers {
		...
		_, err = linkContract.Transfer(sergey, consumerContractAddress, assets.ItoEther(500).ToInt()) // Actually, LINK
	...
	_, err = linkContract.Transfer(sergey, maliciousConsumerContractAddress, assets.ItoEther(1).ToInt()) // Actually, LINK
	...
	_, err = linkContract.Transfer(sergey, revertingConsumerContractAddress, assets.ItoEther(500).ToInt()) // Actually, LINK
...
func sendEth(t *testing.T, key ethkey.KeyV2, ec *backends.SimulatedBackend, to common.Address, eth int) {
	...
	tx := gethtypes.NewTx(&gethtypes.DynamicFeeTx{
		...
		GasFeeCap: assets.ItoGWei(10).ToInt(), // block base fee in sim
		Gas:       uint64(21_000),
		To:        &to,
...
func TestVRFV2Integration_SingleConsumer_HappyPath_BatchFulfillment(t *testing.T) {
	...
	configureSimChain(t, app, map[string]types.ChainCfg{
		key1.Address.String(): {
			EvmMaxGasPriceWei: assets.ItoGWei(10),
		},
	}, assets.ItoGWei(10).ToInt())
...
func TestVRFV2Integration_SingleConsumer_Wrapper(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = uni.linkContract.TransferAndCall(uni.sergey, uni.rootContractAddress, assets.ItoEther(100).ToInt(), b)
	require.NoError(t, err)
	uni.backend.Commit()

	// Fund Consumer Contract.
	_, err = uni.linkContract.Transfer(uni.sergey, consumerAddress, assets.ItoEther(100).ToInt())
	require.NoError(t, err)
	uni.backend.Commit()
...
func TestVRFV2Integration_SingleConsumer_NeedsTopUp(t *testing.T) {
	...
	configureSimChain(t, app, map[string]types.ChainCfg{
		key.Address.String(): {
			EvmMaxGasPriceWei: assets.ItoGWei(1000),
		},
	}, assets.ItoGWei(1000).ToInt())
	require.NoError(t, app.Start(testutils.Context(t)))
...
func TestVRFV2Integration_SingleConsumer_BigGasCallback_Sandwich(t *testing.T) {
	...
	subID := subscribeAndAssertSubscriptionCreatedEvent(t, consumerContract, consumer, consumerContractAddress, assets.ItoEther(2).ToInt(), uni)
	...
	configureSimChain(t, app, map[string]types.ChainCfg{
		key1.Address.String(): {
			EvmMaxGasPriceWei: assets.ItoGWei(100),
		},
	}, assets.ItoGWei(100).ToInt())
	require.NoError(t, app.Start(testutils.Context(t)))
...
func TestExternalOwnerConsumerExample(t *testing.T) {
	...
	genesisData := core.GenesisAlloc{
		owner.From:  {Balance: assets.ItoEther(10).ToInt()},
		random.From: {Balance: assets.ItoEther(10).ToInt()},
	}
	...
	require.NoError(t, err)
	backend.Commit()
	_, err = linkContract.Transfer(owner, consumerAddress, assets.ItoEther(2).ToInt())
	require.NoError(t, err)
	backend.Commit()
	AssertLinkBalances(t, linkContract, []common.Address{owner.From, consumerAddress}, []*big.Int{assets.ItoEther(999_999_998).ToInt(), assets.ItoEther(2).ToInt()})
...
func TestSimpleConsumerExample(t *testing.T) {
	...
	backend.Commit()
	AssertLinkBalances(t, linkContract, []common.Address{owner.From, consumerAddress}, []*big.Int{assets.ItoEther(999_999_998).ToInt(), assets.ItoEther(2).ToInt()})
	_, err = consumer.TopUpSubscription(owner, assets.ItoEther(1).ToInt())
	require.NoError(t, err)
	backend.Commit()
	AssertLinkBalances(t, linkContract, []common.Address{owner.From, consumerAddress, coordinatorAddress}, []*big.Int{assets.ItoEther(999_999_998).ToInt(), assets.ItoEther(1).ToInt(), assets.ItoEther(1).ToInt()})
	// Non-owner cannot withdraw
	_, err = consumer.Withdraw(random, assets.ItoEther(1).ToInt(), owner.From)
	require.Error(t, err)
	_, err = consumer.Withdraw(owner, assets.ItoEther(1).ToInt(), owner.From)
	require.NoError(t, err)
	backend.Commit()
	AssertLinkBalances(t, linkContract, []common.Address{owner.From, consumerAddress, coordinatorAddress}, []*big.Int{assets.ItoEther(999_999_999).ToInt(), assets.ItoEther(0).ToInt(), assets.ItoEther(1).ToInt()})
	_, err = consumer.Unsubscribe(owner, owner.From)
	require.NoError(t, err)
	backend.Commit()
	AssertLinkBalances(t, linkContract, []common.Address{owner.From, consumerAddress, coordinatorAddress}, []*big.Int{assets.ItoEther(1_000_000_000).ToInt(), assets.ItoEther(0).ToInt(), assets.ItoEther(0).ToInt()})
}
...
func TestIntegrationVRFV2(t *testing.T) {
	...
	gasPrice := decimal.NewFromBigInt(assets.ItoGWei(1).ToInt(), 0)
	configureSimChain(t, app, map[string]types.ChainCfg{
	...
	AssertLinkBalances(t, uni.linkContract, []common.Address{
		...
		assets.ItoEther(500).ToInt(), // 500 link
		big.NewInt(0),                // 0 link
	})
...
func TestMaliciousConsumer(t *testing.T) {
	...
	config.Overrides.GlobalEvmMaxGasPriceWei = assets.ItoGWei(1)
	config.Overrides.GlobalEvmGasPriceDefault = assets.ItoGWei(1)
	config.Overrides.GlobalEvmGasFeeCapDefault = assets.ItoGWei(1)
