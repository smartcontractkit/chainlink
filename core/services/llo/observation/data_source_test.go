...
func Test_observationTuningHelpers(t *testing.T) {
	...
	assert.Equal(t, 120*time.Millisecond, staleRefreshSkipThreshold(100*time.Millisecond))
	assert.Less(t, staleRefreshSkipThreshold(100*time.Millisecond), cacheEntryTTL(100*time.Millisecond))
	assert.Less(t, staleRefreshSkipThreshold(100*time.Millisecond)+observationLoopPacing(100*time.Millisecond), cacheEntryTTL(100*time.Millisecond))
