...
func (backup *databaseBackup) RunBackupGracefully() {
	...
	if err != nil {
		backup.logger.Errorf("DatabaseBackup: Failed after %s with: %v", duration, err)
	} else {
		backup.logger.Infof("DatabaseBackup: Database backup finished successfully after %s: %d bytes written to %s", duration, result.size, result.path)
	}
}
