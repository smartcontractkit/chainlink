...
func New(ds sqlutil.DataSource, scryptParams scrypt.ScryptParams, announce Logf) Master {
	return newMaster(ds, scryptParams, announce)
}

func newMaster(ds sqlutil.DataSource, scryptParams scrypt.ScryptParams, announce Logf) *master {
	orm := NewORM(ds)
	km := &keyManager{
		orm:          orm,
		keystateORM:  orm,
