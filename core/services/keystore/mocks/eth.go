...
import (
	...
	zksync "github.com/smartcontractkit/chainlink/v2/core/chains/evm/types/zksync"
)
...
// SignZkSyncTx provides a mock function with given fields: ctx, address, tx, chainID
func (_m *Eth) SignZkSyncTx(ctx context.Context, address common.Address, tx *zksync.Transaction712, chainID *big.Int) ([]byte, error) {
	ret := _m.Called(ctx, address, tx, chainID)

	if len(ret) == 0 {
		panic("no return value specified for SignZkSyncTx")
	}

	var r0 []byte
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, common.Address, *zksync.Transaction712, *big.Int) ([]byte, error)); ok {
		return rf(ctx, address, tx, chainID)
	}
	if rf, ok := ret.Get(0).(func(context.Context, common.Address, *zksync.Transaction712, *big.Int) []byte); ok {
		r0 = rf(ctx, address, tx, chainID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]byte)
		}
	...
	if rf, ok := ret.Get(1).(func(context.Context, common.Address, *zksync.Transaction712, *big.Int) error); ok {
		r1 = rf(ctx, address, tx, chainID)
	} else {
		r1 = ret.Error(1)
	}
