...
type OCR2 interface {
	...
	GetAllOfType(types.ChainType) ([]ocr2key.KeyBundle, error)
	Create(context.Context, types.ChainType) (ocr2key.KeyBundle, error)
	...
	EnsureKeys(ctx context.Context, enabledChains ...types.ChainType) error
}
...
func (ks ocr2) GetAllOfType(chainType types.ChainType) ([]ocr2key.KeyBundle, error) {
	keys := []ocr2key.KeyBundle{}
	ks.lock.RLock()
	defer ks.lock.RUnlock()
	if ks.isLocked() {
...
func (ks ocr2) Create(ctx context.Context, chainType types.ChainType) (ocr2key.KeyBundle, error) {
	ks.lock.Lock()
...
func (ks ocr2) EnsureKeys(ctx context.Context, enabledChains ...types.ChainType) error {
	ks.lock.Lock()
	defer ks.lock.Unlock()
	if ks.isLocked() {
		return ErrLocked
...
func (ks ocr2) create(ctx context.Context, chainType types.ChainType) (ocr2key.KeyBundle, error) {
	if !types.IsSupportedChainType(chainType) {
