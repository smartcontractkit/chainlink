...
// Test_StellarLooppSigner verifies the loop.Keystore adapter the Stellar
...
func Test_StellarLooppSigner(t *testing.T) {
	...
	signer := &keystore.StellarLooppSigner{Stellar: ks}

	accounts, err := signer.Accounts(ctx)
	require.NoError(t, err)
	require.Contains(t, accounts, key.ID()) // account == StrKey "G..." address

	payload := []byte("tx-hash")
	viaSigner, err := signer.Sign(ctx, key.ID(), payload)
	require.NoError(t, err)
	viaKey, err := key.Sign(payload)
	require.NoError(t, err)
