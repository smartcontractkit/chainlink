...
func Test_EthKeyStore(t *testing.T) {
	...
	t.Run("Create / GetAll / Get", func(t *testing.T) {
		...
		require.NoError(t, err)
		commontestutils.RequireEqualKeys(t, key, foundKey)
		// adds ethkey.State
	...
	t.Run("GetAll ordering", func(t *testing.T) {
		...
		for i := range keys {
			commontestutils.RequireEqualKeys(t, keys[i], retrievedKeys[i])
		}
	})
	...
	t.Run("EnsureKeys / EnabledKeysForChain", func(t *testing.T) {
		...
		for i := range sendingKeys1 {
			commontestutils.RequireEqualKeys(t, sendingKeys1[i], sendingKeys2[i])
		}
	})
	...
	t.Run("EnabledKeysForChain with specified chain ID", func(t *testing.T) {
		...
		commontestutils.RequireEqualKeys(t, key, keys[0])

		keys, err = ethKeyStore.EnabledKeysForChain(ctx, big.NewInt(1337))
		require.NoError(t, err)
		require.Len(t, keys, 1)
		commontestutils.RequireEqualKeys(t, key2, keys[0])
...
func Test_EthKeyStore_ListKeys(t *testing.T) {
	...
	t.Run("returns keys sorted by State.ID (lowest first)", func(t *testing.T) {
		...
		commontestutils.RequireEqualKeys(t, key1, keys[0])
		commontestutils.RequireEqualKeys(t, key2, keys[1])
		commontestutils.RequireEqualKeys(t, key3, keys[2])
	})
	...
	t.Run("excludes disabled keys", func(t *testing.T) {
		...
		commontestutils.RequireEqualKeys(t, key1, keys[0])
		commontestutils.RequireEqualKeys(t, key3, keys[1])
	...
	t.Run("deterministic ordering across multiple calls", func(t *testing.T) {
		...
		for i := range keys1 {
			commontestutils.RequireEqualKeys(t, keys1[i], keys2[i])
			commontestutils.RequireEqualKeys(t, keys1[i], keys3[i])
		}
	})
	t.Run("handles keys enabled for multiple chains correctly", func(t *testing.T) {
		...
		commontestutils.RequireEqualKeys(t, key1, keysSimulated[0])
	})
