...
INSERT INTO offchainreporting2_discoverer_announcements (local_peer_id, remote_peer_id, ann, created_at, updated_at)
VALUES ($1, $2, $3, NOW(), NOW())
ON CONFLICT (local_peer_id, remote_peer_id)
DO UPDATE SET
	ann = EXCLUDED.ann,
...
SELECT remote_peer_id, ann
FROM offchainreporting2_discoverer_announcements
WHERE remote_peer_id = ANY($1) AND local_peer_id = $2`, pq.Array(peerIDs), d.peerID)
	if err != nil {
