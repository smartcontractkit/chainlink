...
func (ds *dataSource) Observe(ctx context.Context) (*big.Int, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "error executing run for spec ID %v", ds.spec.ID)
	}
...
type mockJuelsEthDataSource struct {
	v *big.Int
}

func NewMockJuelsEthDatasource(v *big.Int) *mockJuelsEthDataSource {
	return &mockJuelsEthDataSource{v}
}

func (ds *mockJuelsEthDataSource) Observe(ctx context.Context) (*big.Int, error) {
	return ds.v, nil
}
