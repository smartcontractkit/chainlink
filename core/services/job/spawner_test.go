...
func TestSpawner_CreateJobDeleteJob(t *testing.T) {
	...
	t.Run("starts and stops job services when jobs are added and removed", func(t *testing.T) {
		...
		orm := job.NewORM(db, config.Config, pipeline.NewORM(db, config), eventBroadcaster, &postgres.NullAdvisoryLocker{})
		defer orm.Close()
		...
		delegateA := &delegate{jobSpecA.Type, []job.Service{serviceA1, serviceA2}, 0, make(chan struct{}), offchainreporting.NewDelegate(nil, nil, orm, nil, nil, nil, ethClient, nil, nil, monitoringEndpoint, nil)}
		...
		delegateB := &delegate{jobSpecB.Type, []job.Service{serviceB1, serviceB2}, 0, make(chan struct{}), offchainreporting.NewDelegate(nil, nil, orm, nil, nil, nil, ethClient, nil, nil, monitoringEndpoint, nil)}
	...
	t.Run("starts job services from the DB when .Start() is called", func(t *testing.T) {
		...
		orm := job.NewORM(db, config.Config, pipeline.NewORM(db, config), eventBroadcaster, &postgres.NullAdvisoryLocker{})
		defer orm.Close()
		delegateA := &delegate{jobSpecA.Type, []job.Service{serviceA1, serviceA2}, 0, nil, offchainreporting.NewDelegate(nil, nil, orm, nil, nil, nil, ethClient, nil, nil, monitoringEndpoint, nil)}
		spawner := job.NewSpawner(orm, config, map[job.Type]job.Delegate{
			jobSpecA.Type: delegateA,
		})
	...
	t.Run("closes job services on 'delete_from_jobs' postgres event", func(t *testing.T) {
		...
		orm := job.NewORM(db, config.Config, pipeline.NewORM(db, config), eventBroadcaster, &postgres.NullAdvisoryLocker{})
		defer orm.Close()
		delegateA := &delegate{jobSpecA.Type, []job.Service{serviceA1, serviceA2}, 0, nil, offchainreporting.NewDelegate(nil, nil, nil, nil, nil, nil, ethClient, nil, nil, monitoringEndpoint, nil)}
		spawner := job.NewSpawner(orm, config, map[job.Type]job.Delegate{
			jobSpecA.Type: delegateA,
		})
