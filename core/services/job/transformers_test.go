...
func TestJSONParseTransformer(t *testing.T) {
	tests := []struct {
		name            string
		input           string
		path            []string
	...
	for _, tt := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			transformer := job.JSONParseTransformer{Path: test.path}
...
func TestMultiplyTransformer_Happy(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		test := test
		t.Run(test.name, func(t *testing.T) {
			transformer := job.MultiplyTransformer{Times: test.times}
			result, err := transformer.Transform(test.input)
			require.NoError(t, err)
...
func TestMultiplyTransformer_Unhappy(t *testing.T) {
	tests := []struct {
		name  string
		times decimal.Decimal
		input interface{}
