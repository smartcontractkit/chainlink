...
func Test_FindGatewayJobID(t *testing.T) {
	...
	assert.NoError(t, err, "failed to find gateway job by auth gateway id")
	assert.Equal(t, jobSpec.ID, id, "mismatch job id")

	// find only by node server config
	gatewayJobSpec = job.GatewaySpec{
		GatewayConfig: job.JSONConfig{
	...
	id, err = orm.FindGatewayJobID(ctx, gatewayJobSpec)
	assert.NoError(t, err, "failed to find gateway job by node server config")
	assert.Equal(t, jobSpec.ID, id, "mismatch job id")

	// find only by user server config
	gatewayJobSpec = job.GatewaySpec{
		GatewayConfig: job.JSONConfig{
			"UserServerConfig": map[string]interface{}{
				"Path": "/user",
				"Port": int64(8081),
			},
		},
	...
	id, err = orm.FindGatewayJobID(ctx, gatewayJobSpec)
	assert.NoError(t, err, "failed to find gateway job by user server config")
	assert.Equal(t, jobSpec.ID, id, "mismatch job id")
