...
func (o *orm) CreateJob(ctx context.Context, jb *Job) error {
	...
	err := o.transact(ctx, false, func(tx *orm) error {
		...
		case StandardCapability:
			sql := `INSERT INTO standardcapability_specs (command, config, created_at, updated_at)
			VALUES (:command, :config, NOW(), NOW())
			RETURNING id;`
			specID, err := tx.prepareQuerySpecID(ctx, sql, jb.StandardCapabilitySpec)
			if err != nil {
				return errors.Wrap(err, "failed to create StandardCapability for jobSpec")
			}
			jb.StandardCapabilitySpecID = &specID
	...
	return o.transact(ctx, false, func(tx *orm) error {
		...
		if job.ID == 0 {
			query = `INSERT INTO jobs (name, stream_id, schema_version, type, max_task_duration, ocr_oracle_spec_id, ocr2_oracle_spec_id, direct_request_spec_id, flux_monitor_spec_id,
                ...
                legacy_gas_station_server_spec_id, legacy_gas_station_sidecar_spec_id, workflow_spec_id, standard_capability_spec_id, external_job_id, gas_limit, forwarding_allowed, created_at)
		VALUES (:name, :stream_id, :schema_version, :type, :max_task_duration, :ocr_oracle_spec_id, :ocr2_oracle_spec_id, :direct_request_spec_id, :flux_monitor_spec_id,
				:keeper_spec_id, :cron_spec_id, :vrf_spec_id, :webhook_spec_id, :blockhash_store_spec_id, :bootstrap_spec_id, :block_header_feeder_spec_id, :gateway_spec_id, 
				:legacy_gas_station_server_spec_id, :legacy_gas_station_sidecar_spec_id, :workflow_spec_id, :standard_capability_spec_id, :external_job_id, :gas_limit, :forwarding_allowed, NOW())
		RETURNING *;`
		} else {
...
func (o *orm) DeleteJob(ctx context.Context, id int32) error {
	...
	query := `
		...
		deleted_standardcapability_specs AS (
			DELETE FROM standardcapability_specs WHERE id in (SELECT standard_capability_spec_id FROM deleted_jobs)
		),	                               
...
func (o *orm) loadAllJobTypes(ctx context.Context, job *Job) error {
	return multierr.Combine(
		...
		o.loadJobType(ctx, job, "StandardCapabilitySpec", "standardcapability_specs", job.StandardCapabilitySpecID),
	)
}
