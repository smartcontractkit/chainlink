...
func (o *orm) InsertJob(job *Job, qopts ...pg.QOpt) error {
	...
	if job.ID == 0 {
		query = `INSERT INTO jobs (pipeline_spec_id, name, schema_version, type, max_task_duration, external_job_id, gas_limit,  forwarding_allowed, type_spec, created_at)
		VALUES (:pipeline_spec_id, :name, :schema_version, :type, :max_task_duration, :external_job_id, :gas_limit, :forwarding_allowed, :type_spec, NOW())
		RETURNING *;`
	} else {
		query = `INSERT INTO jobs (id, pipeline_spec_id, name, schema_version, type, max_task_duration, external_job_id, gas_limit, forwarding_allowed, type_spec, created_at)
		VALUES (:id, :pipeline_spec_id, :name, :schema_version, :type, :max_task_duration, :external_job_id, :gas_limit, :forwarding_allowed, :type_spec, NOW())
		RETURNING *;`
	}
