...
func (_m *Delegate) FromDBRow(spec job.JobSpecV2) job.Spec {
	ret := _m.Called(spec)

	var r0 job.Spec
	if rf, ok := ret.Get(0).(func(job.JobSpecV2) job.Spec); ok {
		r0 = rf(spec)
	} else {
		if ret.Get(0) != nil {
...
func (_m *Delegate) ToDBRow(spec job.Spec) job.JobSpecV2 {
	ret := _m.Called(spec)

	var r0 job.JobSpecV2
	if rf, ok := ret.Get(0).(func(job.Spec) job.JobSpecV2); ok {
		r0 = rf(spec)
	} else {
