...
const (
	...
	CapabilityRunner     Type = (Type)(pipeline.CapabilityRunnerJobType)
...
type Job struct {
	...
	CapabilityRunnerSpecID     *int32
	CapabilityRunnerSpec       *CapabilityRunnerSpec
...
// CapabilityRunnerSpec defines a capability binary run via the empty LOOP: the
// node launches Command with Args and notifies it of CRE settings (limits)
// updates over its --http_port reload endpoint.
type CapabilityRunnerSpec struct {
	ID        int32
	CreatedAt time.Time `toml:"-"`
	UpdatedAt time.Time `toml:"-"`

	// Command is the name of the binary to run, resolved the same way as
	// StandardCapabilitiesSpec.Command.
	Command string `toml:"command"`
	// Args are passed to the binary verbatim; they must include --http_port.
	Args pq.StringArray `toml:"args" db:"args"`
}
