...
type (
	...
	Delegate interface {
		...
		OnDeleteJob(ctx context.Context, jb Job, ds sqlutil.DataSource) error
	}
...
func (js *spawner) DeleteJob(ctx context.Context, ds sqlutil.DataSource, jobID int32) error {
	...
	err := sqlutil.Transact(ctx, js.orm.WithDataSource, ds, nil, func(tx ORM) error {
		...
		err = aj.delegate.OnDeleteJob(ctx, aj.spec, tx.DataSource())
		if err != nil {
			return err
		}
...
func (n *NullDelegate) OnDeleteJob(ctx context.Context, spec Job, q sqlutil.DataSource) error {
	return nil
}
