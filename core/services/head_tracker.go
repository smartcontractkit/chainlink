...
func (ht *HeadTracker) handleNewHighestHead(ctx context.Context, head models.Head) error {
	...
	if err != nil {
		return errors.Wrap(err, "HeadTracker#batchFetchReceipts failed ")
	}

	//for _, receipt := range receipts {
	//	ht.logger.Debugw("HeadTracker: TransactionReceipt", "numLogs", len(receipt.Logs), "GasUsed", receipt.GasUsed)
	//	for _, log := range receipt.Logs {
	//		ht.logger.Debugw("HeadTracker: LOG", "Topics.len", len(log.Topics))
	//	}
	//}
...
func (ht *HeadTracker) batchFetchReceipts(ctx context.Context, txs []*types.Transaction) (receipts []bulletprooftxmanager.Receipt, err error) {
