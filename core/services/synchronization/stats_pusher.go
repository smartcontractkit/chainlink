...
func createSyncEventWithStatsPusher(sp StatsPusher, orm *orm.ORM) func(*gorm.Scope) {
	return func(scope *gorm.Scope) {
		...
		if err != nil {
			scopeErr := scope.Err(errors.Wrap(err, "createSyncEvent#json.Marshal failed"))
			logger.ErrorIf(scopeErr)
			return
		}
		...
		if err != nil {
			scopeErr := scope.Err(errors.Wrap(err, "createSyncEvent#Create failed"))
			logger.ErrorIf(scopeErr)
			return
		}
	}
