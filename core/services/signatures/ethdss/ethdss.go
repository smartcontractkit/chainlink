...
func NewDSS(secret kyber.Scalar, participants []kyber.Point,
	long, random DistKeyShare, msg *big.Int, T int) (*DSS, error) {
	public := secp256k1Group.Point().Mul(secret, nil)
	var i int
	var found bool
	...
	return &DSS{
		...
		longPoly: share.NewPubPoly(secp256k1Suite,
			secp256k1Group.Point().Base(), long.Commitments()),
		random: random,
		randomPoly: share.NewPubPoly(secp256k1Suite,
			secp256k1Group.Point().Base(), random.Commitments()),
		msg:         msg,
		T:           T,
		partialsIdx: make(map[int]bool),
		sessionID:   sessionID(secp256k1Suite, long, random),
	}, nil
}
