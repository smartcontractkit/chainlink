...
func TestMultiplyTask_Unhappy(t *testing.T) {
	...
	tests := []struct {
		...
		{"map", "100", "", []pipeline.Result{{Value: map[string]interface{}{"chain": "link"}}}, pipeline.ErrBadInput, "input"},
		{"slice", "100", "", []pipeline.Result{{Value: []interface{}{"chain", "link"}}}, pipeline.ErrBadInput, "input"},
	}
