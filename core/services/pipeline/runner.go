...
func NewRun(spec Spec, vars Vars) *Run {
	return &Run{
		...
		Inputs:         json_serializable.JSONSerializable{Val: vars.vars, Valid: true},
		Outputs:        json_serializable.JSONSerializable{Val: nil, Valid: false},
		CreatedAt:      time.Now(),
	}
}
...
func (r *runner) run(ctx context.Context, pipeline *Pipeline, run *Run, vars Vars, l logger.Logger) TaskRunResults {
	...
	if run.FinishedAt.Valid {
		...
		run.Outputs = json_serializable.JSONSerializable{Val: outputs, Valid: true}
