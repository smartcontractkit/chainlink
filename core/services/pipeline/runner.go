...
type Runner interface {
	...
	ExecuteRun(ctx context.Context, spec Spec, pipelineInputs []Result, meta JSONSerializable, l logger.Logger) (trrs TaskRunResults, err error)
...
func (r *runner) ExecuteRun(ctx context.Context, spec Spec, pipelineInputs []Result, meta JSONSerializable, l logger.Logger) (TaskRunResults, error) {
	var (
		trrs            TaskRunResults
		err             error
		retry           bool
	...
	for i = 0; i < numPanicRetries; i++ {
		trrs, retry, err = r.executeRun(ctx, r.orm.DB(), spec, pipelineInputs, meta, l)
		if retry {
...
func (r *runner) executeRun(
	...
	return taskRunResults, retry, err
}
...
func (r *runner) ExecuteAndInsertFinishedRun(ctx context.Context, spec Spec, pipelineInputs []Result, meta JSONSerializable, l logger.Logger, saveSuccessfulTaskRuns bool) (runID int64, finalResult FinalResult, err error) {
	...
	trrs, err := r.ExecuteRun(ctx, spec, pipelineInputs, meta, l)
	if err != nil {
