...
func TestStringSliceParam_UnmarshalPipelineParam(t *testing.T) {
	...
	tests := []struct {
		...
		{"[]any with strings", []any{"foo", "bar", "baz"}, expected, nil},
		{"[]any with []byte", []any{[]byte("foo"), []byte("bar"), []byte("baz")}, expected, nil},
		...
		{"[]any with bad types", []any{123, true}, nil, pipeline.ErrBadInput},
	}
...
func TestAddressSliceParam_UnmarshalPipelineParam(t *testing.T) {
	...
	tests := []struct {
		...
		{"[]any with common.Address", []any{addr1, addr2}, expected, nil},
		{"[]any with strings", []any{addr1.String(), addr2.String()}, expected, nil},
		{"[]any with []byte", []any{[]byte(addr1.String()), []byte(addr2.String())}, expected, nil},
...
func TestSliceParam_UnmarshalPipelineParam(t *testing.T) {
	...
	tests := []struct {
		...
		{"[]any", []any{1, 2, 3}, pipeline.SliceParam([]any{1, 2, 3}), nil},
...
func TestHashSliceParam_UnmarshalPipelineParam(t *testing.T) {
	...
	tests := []struct {
		...
		{"[]any with common.Hash", []any{hash1, hash2}, expected, nil},
		{"[]any with strings", []any{hash1.String(), hash2.String()}, expected, nil},
		{"[]any with []byte", []any{[]byte(hash1.String()), []byte(hash2.String())}, expected, nil},
...
func TestDecimalSliceParam_UnmarshalPipelineParam(t *testing.T) {
	...
	tests := []struct {
		...
		{"[]any", []any{1.1, "2.2", *mustDecimal(t, "3.3")}, expected, nil},
		{"string", `[1.1, "2.2", 3.3]`, expected, nil},
		{"[]byte", `[1.1, "2.2", 3.3]`, expected, nil},
		{"[]any with error", `[1.1, true, "abc"]`, pipeline.DecimalSliceParam(nil), pipeline.ErrBadInput},
...
func TestJSONPathParam_UnmarshalPipelineParam(t *testing.T) {
	...
	tests := []struct {
		...
		{"[]any", []any{"1.1", "2.2", "3.3", "sergey"}, expected, nil},
