...
// InsertFinishedRun provides a mock function with given fields: tx, run, trrs, saveSuccessfulTaskRuns
func (_m *ORM) InsertFinishedRun(tx *gorm.DB, run pipeline.Run, trrs []pipeline.TaskRunResult, saveSuccessfulTaskRuns bool) (int64, error) {
	ret := _m.Called(tx, run, trrs, saveSuccessfulTaskRuns)

	var r0 int64
	if rf, ok := ret.Get(0).(func(*gorm.DB, pipeline.Run, []pipeline.TaskRunResult, bool) int64); ok {
		r0 = rf(tx, run, trrs, saveSuccessfulTaskRuns)
	} else {
		r0 = ret.Get(0).(int64)
	}
	...
	if rf, ok := ret.Get(1).(func(*gorm.DB, pipeline.Run, []pipeline.TaskRunResult, bool) error); ok {
		r1 = rf(tx, run, trrs, saveSuccessfulTaskRuns)
	} else {
		r1 = ret.Error(1)
	}
