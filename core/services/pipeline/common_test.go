...
func TestUnmarshalTaskFromMap(t *testing.T) {
	...
	t.Run("returns error if task is not the right type", func(t *testing.T) {
		...
		require.EqualError(t, err, "UnmarshalTaskFromMap: UnmarshalTaskFromMap only accepts a map[string]any or a map[string]string. Got <nil> (<nil>) of type <nil>")
		...
		_, err = pipeline.UnmarshalTaskFromMap(pipeline.TaskType("http"), taskMap, 0, "foo-dot-id")
		require.Error(t, err)
		require.Contains(t, err.Error(), "UnmarshalTaskFromMap: UnmarshalTaskFromMap only accepts a map[string]any or a map[string]string")
	})
