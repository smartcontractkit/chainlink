...
func Test_PipelineRunner_ExecuteTaskRunsWithVars(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			submit, submitBt := makeBridge(t, db, expectedRequestSubmit, map[string]any{"ok": true})
			defer submit.Close()
			...
			_, taskRunResults, err := runner.ExecuteRun(t.Context(), spec, pipeline.NewVarsFrom(test.vars))
			require.NoError(t, err)
...
func Test_PipelineRunner_MultipleOutputs(t *testing.T) {
	...
	_, trrs, err := r.ExecuteRun(t.Context(), pipeline.Spec{
		DotDagSource: `
...
`, bt.Name.String(), s2.URL, s4.URL, s5.URL)
	...
	incomplete, err := r.Run(t.Context(), run, false, nil)
	require.NoError(t, err)
