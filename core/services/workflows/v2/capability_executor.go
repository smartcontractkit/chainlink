...
func (c *ExecutionHelper) checkChainAllowed(ctx context.Context, request *sdkpb.CapabilityRequest) (string, error) {
	...
	if chainSelector, err := capabilities.ChainSelectorLabel(labels); err != nil {
		...
		if err2 != nil {
			if errors.Is(err2, limits.ErrorNotAllowed{}) {
				return capName, fmt.Errorf("unable to call capability %s: ChainSelector %d: %w", request.Id, *chainSelector, err2)
			}
			return capName, fmt.Errorf("failed to check access for ChainSelector %d: %w", *chainSelector, err2)
		}
	}
...
func (c *ExecutionHelper) checkLimit(ctx context.Context, capName, requestMethod string) error {
	limiter, ok := c.callLimiters[capCall{name: capName, method: requestMethod}]
	if ok {
		c.mu.Lock()
		if c.callCounts == nil {
