...
func Test_Handler(t *testing.T) {
	...
	t.Run("fails with unsupported event type", func(t *testing.T) {
		...
		h, err := NewEventHandler(lggr, wfStore, nil, true, registry, NewEngineRegistry(), emitter, rl, workflowLimits, store, workflowEncryptionKey)
		require.NoError(t, err)
...
func testRunningWorkflow(t *testing.T, tc testCase) {
	...
	t.Run(tc.Name, func(t *testing.T) {
		...
		h, err := NewEventHandler(lggr, store, nil, true, registry, NewEngineRegistry(), emitter, rl, workflowLimits, artifactStore, workflowEncryptionKey, opts...)
		require.NoError(t, err)
...
func Test_workflowDeletedHandler(t *testing.T) {
	t.Run("success deleting existing engine and spec", func(t *testing.T) {
		...
		h, err := NewEventHandler(lggr, store, nil, true, registry, NewEngineRegistry(), emitter, rl, workflowLimits, artifactStore, workflowEncryptionKey, WithEngineRegistry(er))
		require.NoError(t, err)
	...
	t.Run("removes from DB before engine registry", func(t *testing.T) {
		...
		h, err := NewEventHandler(lggr, store, nil, true, registry, NewEngineRegistry(), emitter, rl, workflowLimits, mockAS, workflowEncryptionKey, WithEngineRegistry(er))
		require.NoError(t, err)
		err =
