...
type stepDefinitionYaml struct {
	...
	//
	// There are two ways to specify a id - using a string as a fully qualified ID or a structured table. When using a table, labels are ordered alphanumerically and joined into a string following a
	//  {id}:{label1_key}_{label1_value}:{label2_key}_{label2_value}@{version}
	// pattern.
	//
