...
func interpolateKey(key string, state executionState) (any, error) {
	...
	switch parts[1] {
	case "inputs":
		// Can this error? What happens if this input depends on a previous step's output?
		value = sc.inputs
	case "outputs":
	...
	for _, r := range remainingParts {
		...
		case map[string]any:
			inner, ok := v[r]
			if !ok {
				// probably worthwhile to print the entire reference here
		...
		case []any:
			...
			if (i > len(v)-1) || (i < 0){
				return "", fmt.Errorf("could not interpolate ref part `%s` in `%+v`: index out of bounds %d", r, v, i)
			}
