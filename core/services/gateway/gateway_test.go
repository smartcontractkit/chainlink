...
`)
	...
	_, err := gateway.NewGatewayFromConfig(parseTOMLConfig(t, tomlConfig), gateway.NewHandlerFactory(nil, lggr), nil, nil, lggr)
	require.NoError(t, err)
}
...
func TestGateway_CleanStartAndClose(t *testing.T) {
	...
	lggr := logger.TestLogger(t)
	gateway, err := gateway.NewGatewayFromConfig(parseTOMLConfig(t, buildConfig("")), gateway.NewHandlerFactory(nil, lggr), nil, nil, lggr)
	require.NoError(t, err)
