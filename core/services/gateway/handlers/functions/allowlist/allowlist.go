...
// updateAllowedSendersInBatches will perform the update of allowed senders in batches
// first it will sync the blocked senders by removing every blocked list from the allowed list
// then we will get the current allowlist and start fetching and adding in batches the latest added addresses to give priority to new allowed senders
func (a *onchainAllowlist) updateAllowedSendersInBatches(ctx context.Context, tosContract *functions_allow_list.TermsOfServiceAllowList, blockNum *big.Int) error {
	...
	// we fetch the current allowlist state, given we have already removed the blocked senders,
	// this will be the starting point from which we will be adding the new allowed senders
