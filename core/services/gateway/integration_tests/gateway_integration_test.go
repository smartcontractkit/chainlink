...
func TestIntegration_Gateway_NoFullNodes_BasicConnectionAndMessage(t *testing.T) {
	...
	require.NoError(t, err)
	gateway, err := gateway.NewGatewayFromConfig(parseGatewayConfig(t, gatewayConfig), gateway.NewHandlerFactory(nil, nil, c, lggr), gateway.HandlerTypeForMethod, lggr)
	require.NoError(t, err)
