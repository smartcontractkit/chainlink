...
func NewApplication(config *orm.Config, ethClient eth.Client, advisoryLocker postgres.AdvisoryLocker, keyStoreGenerator strpkg.KeyStoreGenerator, externalInitiatorManager ExternalInitiatorManager, onConnectCallbacks ...func(Application)) (Application, error) {
	...
	headTrackerAddressChannel := make(chan common.Address, 100)
	...
	logBroadcaster := log.NewBroadcaster(log.NewORM(store.DB), ethClient, store.Config, headTrackerAddressChannel)
	...
	app.HeadTracker = services.NewHeadTracker(headTrackerLogger, store, headTrackerAddressChannel, headTrackables)
