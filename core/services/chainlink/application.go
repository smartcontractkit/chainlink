...
func NewApplication(opts ApplicationOpts) (Application, error) {
	...
	var (
		...
		pipelineRunner = pipeline.NewRunner(pipelineORM, bridgeORM, cfg.JobPipeline(), cfg.WebServer(), legacyEVMChains, keyStore.Eth(), keyStore.VRF(), globalLogger, restrictedHTTPClient, unrestrictedHTTPClient, relayerChainInterops.loopRelayers)
