	...
	if ds.feedID == ds.linkFeedID {
		...
		go func() {
			...
			if obs.LinkPrice.Val == nil && obs.LinkPrice.Err == nil {
				ds.lggr.Warnw(fmt.Sprintf("Mercury server was missing LINK feed, using sentinel value of %s", relaymercuryv2.MISSING_PRICE), "linkFeedID", ds.linkFeedID)
				obs.LinkPrice.Val = relaymercuryv2.MISSING_PRICE
			}
		}()
	}
	...
	if ds.feedID == ds.nativeFeedID {
		...
		go func() {
			...
			if obs.NativePrice.Val == nil && obs.NativePrice.Err == nil {
				ds.lggr.Warnw(fmt.Sprintf("Mercury server was missing native feed, using sentinel value of %s", relaymercuryv2.MISSING_PRICE), "nativeFeedID", ds.nativeFeedID)
				obs.NativePrice.Val = relaymercuryv2.MISSING_PRICE
			}
		}()
	}
