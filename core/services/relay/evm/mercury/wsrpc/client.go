...
func NewClient(lggr logger.Logger, privKey csakey.KeyV2, serverPubKey []byte, serverHost string) Client {
	return &client{
		csaKey:       privKey,
...
func (w *client) Start(_ context.Context) error {
	return w.StartOnce("WSRPC Client", func() error {
		...
		// with error.
		conn, err := wsrpc.DialWithContext(context.Background(), w.serverHost,
