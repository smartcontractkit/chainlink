...
func TestCodec(t *testing.T) {
	...
	RunCodecInterfaceTests(t, looptestutils.WrapCodecTesterForLoop[*testing.T](tester))
...
func encodeFieldsOnSliceOrArray(t *testing.T, request *EncodeRequest) []byte {
	...
	case TestItemArray1Type:
		args[0] = [1]chain_reader_example.TestStruct{evmtesting.ToInternalType(request.TestStructs[0])}
	case TestItemArray2Type:
		args[0] = [2]chain_reader_example.TestStruct{
			evmtesting.ToInternalType(request.TestStructs[0]), evmtesting.ToInternalType(request.TestStructs[1])}
	default:
