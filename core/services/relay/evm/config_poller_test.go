...
func TestConfigPoller_WithFeedID(t *testing.T) {
	feedID := utils.NewHash()
	feedIDBytes := [32]byte(feedID)
	fmt.Println(feedIDBytes, feedID, common.Hash(feedIDBytes))
	key, err := crypto.GenerateKey()
	require.NoError(t, err)
	user, err := bind.NewKeyedTransactorWithChainID(key, big.NewInt(1337))
	require.NoError(t, err)
	...
	require.NoError(t, lp.Start(ctx))
	t.Cleanup(func() { lp.Close() })
	logPoller, err := NewConfigPoller(lggr, lp, verifierAddress, WithFeedId(&feedID))
	require.NoError(t, err)
	// Should have no config to begin with.
	_, config, err := logPoller.LatestConfigDetails(testutils.Context(t))
	require.NoError(t, err)
