...
func NewRelayer(ctx context.Context, lggr logger.Logger, chain legacyevm.Chain, opts RelayerOpts) (*Relayer, error) {
	...
	if fromAddress == nil {
		// Neither fromAddress nor fromAddressIndex is specified, return without registering write target
		if wCfg.FromAddressIndex() == nil {
			return relayer, nil
		}
		...
		fromAddress = &ethKeys[fromAddressIndex].EIP55Address
	}
