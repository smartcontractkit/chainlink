...
func TestKeeperEthIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			regAddr, _, registryContract, err := keeper_registry_wrapper.DeployKeeperRegistry(
				steve, backend, linkAddr, linkFeedAddr, gasFeedAddr, 250_000_000, 0, big.NewInt(1),
				20_000_000, big.NewInt(3600), 1, big.NewInt(60000000000), big.NewInt(20000000000000000),
			)
			require.NoError(t, err)
