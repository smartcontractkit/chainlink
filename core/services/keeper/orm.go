...
WHERE
    ...
    ((
                keeper_registries.keeper_index = ABS((least_significant(uint256_to_bit(upkeep_registrations.upkeep_id), 32) #
                                                   CAST($4 AS bit(32)))::integer % keeper_registries.num_keepers)
            AND
                (
   ...
   OR
    (
                    ...
                    ABS((least_significant(uint256_to_bit(upkeep_registrations.upkeep_id), 32) #
                      CAST($4 AS bit(32)))::integer % keeper_registries.num_keepers)
            AND
