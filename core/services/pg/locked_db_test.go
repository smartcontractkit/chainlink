...
func TestLockedDB_HappyPath(t *testing.T) {
	...
	ldb := pg.NewLockedDB(config, config.Database().Lock(), lggr)
...
func TestLockedDB_TwoInstances(t *testing.T) {
	...
	ldb1 := pg.NewLockedDB(config, config.Database().Lock(), lggr)
	...
	defer cancel()
	ldb2 := pg.NewLockedDB(config, config.Database().Lock(), lggr)
	err = ldb2.Open(ctx)
	require.Error(t, err)
}
