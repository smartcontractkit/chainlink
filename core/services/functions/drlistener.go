...
import (
	...
	"github.com/smartcontractkit/chainlink/core/gethwrappers/generated/functions_don"
...
type DRListener struct {
	utils.StartStopOnce
	oracle            *functions_don.FunctionsDON
	job               job.Job
	pipelineRunner    pipeline.Runner
	jobORM            job.ORM
	logBroadcaster    log.Broadcaster
	shutdownWaitGroup sync.WaitGroup
	mbDONEvents    *utils.Mailbox[log.Broadcast]
...
func NewDRListener(oracle *functions_don.FunctionsDON, jb job.Job, runner pipeline.Runner, jobORM job.ORM, pluginORM ORM, pluginConfig config.PluginConfig, logBroadcaster log.Broadcaster, lggr logger.Logger, mailMon *utils.MailboxMonitor) *DRListener {
	return &DRListener{
		oracle:         oracle,
		job:            jb,
		...
		mbDONEvents: utils.NewHighCapacityMailbox[log.Broadcast](),
	...
	return l.StartOnce("DirectRequestListener", func() error {
		...
		unsubscribeLogs := l.logBroadcaster.Register(l, log.ListenerOpts{
			...
			LogsWithTopics: map[common.Hash][][]log.Topic{
				functions_don.FunctionsDONDONRequest{}.Topic():        {},
				functions_don.FunctionsDONDONResponse{}.Topic():       {},
				functions_don.FunctionsDONUserCallbackError{}.Topic():    {},
				functions_don.FunctionsDONUserCallbackRawError{}.Topic(): {},
			},
			MinIncomingConfirmations: l.pluginConfig.MinIncomingConfirmations,
		})
		l.shutdownWaitGroup.Add(3)
		go l.processDONEvents()
		...
		l.mailMon.Monitor(l.mbDONEvents, "DirectRequestListener", "DONEvents", fmt.Sprint(l.job.ID))
...
func (l *DRListener) HandleLog(lb log.Broadcast) {
	...
	switch log := log.(type) {
	case *functions_don.FunctionsDONDONRequest, *functions_don.FunctionsDONDONResponse, *functions_don.FunctionsDONUserCallbackError, *functions_don.FunctionsDONUserCallbackRawError:
		wasOverCapacity := l.mbDONEvents.Deliver(lb)
		if wasOverCapacity {
			l.logger.Error("DONRequest log mailbox is over capacity - dropped the oldest log")
		}
	default:
...
func (l *DRListener) processDONEvents() {
	defer l.shutdownWaitGroup.Done()
	for {
		select {
		case <-l.chStop:
			return
		case <-l.mbDONEvents.Notify():
			for {
				select {
				case <-l.chStop:
					return
				...
				lb, exists := l.mbDONEvents.Retrieve()
				if !exists {
					break
				}
				...
				log := lb.DecodedLog()
				if log == nil || reflect.ValueOf(log).IsNil() {
					l.logger.Error("processDONEvents: ignoring nil value")
					continue
				}

				switch log := log.(type) {
				case *functions_don.FunctionsDONDONRequest:
					l.shutdownWaitGroup.Add(1)
					go l.handleDONRequest(log, lb)
				case *functions_don.FunctionsDONDONResponse:
					l.shutdownWaitGroup.Add(1)
					go l.handleDONResponse("DONResponse", log.RequestId, lb)
				case *functions_don.FunctionsDONUserCallbackError:
					l.shutdownWaitGroup.Add(1)
					go l.handleDONResponse("UserCallbackError", log.RequestId, lb)
				case *functions_don.FunctionsDONUserCallbackRawError:
					l.shutdownWaitGroup.Add(1)
					go l.handleDONResponse("UserCallbackRawError", log.RequestId, lb)
				default:
...
func (l *DRListener) handleDONRequest(request *functions_don.FunctionsDONDONRequest, lb log.Broadcast) {
	defer l.shutdownWaitGroup.Done()
	l.logger.Infow("DON request received",
...
func (l *DRListener) handleDONResponse(responseType string, requestID [32]byte, lb log.Broadcast) {
	defer l.shutdownWaitGroup.Done()
