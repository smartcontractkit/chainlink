...
type DRListenerUniverse struct {
	...
	service        *functions_service.DRListener
...
func NewDRListenerUniverse(t *testing.T, timeoutSec int) *DRListenerUniverse {
	...
	return &DRListenerUniverse{
		runner:         runner,
		service:        service.(*functions_service.DRListener),
		jobORM:         jobORM,
		pluginORM:      pluginORM,
...
func PrepareAndStartDRListener(t *testing.T) (*DRListenerUniverse, *log_mocks.Broadcast, cltest.Awaiter) {
	...
	logOracleRequest := functions_don.FunctionsDONDONRequest{
		RequestId: RequestID,
		Data:      []byte("data"),
	}
...
func TestDRListener_RequestTimeout(t *testing.T) {
	...
	uni.logBroadcaster.On("Register", mock.Anything, mock.Anything).Return(func() {})
	uni.pluginORM.On("TimeoutExpiredResults", mock.Anything, uint32(1)).Return([]functions_service.RequestID{reqId}, nil).Run(func(args mock.Arguments) {
		done <- true
	})
