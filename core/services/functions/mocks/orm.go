...
func (_m *ORM) SetError(requestID functions.RequestID, runID int64, errorType functions.ErrType, computationError []byte, readyAt time.Time, qopts ...pg.QOpt) error {
	_va := make([]interface{}, len(qopts))
	for _i := range qopts {
		_va[_i] = qopts[_i]
	}
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(functions.RequestID, int64, functions.ErrType, []byte, time.Time, ...pg.QOpt) error); ok {
