...
func getSecureMintJobSpec(t *testing.T, ocrContractAddress, keyBundleID string, publicKey ed25519.PublicKey, bridgeName string, forwarderAddress common.Address) string {
	...
	return fmt.Sprintf(`
		...
		getKeystoneForwarderRelayConfig(), // contract writer relay config
	)
}

func getKeystoneForwarderRelayConfig() string {
	chainReaderSpec := `
	[relayConfig.contracts.keystoneforwarder]
	contractABI = '''
	...
	[relayConfig.contracts.keystoneforwarder.configs]
