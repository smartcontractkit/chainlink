...
func NewEthTxManager(ethClient *EthClient, config *orm.Config, keyStore *KeyStore, orm *orm.ORM) *EthTxManager {
	return &EthTxManager{
		EthClient:     ethClient,
		config:        config,
		keyStore:      keyStore,
...
func normalizeGasParams(gasPriceWei *big.Int, gasLimit uint64, config *orm.Config) (*big.Int, uint64) {
	if !config.Dev() {
