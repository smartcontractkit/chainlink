	...
	go func() {
		...
		for {
			...
			case <-txm.chUnsentTxs:
				// FIXME: What is this commented code about?
...
func (txm *EthTxManager) CreateTxWithEth(from, to common.Address, value *assets.Eth) (*models.Tx, error) {
	...
	tx, err := txm.createTx(null.String{}, ma, to, []byte{}, txm.config.EthGasPriceDefault(), txm.config.EthGasLimitDefault(), value)
	if err != nil {
		return nil, err
	}
...
func (txm *EthTxManager) bumpGas(tx *models.Tx, attemptIndex int, blockHeight uint64) error {
	...
	for {
		...
		if bumpedGasPrice.Cmp(txm.config.EthMaxGasPriceWei()) > 0 {
			// Fatal error here means we error the job run and abandon all
			// further attempts to bump gas since they are guaranteed to fail
			// anyway
			promGasBumpExceedsLimit.Inc()
			msg := fmt.Sprintf("bumped gas price of %v would exceed maximum configured limit of %v, set by ETH_MAX_GAS_PRICE_WEI", bumpedGasPrice, txm.config.EthMaxGasPriceWei())
			logger.Error(msg)
