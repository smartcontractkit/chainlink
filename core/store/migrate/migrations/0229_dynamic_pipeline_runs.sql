...
CREATE TABLE job_pipeline_specs (
    ...
    CONSTRAINT fk_pipeline_spec FOREIGN KEY (pipeline_spec_id) REFERENCES pipeline_specs(id) ON DELETE CASCADE
);

CREATE UNIQUE INDEX idx_unique_primary_per_job ON job_pipeline_specs(job_id) WHERE is_primary;
