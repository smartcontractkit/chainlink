...
// The second unique partial index ensures we cannot have multiple jobs names with the same
// name but different, not null deleted_at timestamps.
func Migrate(tx *gorm.DB) error {
	return tx.Exec(`
        ...
        CREATE UNIQUE INDEX job_specs_name_index_deleted on job_specs (LOWER(name)) where deleted_at is NOT NULL;
    `).Error
}
