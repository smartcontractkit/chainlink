...
func TestMigrate_CreateWebhookTables(t *testing.T) {
	...
	require.NoError(t, migrations.MigrateUp(orm.DB, "0027_add_webhook_spec_tables"))
	...
	require.NoError(t, orm.DB.Create(&cs).Error)
	require.NoError(t, orm.DB.Find(&cs).Error)
	require.NoError(t, migrations.MigrateDownFrom(orm.DB, "0027_add_webhook_spec_tables"))
}
