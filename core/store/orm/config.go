...
func init() {
	...
	optimism := ChainSpecificDefaultSet{
		...
		HeadTimeBudget:                   1 * time.Second, // Actually heads on Optimism happen every time a transaction is sent so it could be much more frequent than this. Will need to observe in practice how rapid they are and maybe implement special casing
