...
func TestStore_SyncDiskKeyStoreToDB_HappyPath(t *testing.T) {
	...
	files, err := utils.FilesInDir(app.Configger.KeysDir())
	require.NoError(t, err)
	...
	content, err := utils.FileContents(filepath.Join(app.Configger.KeysDir(), files[0]))
	require.NoError(t, err)
...
func TestStore_SyncDiskKeyStoreToDB_MultipleKeys(t *testing.T) {
	...
	for _, f := range files {
		path := filepath.Join(app.Configger.KeysDir(), f)
