...
// OkayToStart checks if the state may be started, and if so, it transitions to
// started.
func (once *StartStopOnce) OkayToStart() (ok bool) {
	ok = once.state.CAS(int32(StartStopOnce_Unstarted), int32(StartStopOnce_Started))
	return
}

// OkayToStop checks if the state may be stopped, and if so, it transitions to
// stopped.
func (once *StartStopOnce) OkayToStop() (ok bool) {
	ok = once.state.CAS(int32(StartStopOnce_Started), int32(StartStopOnce_Stopped))
	return ok
}
