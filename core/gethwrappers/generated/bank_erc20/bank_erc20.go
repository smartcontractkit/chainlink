...
func DeployBankERC20(auth *bind.TransactOpts, backend bind.ContractBackend, name string, symbol string, totalSupply *big.Int, forwarder common.Address, ccipRouter common.Address, ccipFeeProvider common.Address, ccipChainId uint64) (common.Address, *types.Transaction, *BankERC20, error) {
	...
	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(BankERC20Bin), backend, name, symbol, totalSupply, forwarder, ccipRouter, ccipFeeProvider, ccipChainId)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
