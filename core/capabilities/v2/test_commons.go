...
func (tt *TestTopology) IDsToNodesMaker(triggerCapID string) map[p2ptypes.PeerID]capabilities_registry_v2.INodeInfoProviderNodeInfo {
	IDsToNodes := map[p2ptypes.PeerID]capabilities_registry_v2.INodeInfoProviderNodeInfo{}
	for i := range tt.capabilityDonNodes {
		IDsToNodes[tt.capabilityDonNodes[i]] = capabilities_registry_v2.INodeInfoProviderNodeInfo{
			NodeOperatorId:      1,
			Signer:              RandomUTF8BytesWord(),
			P2pId:               tt.capabilityDonNodes[i],
			EncryptionPublicKey: RandomUTF8BytesWord(),
			CapabilityIds:       []string{triggerCapID},
			CapabilitiesDONIds:  nil,
		}
	}
	for i := range tt.workflowDonNodes {
		IDsToNodes[tt.workflowDonNodes[i]] = capabilities_registry_v2.INodeInfoProviderNodeInfo{
			NodeOperatorId:      1,
			Signer:              RandomUTF8BytesWord(),
