...
func (c *Compute) initModule(id string, cfg *host.ModuleConfig, binary []byte, requestMetadata capabilities.RequestMetadata) (*module, error) {
	...
	cfg.MaxFetchResponseSizeBytes = c.maxFetchResponseSizeBytes
...
func (c *Config) ApplyDefaults() {
	...
	if c.MaxFetchResponseSizeBytes == 0 {
		c.MaxFetchResponseSizeBytes = uint64(defaultMaxFetchResponseSizeBytes)
	}
}
...
func NewAction(
	...
	var (
		...
		compute = &Compute{
			...
			maxFetchResponseSizeBytes: config.MaxFetchResponseSizeBytes,
		}
	)
