...
func TestSyncer_WiresUpExternalCapabilities(t *testing.T) {
	...
	dispatcher.On("SetReceiver", fullTriggerCapID, fmt.Sprint(donID), mock.AnythingOfType("*remote.triggerPublisher")).Return(nil)
	dispatcher.On("SetReceiver", fullTargetID, fmt.Sprint(donID), mock.AnythingOfType("*target.server")).Return(nil)
