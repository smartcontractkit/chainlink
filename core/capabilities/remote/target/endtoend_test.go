...
func testRemoteTarget(t *testing.T, ctx context.Context, underlying commoncap.TargetCapability, numWorkflowPeers int, workflowDonF uint8, workflowNodeTimeout time.Duration,
	...
	for i := 0; i < numWorkflowPeers; i++ {
		...
		workflowNode := target.NewRemoteTargetCaller(ctx, lggr, capInfo, workflowDonInfo, workflowPeerDispatcher, workflowNodeTimeout)
...
func (r *testMessageBroker) Send(msg *remotetypes.MessageBody) {
	...
	if receiver, ok := r.nodes[receiverId]; ok {
