...
// TestCapability_CreateSecrets_RejectsOversizedBlobPayload confirms the CRIT-1 ingress
// fix: a request whose queued representation (StoredPendingQueueItem carrying the stamped
// request ID) exceeds VaultMaxBlobPayloadSizeLimit is rejected with a deterministic
// user-facing error before entering the local queue, instead of fanning out to nodes that
// could never advertise it. EncryptedValue is hex on the wire (2x its decoded size), so a
...
func TestCapability_CreateSecrets_RejectsOversizedBlobPayload(t *testing.T) {
	...
	var label [32]byte
	copy(label[12:], ownerAddr.Bytes())

	// Plaintext sized so the marshaled ciphertext sits at the ingress cap (2KB decoded).
	limit := int(cresettings.Default.PerOwner.VaultCiphertextSizeLimit.DefaultValue)
	probe, err := tdh2easy.EncryptWithLabel(pk, make([]byte, limit), label)
	require.NoError(t, err)
	...
	cipherBytes, err := cipher.Marshal()
	require.NoError(t, err)
	require.LessOrEqual(t, len(cipherBytes), limit)
	cipherHex := hex.EncodeToString(cipherBytes)

	requestID := owner + "::" + "test-request-id"
	secrets := make([]*vault.EncryptedSecret, vaulttypes.MaxBatchSize)
	for i := range secrets {
		secrets[i] = &vault.EncryptedSecret{
			Id: &vault.SecretIdentifier{
