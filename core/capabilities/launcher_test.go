...
func TestLauncher_OnNewRegistry_CallsLocalCapabilityManagerReconcile(t *testing.T) {
	...
	mockLCM := &mockLocalCapabilityManager{
		reconcileFn: func(ctx context.Context, dons []registrysyncer.DON, lr *registrysyncer.LocalRegistry) error {
			assert.Len(t, dons, 1, "should pass one capability DON")
			assert.Equal(t, capDonID, dons[0].ID)
			assert.Equal(t, localRegistry, lr)
...
func (m *mockLocalCapabilityManager) Reconcile(ctx context.Context, dons []registrysyncer.DON, lr *registrysyncer.LocalRegistry) error {
	if m.reconcileFn != nil {
		return m.reconcileFn(ctx, dons, lr)
	}
	return nil
}
