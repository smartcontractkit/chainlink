...
func bindReader(ctx context.Context,
	...
	capRegistryBoundContract := types.BoundContract{
		Address: capRegAddress,
		Name:    consts.ContractNameCapabilitiesRegistry,
	}
	err = reader.Bind(ctx, []types.BoundContract{capRegistryBoundContract})
	if err != nil {
		return nil, types.BoundContract{}, fmt.Errorf("failed to bind home chain contract reader: %w", err)
	}
	...
	err = reader.GetLatestValue(ctx, capRegistryBoundContract.ReadIdentifier(consts.MethodNameGetCapability), primitives.Unconfirmed, map[string]any{
		"hashedId": hid,
	}, &ccipCapabilityInfo)
	if err != nil {
