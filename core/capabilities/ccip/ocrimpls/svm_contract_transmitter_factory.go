...
// SVMContractTransmitterFactory implements TransmitterFactory for Solana-based chains.
...
func (f *SVMContractTransmitterFactory) NewCommitTransmitter(
	...
	//Returns a ToCalldataFunc that is used to generate the calldata for the commit method.
...
func (f *SVMContractTransmitterFactory) NewExecTransmitter(
	...
	//used to generate the calldata for the execute method.
	toCalldataFunc := func(rawReportCtx [2][32]byte,
		report ocr3types.ReportWithInfo[[]byte],
		_, _ [][32]byte,
		_ [32]byte,
	) (contract string, method string, args any, err error) {
