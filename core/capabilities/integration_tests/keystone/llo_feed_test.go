...
func Test_runLLOWorkflow(t *testing.T) {
	...
	triggerSink := framework.NewTriggerSink("streams-trigger:don_16nodes", "2.0.0") // note the label {"don": "16nodes"} to ensure that we can support labelled capabilities; it must match the llo wf spec in [workflow.go]. the label nor the value are important for this test
