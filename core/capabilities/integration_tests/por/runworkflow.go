...
func RunWorkflow(t *testing.T) {

	wasmFileName := os.Getenv("WASM_FILE_NAME")
	if wasmFileName == "" {
		t.Fatalf("environment variable WASM_FILE_NAME is not set")
	}
	...
	wasmFile := filepath.Join(fetchTrueUSDPath, wasmFileName)
