...
type APIInitializer interface {
	...
	Initialize(um sessions.UserManager, lggr logger.Logger) (sessions.User, error)
}
...
func (t *promptingAPIInitializer) Initialize(um sessions.UserManager, lggr logger.Logger) (sessions.User, error) {
	// Load list of users to determine which to assume, or if a user needs to be created
	dbUsers, err := um.LocalAdminListUsers()
	if err != nil {
	...
	if len(dbUsers) == 0 {
		...
		for {
			...
			if err = um.LocalAdminCreateUser(&user); err != nil {
	...
	user, err := um.LocalAdminFindUser(email)
...
func (f fileAPIInitializer) Initialize(um sessions.UserManager, lggr logger.Logger) (sessions.User, error) {
	...
	if len(dbUsers) == 0 {
		...
		return user, um.LocalAdminCreateUser(&user)
	}
	...
	user, err := um.LocalAdminFindUser(request.Email)
	if err != nil {
		return sessions.User{}, err
	}
	return user, nil
