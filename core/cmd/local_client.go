...
func (cli *Client) PrepareTestDatabaseUserOnly(c *clipkg.Context) error {
	...
	cfg := cli.Config
	if err := insertFixtures(cfg.URL(), "../store/fixtures/users_only_fixtures.sql"); err != nil {
		return cli.errorOut(err)
	}
	return nil
}
...
func newConnection(cfg dbConfig) (*sqlx.DB, error) {
	...
	return pg.NewConnection(parsed.String(), cfg.GetDialectConfiguredOrDefault(), cfg)
}
