...
func (cli *Client) RebroadcastTransactions(c *clipkg.Context) (err error) {
	...
	txBuilder := txmgr.NewEvmAttemptBuilder(*ethClient.ChainID(), chain.Config(), keyStore.Eth())
	ec := txmgr.NewEthConfirmer(orm, ethClient, chain.Config(), keyStore.Eth(), keyStates, nil, nil, txBuilder, chain.Logger())
