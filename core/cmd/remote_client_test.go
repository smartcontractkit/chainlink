...
func TestClient_IndexJobSpecs(t *testing.T) {
	...
	j1 := cltest.NewJob()
	assert.NoError(t, app.Store.CreateJob(&j1))
	j2 := cltest.NewJob()
	assert.NoError(t, app.Store.CreateJob(&j2))
...
func TestClient_ShowJobRun_Exists(t *testing.T) {
	...
	assert.NoError(t, set.Parse([]string{jr.ID.String()}))
...
func TestClient_ShowJobRun_NotFound(t *testing.T) {
	...
	set := flag.NewFlagSet("test", 0)
	assert.NoError(t, set.Parse([]string{"bogus-ID"}))
	c := cli.NewContext(nil, set, nil)
...
func TestClient_ShowJobSpec_Exists(t *testing.T) {
	...
	job := cltest.NewJob()
	assert.NoError(t, app.Store.CreateJob(&job))

	client, r := app.NewClientAndRenderer()

	set := flag.NewFlagSet("test", 0)
	assert.NoError(t, set.Parse([]string{job.ID.String()}))
	c := cli.NewContext(nil, set, nil)
...
func TestClient_DestroyExternalInitiator(t *testing.T) {
	...
	set := flag.NewFlagSet("test", 0)
	assert.NoError(t, set.Parse([]string{exi.Name}))
	c := cli.NewContext(nil, set, nil)
...
func TestClient_CreateJobSpec_JSONAPIErrors(t *testing.T) {
	...
	set := flag.NewFlagSet("create", 0)
	assert.NoError(t, set.Parse([]string{`{"initiators":[{"type":"runAt"}],"tasks":[{"type":"NoOp"}]}`}))
	c := cli.NewContext(nil, set, nil)
...
func TestClient_CreateBridge(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			assert.NoError(t, set.Parse([]string{test.param}))
			c := cli.NewContext(nil, set, nil)
			if test.errored {
...
func TestClient_ShowBridge(t *testing.T) {
	...
	set := flag.NewFlagSet("test", 0)
	assert.NoError(t, set.Parse([]string{bt.Name.String()}))
	c := cli.NewContext(nil, set, nil)
...
func TestClient_WithdrawSuccess(t *testing.T) {
	...
	assert.NoError(t, set.Parse([]string{"0x342156c8d3bA54Abc67920d35ba1d1e67201aC9C", "1"}))
...
func TestClient_SendEther(t *testing.T) {
	...
	assert.NoError(t, set.Parse([]string{"100", "0x342156c8d3bA54Abc67920d35ba1d1e67201aC9C"}))
...
func TestClient_ShowTransaction(t *testing.T) {
	...
	assert.NoError(t, set.Parse([]string{tx.Hash.Hex()}))
	c := cli.NewContext(nil, set, nil)
...
func TestClient_SetMinimumGasPrice(t *testing.T) {
	...
	assert.NoError(t, set.Parse([]string{"8616460799"}))
	...
	set.Bool("gwei", true, "")
	assert.NoError(t, set.Parse([]string{"-gwei", "861.6460799"}))
...
func TestClient_CancelJobRun(t *testing.T) {
	...
	assert.NoError(t, set.Parse([]string{run.ID.String()}))
	c := cli.NewContext(nil, set, nil)
