...
func NewORM(chainID *big.Int, db *evmdb.ScopedDB, lggr logger.Logger, cfg pg.QConfig) *ORM {
	namedLogger := lggr.Named("Configs")
	q := pg.NewQ(db.SqlxDB(), namedLogger, cfg)
	return &ORM{
		chainID: chainID,
		q:       q,
	}
...
func (o *ORM) InsertBlock(h common.Hash, n int64, t time.Time, qopts ...pg.QOpt) error {
	...
	err := q.ExecQ(`INSERT INTO log_poller_blocks (evm_chain_id, block_hash, block_number, block_timestamp, created_at) 
		...
		SELECT * FROM logs WHERE (block_number, address, event_sig) IN (
			SELECT MAX(block_number), address, event_sig FROM logs 
				WHERE evm_chain_id = $1 AND
					...
					address = ANY($3) AND
		   			block_number > $4 AND
					block_number <= (SELECT COALESCE(block_number, 0) FROM log_poller_blocks WHERE evm_chain_id = $1 ORDER BY block_number DESC LIMIT 1) - $5
...
func (o *ORM) SelectLatestBlockNumberEventSigsAddrsWithConfs(fromBlock int64, eventSigs []common.Hash, addresses []common.Address, confs int, qopts ...pg.QOpt) (int64, error) {
	...
	err := q.Get(&blockNumber, `
			SELECT COALESCE(MAX(block_number), 0) FROM logs 
				WHERE evm_chain_id = $1 AND
				    event_sig = ANY($2) AND
					address = ANY($3) AND
					block_number > $4 AND
...
func (o *ORM) SelectDataWordRange(address common.Address, eventSig common.Hash, wordIndex int, wordValueMin, wordValueMax common.Hash, confs int, qopts ...pg.QOpt) ([]Log, error) {
	...
	err := q.Select(&logs,
		`SELECT * FROM logs 
			...
			AND block_number <= (SELECT COALESCE(block_number, 0) FROM log_poller_blocks WHERE evm_chain_id = $1 ORDER BY block_number DESC LIMIT 1) - $7
...
func (o *ORM) SelectDataWordGreaterThan(address common.Address, eventSig common.Hash, wordIndex int, wordValueMin common.Hash, confs int, qopts ...pg.QOpt) ([]Log, error) {
	...
	err := q.Select(&logs,
		`SELECT * FROM logs 
			...
			AND address = $2 AND event_sig = $3
			AND substring(data from 32*$4+1 for 32) >= $5
			AND block_number <= (SELECT COALESCE(block_number, 0) FROM log_poller_blocks WHERE evm_chain_id = $1 ORDER BY block_number DESC LIMIT 1) - $6
...
func (o *ORM) SelectIndexedLogs(address common.Address, eventSig common.Hash, topicIndex int, topicValues []common.Hash, confs int, qopts ...pg.QOpt) ([]Log, error) {
	...
	err := q.Select(&logs, `
		SELECT * FROM logs 
			...
			AND topics[$4] = ANY($5)
			AND block_number <= (SELECT COALESCE(block_number, 0) FROM log_poller_blocks WHERE evm_chain_id = $1 ORDER BY block_number DESC LIMIT 1) - $6
			ORDER BY (logs.block_number, logs.log_index)`, utils.NewBig(o.chainID), address, eventSig.Bytes(), topicIndex+1, topicValuesBytes, confs)
	if err != nil {
		return nil, err
	}
	return logs, nil
...
func (o *ORM) SelectIndexedLogsByBlockRangeFilter(start, end int64, address common.Address, eventSig common.Hash, topicIndex int, topicValues []common.Hash, qopts ...pg.QOpt) ([]Log, error) {
	...
	err := q.Select(&logs, `
		SELECT * FROM logs 
			...
			AND address = $4 AND event_sig = $5
			AND topics[$6] = ANY($7)
			ORDER BY (logs.block_number, logs.log_index)`, start, end, utils.NewBig(o.chainID), address, eventSig.Bytes(), topicIndex+1, topicValuesBytes)
	if err != nil {
		return nil, err
	}
	return logs, nil
...
func (o *ORM) SelectIndexedLogsByTxHash(eventSig common.Hash, txHash common.Hash, qopts ...pg.QOpt) ([]Log, error) {
	...
	err := q.Select(&logs, `
		SELECT * FROM logs 
			...
			AND tx_hash = $2
			AND event_sig = $3
			ORDER BY (logs.block_number, logs.log_index)`,
...
func (o *ORM) SelectIndexedLogsWithSigsExcluding(sigA, sigB common.Hash, topicIndex int, address common.Address, startBlock, endBlock int64, confs int, qopts ...pg.QOpt) ([]Log, error) {
	...
	err := q.Select(&logs, `
		...
		AND block_number <= (SELECT COALESCE(block_number, 0) FROM log_poller_blocks WHERE evm_chain_id = $1 ORDER BY block_number DESC LIMIT 1) - $8
		...
		AND		   b.block_number <= (SELECT COALESCE(block_number, 0) FROM log_poller_blocks WHERE evm_chain_id = $1 ORDER BY block_number DESC LIMIT 1) - $8
