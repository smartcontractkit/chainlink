...
func (set chainSpecificConfigDefaultSet) asV2() v2.Chain {
	c := v2.Chain{
		...
		GasEstimator: &v2.GasEstimator{
			...
			LimitOCRJobType:    ptr(uint32(set.gasLimitOCRJobType)),
			LimitDRJobType:     ptr(uint32(set.gasLimitDRJobType)),
			LimitVRFJobType:    ptr(uint32(set.gasLimitVRFJobType)),
			LimitFMJobType:     ptr(uint32(set.gasLimitFMJobType)),
			LimitKeeperJobType: ptr(uint32(set.gasLimitKeeperJobType)),
