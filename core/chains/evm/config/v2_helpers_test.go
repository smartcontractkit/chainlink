...
func (set chainSpecificConfigDefaultSet) asV2() v2.Chain {
	c := v2.Chain{
		...
		GasEstimator: &v2.GasEstimator{
			...
			OCRJobLimit:        ptr(uint32(set.ocrJobGasLimit)),
			DRJobLimit:         ptr(uint32(set.drJobGasLimit)),
			VRFJobLimit:        ptr(uint32(set.vrfJobGasLimit)),
			FMJobLimit:         ptr(uint32(set.fmJobGasLimit)),
			KeeperJobLimit:     ptr(uint32(set.keeperJobGasLimit)),
