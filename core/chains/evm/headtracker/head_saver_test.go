...
func TestHeadSaver_Save(t *testing.T) {
	...
	latest, err := saver.LatestHeadFromMemory(testutils.Context(t))
	require.NoError(t, err)
...
func TestHeadSaver_LoadFromMemory(t *testing.T) {
	t.Parallel()
	...
	latestHead, err := saver.LoadFromMemory(testutils.Context(t))
	require.NoError(t, err)
