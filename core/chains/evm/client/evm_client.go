	...
	for i, node := range nodes {
		if node.SendOnly != nil && *node.SendOnly {
			rpc := NewRPCClient(cfg, lggr, nil, node.HTTPURL.URL(), *node.Name, i, chainID,
				commonclient.Secondary, largePayloadRPCTimeout, defaultRPCTimeout, chainType, clientErrors)
			...
			rpc := NewRPCClient(cfg, lggr, node.WSURL.URL(), node.HTTPURL.URL(), *node.Name, i,
				chainID, commonclient.Primary, largePayloadRPCTimeout, defaultRPCTimeout, chainType, clientErrors)
