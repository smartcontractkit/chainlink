...
func NewEvmClient(cfg evmconfig.NodePool, noNewHeadsThreshold time.Duration, lggr logger.Logger, chainID *big.Int, chainType config.ChainType, nodes []*toml.Node) Client {
	...
	for i, node := range nodes {
		if node.SendOnly != nil && *node.SendOnly {
			...
			sendonly := commonclient.NewSendOnlyNode[*big.Int, RPCClient](lggr, (url.URL)(*node.HTTPURL),
			...
			primaryNode := commonclient.NewNode[*big.Int, *evmtypes.Head, RPCClient](cfg, noNewHeadsThreshold,
