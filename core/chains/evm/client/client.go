...
func NewClientWithNodes(logger logger.Logger, selectionMode string, switchToBestNodeInterval time.Duration, noNewHeadsThreshold time.Duration, primaryNodes []Node, sendOnlyNodes []SendOnlyNode, chainID *big.Int, chainType config.ChainType) (*client, error) {
	pool := NewPool(logger, selectionMode, switchToBestNodeInterval, noNewHeadsThreshold, primaryNodes, sendOnlyNodes, chainID, chainType)
	return &client{
		logger: logger,
		pool:   pool,
	}, nil
