...
func NewChainClientWithTestNode(
	...
	lggr := logger.Test(t)
	rpc := NewRPCClient(lggr, *parsed, rpcHTTPURL, "eth-primary-rpc-0", id, chainID, commonclient.Primary, 0)
	...
	for i, u := range sendonlyRPCURLs {
		...
		var empty url.URL
		rpc := NewRPCClient(lggr, empty, &sendonlyRPCURLs[i], fmt.Sprintf("eth-sendonly-rpc-%d", i), id, chainID, commonclient.Secondary, 0)
