...
func TestRPCClient_SubscribeToHeads(t *testing.T) {
	...
	t.Run("Updates chain info on new blocks", func(t *testing.T) {
		...
		rpc := client.NewRPCClient(nodePoolCfgWSSub, lggr, wsURL, nil, "rpc", 1, chainId, commonclient.Primary, commonclient.QueryTimeout, commonclient.QueryTimeout, "", nil)
		defer rpc.Close()
	...
	t.Run("SubscribeToHeads with http polling enabled will update new heads", func(t *testing.T) {
		...
		rpc := client.NewRPCClient(nodePoolCfgHeadPolling, lggr, server.URL, nil, "rpc", 1, chainId, commonclient.Primary, commonclient.QueryTimeout, commonclient.QueryTimeout, "", nil)
		defer rpc.Close()
		require.NoError(t, rpc.Dial(ctx))
		latest, highestUserObservations := rpc.GetInterceptedChainInfo()
	...
	t.Run("Failed SubscribeToHeads returns and logs proper error", func(t *testing.T) {
		...
		rpc := client.NewRPCClient(nodePoolCfgWSSub, observedLggr, wsURL, nil, "rpc", 1, chainId, commonclient.Primary, commonclient.QueryTimeout, commonclient.QueryTimeout, "", nil)
		require.NoError(t, rpc.Dial(ctx))
		server.Close()
