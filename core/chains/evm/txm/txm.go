...
func (t *Txm) CreateTransaction(ctx context.Context, txRequest *types.TxRequest) (tx *types.Transaction, err error) {
	tx, err = t.storage.CreateTransaction(ctx, txRequest)
	if err == nil {
...
func (t *Txm) broadcastTransaction() error {
	_, unconfirmedCount, err := t.storage.FetchUnconfirmedTransactionAtNonceWithCount(context.TODO(), 0, t.address)
	if err != nil {
		return err
	}
	...
	if unconfirmedCount >= int(maxInFlightTransactions)/3 {
		...
		if t.nonce.Load() > pendingNonce {
			t.lggr.Warnf("Reached transaction limit. LocalNonce: %d, PendingNonce %d, maxInFlightTransactions: %d",
				t.nonce.Load(), pendingNonce, maxInFlightTransactions)
		}
	}
