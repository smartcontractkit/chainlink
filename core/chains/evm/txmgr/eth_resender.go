...
func (er *EthResender) resendUnconfirmed() (merr error) {
	...
	for _, k := range keys {
		go func() {
			...
			if err != nil {
				errs <- errors.Wrap(err, "failed to FindEthTxAttemptsRequiringResend")
				return
			}
			...
			if err != nil {
				errs <- errors.Wrap(err, "failed to re-send transactions")
				return
			}
