...
func TestEthConfirmer_CheckForReceipts_confirmed_missing_receipt_scoped_to_key(t *testing.T) {
	...
	pgtest.MustExec(t, db, `UPDATE evm.eth_txes SET state='confirmed' WHERE id = $1`, etx_other_chain.ID)
...
func TestEthConfirmer_CheckForReceipts_confirmed_missing_receipt(t *testing.T) {
	...
	pgtest.MustExec(t, db, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num = 41 WHERE broadcast_before_block_num IS NULL`)
...
func TestEthConfirmer_FindTxsRequiringRebroadcast(t *testing.T) {
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, tooNew, attempt1_1.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, tooNew, attempt2_1.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt3_1.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attemptOther1.ID))
	...
	attempt4_1 := etx4.TxAttempts[0]
	dbAttempt = txmgr.DbEthTxAttemptFromEthTxAttempt(&attemptOther1)
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt4_1.ID))
	...
	t.Run("ignores pending transactions for another key", func(t *testing.T) {
		...
		require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, aOther.ID))
...
func TestEthConfirmer_RebroadcastWhereNecessary_WithConnectivityCheck(t *testing.T) {
	...
	t.Run("should retry previous attempt if connectivity check failed for legacy transactions", func(t *testing.T) {
		...
		require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt1.ID))
...
func TestEthConfirmer_RebroadcastWhereNecessary(t *testing.T) {
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt1_1.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt1_2.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt1_3.ID))
	...
	pgtest.MustExec(t, db, `UPDATE evm.eth_txes SET state = 'confirmed'`)

	etx2 := cltest.MustInsertUnconfirmedEthTxWithBroadcastLegacyAttempt(t, txStore, nonce, fromAddress)
	nonce++
	attempt2_1 := etx2.TxAttempts[0]
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt2_1.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt2_2.ID))
	...
	nonce++
	attempt3_1 := etx3.TxAttempts[0]
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1, gas_price=$2 WHERE id=$3 RETURNING *`, oldEnough, assets.NewWeiI(35000000000), attempt3_1.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt3_2.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt3_3.ID))
	...
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, attempt3_4.ID))
	...
	attempt4_1 := etx4.TxAttempts[0]
	require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1, gas_tip_cap=$2, gas_fee_cap=$3 WHERE id=$4 RETURNING *`,
...
func TestEthConfirmer_RebroadcastWhereNecessary_TerminallyUnderpriced_ThenGoesThrough(t *testing.T) {
	...
	t.Run("multiple gas bumps with existing broadcast attempts are retried with more gas until success in legacy mode", func(t *testing.T) {
		...
		require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, legacyAttempt.ID))
	...
	t.Run("multiple gas bumps with existing broadcast attempts are retried with more gas until success in EIP-1559 mode", func(t *testing.T) {
		...
		require.NoError(t, db.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET broadcast_before_block_num=$1 WHERE id=$2 RETURNING *`, oldEnough, dxFeeAttempt.ID))
...
func TestEthConfirmer_RebroadcastWhereNecessary_WhenOutOfEth(t *testing.T) {
	...
	t.Run("resubmitting due to insufficient eth is not limited by EVM.GasEstimator.BumpTxDepth", func(t *testing.T) {
		...
		require.NoError(t, db.Select(&dbAttempts, "SELECT * FROM evm.eth_tx_attempts WHERE state = 'insufficient_eth'"))
