...
func TestReaper_ReapTxes(t *testing.T) {
	...
	t.Run("deletes confirmed evm.eth_txes that exceed the age threshold with at least EVM.FinalityDepth blocks above their receipt", func(t *testing.T) {
		config := txmgrmocks.NewReaperConfig(t)
		config.On("FinalityDepth").Return(uint32(10))
		...
		pgtest.MustExec(t, db, `UPDATE evm.eth_txes SET created_at=$1`, oneDayAgo)
	...
	t.Run("deletes errored evm.eth_txes that exceed the age threshold", func(t *testing.T) {
		config := txmgrmocks.NewReaperConfig(t)
		config.On("FinalityDepth").Return(uint32(10))
		...
		require.NoError(t, utils.JustError(db.Exec(`UPDATE evm.eth_txes SET created_at=$1`, oneDayAgo)))
