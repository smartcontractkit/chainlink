...
func TestTxm_NewAttempt_NonRetryableErrors(t *testing.T) {
	...
	t.Run("NewAttemptWithType: mismatch fee + type", func(t *testing.T) {
		t.Run("dynamic fee with legacy tx type", func(t *testing.T) {
			_, err, retryable := cks.NewAttemptWithType(txmgr.EthTx{}, gas.EvmFee{Dynamic: &dynamicFee}, 100, 0x0, lggr)
			require.Error(t, err)
			assert.False(t, retryable)
		})
		t.Run("legacy fee with dynamic tx type", func(t *testing.T) {
			_, err, retryable := cks.NewAttemptWithType(txmgr.EthTx{}, gas.EvmFee{Legacy: legacyFee}, 100, 0x2, lggr)
			require.Error(t, err)
			assert.False(t, retryable)
		})
	})
	...
	t.Run("NewAttemptWithType: invalid type", func(t *testing.T) {
		_, err, retryable := cks.NewAttemptWithType(txmgr.EthTx{}, gas.EvmFee{}, 100, 0xA, lggr)
		require.Error(t, err)
		assert.False(t, retryable)
	})
}
