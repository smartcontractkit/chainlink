...
func (ec *EthConfirmer) CheckConfirmedMissingReceipt(ctx context.Context) (err error) {
	...
	reqs, err := batchSendTransactions(ctx, ec.orm, attempts, int(ec.config.RPCDefaultBatchSize()), ec.lggr, ec.ethClient)
	if err != nil {
...
func (ec *EthConfirmer) CheckForReceipts(ctx context.Context, blockNum int64) error {
	...
	if err := ec.orm.MarkOldTxesMissingReceiptAsErrored(blockNum, ec.config.FinalityDepth(), ec.chainID); err != nil {
