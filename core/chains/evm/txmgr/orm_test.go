...
func TestORM_updateEthTxAttemptUnbroadcast(t *testing.T) {
	t.Parallel()
	...
	t.Run("unbroadcast attempt", func(t *testing.T) {
		etx := cltest.MustInsertConfirmedEthTxWithLegacyAttempt(t, borm, 777, 1, fromAddress)
		attempt := etx.EthTxAttempts[0]
		require.Equal(t, txmgr.EthTxAttemptBroadcast, attempt.State)

		// use exported method
		txmgr.UpdateEthTxAttemptUnbroadcast(db, attempt)

		result, err := borm.FindEthTxAttempt(attempt.Hash)
		require.NoError(t, err)
		assert.Equal(t, txmgr.EthTxAttemptInProgress, result.State)
		assert.Nil(t, result.BroadcastBeforeBlockNum)
	})
}

func TestORM_updateEthTxUnconfirm(t *testing.T) {
	t.Parallel()
	...
	t.Run("unconfirm transaction", func(t *testing.T) {
		etx := cltest.MustInsertConfirmedEthTxWithLegacyAttempt(t, borm, 777, 1, fromAddress)
		assert.Equal(t, txmgr.EthTxConfirmed, etx.State)

		// use exported method
		txmgr.UpdateEthTxUnconfirm(db, etx)

		result, err := borm.FindEthTxWithAttempts(etx.ID)
		require.NoError(t, err)
		assert.Equal(t, txmgr.EthTxUnconfirmed, result.State)
	})
}

func TestORM_deleteEthReceipts(t *testing.T) {
	t.Parallel()
	...
	t.Run("delete all receipts for eth transaction", func(t *testing.T) {
		...
		// use exported method
		txmgr.DeleteEthReceipts(db, etx.ID)

		result, err := borm.FindEthTxWithAttempts(etx.ID)
		require.NoError(t, err)
		assert.Len(t, result.EthTxAttempts[0].EthReceipts, 0)
	})
}
