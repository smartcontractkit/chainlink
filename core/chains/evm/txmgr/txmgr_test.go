...
func TestTxm_SendNativeToken_DoesNotSendToZero(t *testing.T) {
	...
	estimator, err := gas.NewEstimator(logger.Test(t), ethClient, config.ChainType(), evmConfig.GasEstimator(), nil)
...
func TestTxm_Reset(t *testing.T) {
	...
	estimator, err := gas.NewEstimator(logger.Test(t), ethClient, cfg.EVM().ChainType(), cfg.EVM().GasEstimator(), nil)
	require.NoError(t, err)
