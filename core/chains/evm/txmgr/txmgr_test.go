...
func TestTxm_CreateTransaction(t *testing.T) {
	...
	t.Run("with queue under capacity inserts eth_tx", func(t *testing.T) {
		...
		require.NoError(t, db.Get(&dbEtx, `SELECT * FROM evm.eth_txes ORDER BY id ASC LIMIT 1`))
	...
	t.Run("simulate transmit checker", func(t *testing.T) {
		pgtest.MustExec(t, db, `DELETE FROM evm.eth_txes`)
...
func TestTxm_CreateTransaction_OutOfEth(t *testing.T) {
	...
	require.NoError(t, utils.JustError(db.Exec(`DELETE FROM evm.eth_txes WHERE from_address = $1`, thisKey.Address)))
...
func TestTxm_Reset(t *testing.T) {
	...
	t.Run("calls function and deletes relevant evm.eth_txes if abandon=true", func(t *testing.T) {
		f := new(fnMock)
		...
		var s string
		err = db.Get(&s, `SELECT error FROM evm.eth_txes WHERE from_address = $1 AND state = 'fatal_error'`, addr)
		require.NoError(t, err)
		assert.Equal(t, "abandoned", s)

		// the other address didn't get touched
		var count int
		err = db.Get(&count, `SELECT count(*) FROM evm.eth_txes WHERE from_address = $1 AND state = 'fatal_error'`, addr2)
		require.NoError(t, err)
