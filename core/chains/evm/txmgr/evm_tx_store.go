...
// Directly maps to columns of database table "evm.eth_receipts".
...
// Directly maps to columns of database table "evm.eth_txes".
...
// Directly maps to columns of database table "evm.eth_tx_attempts".
// This is exported, as tests and other external code still directly reads DB using this schema.
...
INSERT INTO evm.eth_tx_attempts (eth_tx_id, gas_price, signed_raw_tx, hash, broadcast_before_block_num, state, created_at, chain_specific_gas_limit, tx_type, gas_tip_cap, gas_fee_cap)
...
func (o *evmTxStore) preloadTxAttempts(txs []Tx) error {
	...
	sql := `SELECT * FROM evm.eth_tx_attempts WHERE eth_tx_id IN (?) ORDER BY id desc;`
...
func (o *evmTxStore) PreloadTxes(attempts []TxAttempt, qopts ...pg.QOpt) error {
	...
	if err := qq.Select(&dbEthTxs, `SELECT * FROM evm.eth_txes WHERE id = ANY($1)`, pq.Array(ethTxIDs)); err != nil {
...
func (o *evmTxStore) Transactions(offset, limit int) (txs []Tx, count int, err error) {
	sql := `SELECT count(*) FROM evm.eth_txes WHERE id IN (SELECT DISTINCT eth_tx_id FROM evm.eth_tx_attempts)`
	if err = o.q.Get(&count, sql); err != nil {
		return
	}

	sql = `SELECT * FROM evm.eth_txes WHERE id IN (SELECT DISTINCT eth_tx_id FROM evm.eth_tx_attempts) ORDER BY id desc LIMIT $1 OFFSET $2`
...
func (o *evmTxStore) TxAttempts(offset, limit int) (txs []TxAttempt, count int, err error) {
	sql := `SELECT count(*) FROM evm.eth_tx_attempts`
	if err = o.q.Get(&count, sql); err != nil {
		return
	}

	sql = `SELECT * FROM evm.eth_tx_attempts ORDER BY created_at DESC, id DESC LIMIT $1 OFFSET $2`
...
func (o *evmTxStore) FindTxAttempt(hash common.Hash) (*TxAttempt, error) {
	...
	sql := `SELECT * FROM evm.eth_tx_attempts WHERE hash = $1`
...
func (o *evmTxStore) FindTxAttemptsByTxIDs(ids []int64) ([]TxAttempt, error) {
	sql := `SELECT * FROM evm.eth_tx_attempts WHERE eth_tx_id = ANY($1)`
...
func (o *evmTxStore) FindTxByHash(hash common.Hash) (*Tx, error) {
	...
	err := o.q.Transaction(func(tx pg.Queryer) error {
		sql := `SELECT evm.eth_txes.* FROM evm.eth_txes WHERE id IN (SELECT DISTINCT eth_tx_id FROM evm.eth_tx_attempts WHERE hash = $1)`
...
func (o *evmTxStore) InsertTx(etx *Tx) error {
	...
	const insertEthTxSQL = `INSERT INTO evm.eth_txes (nonce, from_address, to_address, encoded_payload, value, gas_limit, error, broadcast_at, initial_broadcast_at, created_at, state, meta, subject, pipeline_task_run_id, min_confirmations, evm_chain_id, transmit_checker) VALUES (
...
func (o *evmTxStore) InsertReceipt(receipt *evmtypes.Receipt) (int64, error) {
	...
	const insertEthReceiptSQL = `INSERT INTO evm.eth_receipts (tx_hash, block_hash, block_number, transaction_index, receipt, created_at) VALUES (
...
func (o *evmTxStore) FindTxWithAttempts(etxID int64) (etx Tx, err error) {
	err = o.q.Transaction(func(tx pg.Queryer) error {
		var dbEtx DbEthTx
		if err = tx.Get(&dbEtx, `SELECT * FROM evm.eth_txes WHERE id = $1 ORDER BY created_at ASC, id ASC`, etxID); err != nil {
			return pkgerrors.Wrapf(err, "failed to find eth_tx with id %d", etxID)
		}
		DbEthTxToEthTx(dbEtx, &etx)
		if err = o.LoadTxAttempts(&etx, pg.WithQueryer(tx)); err != nil {
			return pkgerrors.Wrapf(err, "failed to load evm.eth_tx_attempts for eth_tx with id %d", etxID)
		}
		if err = loadEthTxAttemptsReceipts(tx, &etx); err != nil {
			return pkgerrors.Wrapf(err, "failed to load evm.eth_receipts for eth_tx with id %d", etxID)
		}
		return nil
	}, pg.OptReadOnlyTx())
...
func (o *evmTxStore) FindTxAttemptConfirmedByTxIDs(ids []int64) ([]TxAttempt, error) {
	...
	err := o.q.Transaction(func(tx pg.Queryer) error {
		...
		FROM evm.eth_tx_attempts eta
			join evm.eth_receipts er on eta.hash = er.tx_hash where eta.eth_tx_id = ANY($1) ORDER BY eta.gas_price DESC, eta.gas_tip_cap DESC`, ids); err != nil {
			return err
		}
...
func (o *evmTxStore) LoadTxesAttempts(etxs []*Tx, qopts ...pg.QOpt) error {
	...
	var dbTxAttempts []DbEthTxAttempt
	if err := qq.Select(&dbTxAttempts, `SELECT * FROM evm.eth_tx_attempts WHERE eth_tx_id = ANY($1) ORDER BY evm.eth_tx_attempts.gas_price DESC, evm.eth_tx_attempts.gas_tip_cap DESC`, pq.Array(ethTxIDs)); err != nil {
		return pkgerrors.Wrap(err, "loadEthTxesAttempts failed to load evm.eth_tx_attempts")
	}
...
func loadEthTxesAttemptsReceipts(q pg.Queryer, etxs []*Tx) (err error) {
	...
	var rs []dbReceipt
	if err = q.Select(&rs, `SELECT * FROM evm.eth_receipts WHERE tx_hash = ANY($1)`, pq.Array(attemptHashes)); err != nil {
		return pkgerrors.Wrap(err, "loadEthTxesAttemptsReceipts failed to load evm.eth_receipts")
	}
...
func loadConfirmedAttemptsReceipts(q pg.Queryer, attempts []TxAttempt) error {
	...
	var rs []dbReceipt
	if err := q.Select(&rs, `SELECT * FROM evm.eth_receipts WHERE tx_hash = ANY($1)`, pq.Array(hashes)); err != nil {
		return pkgerrors.Wrap(err, "loadConfirmedAttemptsReceipts failed to load evm.eth_receipts")
	}
	var receipts []*evmtypes.Receipt = fromDBReceipts(rs)
	for _, receipt := range receipts {
...
func (o *evmTxStore) FindTxAttemptsRequiringResend(olderThan time.Time, maxInFlightTransactions uint32, chainID *big.Int, address common.Address) (attempts []TxAttempt, err error) {
	...
	var dbAttempts []DbEthTxAttempt
	// this select distinct works because of unique index on evm.eth_txes
	// (evm_chain_id, from_address, nonce)
	err = o.q.Select(&dbAttempts, `
SELECT DISTINCT ON (evm.eth_txes.nonce) evm.eth_tx_attempts.*
FROM evm.eth_tx_attempts
JOIN evm.eth_txes ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id AND evm.eth_txes.state IN ('unconfirmed', 'confirmed_missing_receipt')
WHERE evm.eth_tx_attempts.state <> 'in_progress' AND evm.eth_txes.broadcast_at <= $1 AND evm_chain_id = $2 AND from_address = $3
ORDER BY evm.eth_txes.nonce ASC, evm.eth_tx_attempts.gas_price DESC, evm.eth_tx_attempts.gas_tip_cap DESC
LIMIT $4
`, olderThan, chainID.String(), address, limit)

	attempts = dbEthTxAttemptsToEthTxAttempts(dbAttempts)
	return attempts, pkgerrors.Wrap(err, "FindEthTxAttemptsRequiringResend failed to load evm.eth_tx_attempts")
}
...
func (o *evmTxStore) UpdateBroadcastAts(now time.Time, etxIDs []int64) error {
	...
	_, err := o.q.Exec(`UPDATE evm.eth_txes SET broadcast_at = $1 WHERE id = ANY($2) AND broadcast_at < $1`, now, pq.Array(etxIDs))
	return pkgerrors.Wrap(err, "updateBroadcastAts failed to update evm.eth_txes")
}
...
FROM evm.eth_txes
WHERE evm.eth_tx_attempts.broadcast_before_block_num IS NULL AND evm.eth_tx_attempts.state = 'broadcast'
AND evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id AND evm.eth_txes.evm_chain_id = $2`,
...
func (o *evmTxStore) FindTxAttemptsConfirmedMissingReceipt(chainID *big.Int) (attempts []TxAttempt, err error) {
	var dbAttempts []DbEthTxAttempt
	err = o.q.Select(&dbAttempts,
		`SELECT DISTINCT ON (evm.eth_tx_attempts.eth_tx_id) evm.eth_tx_attempts.*
		FROM evm.eth_tx_attempts
		JOIN evm.eth_txes ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id AND evm.eth_txes.state = 'confirmed_missing_receipt'
		WHERE evm_chain_id = $1
		ORDER BY evm.eth_tx_attempts.eth_tx_id ASC, evm.eth_tx_attempts.gas_price DESC, evm.eth_tx_attempts.gas_tip_cap DESC`,
		chainID.String())
	if err != nil {
...
func (o *evmTxStore) UpdateTxsUnconfirmed(ids []int64) error {
	_, err := o.q.Exec(`UPDATE evm.eth_txes SET state='unconfirmed' WHERE id = ANY($1)`, pq.Array(ids))
...
SELECT evm.eth_tx_attempts.* FROM evm.eth_tx_attempts
JOIN evm.eth_txes ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id AND evm.eth_txes.state IN ('unconfirmed', 'confirmed_missing_receipt') AND evm.eth_txes.evm_chain_id = $1
WHERE evm.eth_tx_attempts.state != 'insufficient_eth'
ORDER BY evm.eth_txes.nonce ASC, evm.eth_tx_attempts.gas_price DESC, evm.eth_tx_attempts.gas_tip_cap DESC
`, chainID.String())
		if err != nil {
			return pkgerrors.Wrap(err, "FindEthTxAttemptsRequiringReceiptFetch failed to load evm.eth_tx_attempts")
		}
		attempts = dbEthTxAttemptsToEthTxAttempts(dbAttempts)
		err = o.PreloadTxes(attempts, pg.WithQueryer(tx))
		return pkgerrors.Wrap(err, "FindEthTxAttemptsRequiringReceiptFetch failed to load evm.eth_txes")
	}, pg.OptReadOnlyTx())
	return
}
...
func (o *evmTxStore) SaveFetchedReceipts(r []*evmtypes.Receipt, chainID *big.Int) (err error) {
	...
	WITH inserted_receipts AS (
		INSERT INTO evm.eth_receipts (tx_hash, block_hash, block_number, transaction_index, receipt, created_at)
		VALUES %s
	...
	updated_evm.eth_tx_attempts AS (
		...
		SET
			state = 'broadcast',
			broadcast_before_block_num = COALESCE(evm.eth_tx_attempts.broadcast_before_block_num, inserted_receipts.block_number)
		FROM inserted_receipts
		WHERE inserted_receipts.tx_hash = evm.eth_tx_attempts.hash
		RETURNING evm.eth_tx_attempts.eth_tx_id
	)
	UPDATE evm.eth_txes
	SET state = 'confirmed'
...
// It is possible that we can fail to get a receipt for all evm.eth_tx_attempts
...
// prevent gas bumping.
//
// NOTE: We continue to attempt to resend evm.eth_txes in this state on
...
WHERE state = 'unconfirmed'
	...
	AND evm.eth_txes.from_address = max_table.from_address
	`, chainID.String())
	if err != nil {
...
func (o *evmTxStore) GetInProgressTxAttempts(ctx context.Context, address common.Address, chainID *big.Int) (attempts []TxAttempt, err error) {
	...
	err = qq.Transaction(func(tx pg.Queryer) error {
		...
		err = tx.Select(&dbAttempts, `
SELECT evm.eth_tx_attempts.* FROM evm.eth_tx_attempts
INNER JOIN evm.eth_txes ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id AND evm.eth_txes.state in ('confirmed', 'confirmed_missing_receipt', 'unconfirmed')
WHERE evm.eth_tx_attempts.state = 'in_progress' AND evm.eth_txes.from_address = $1 AND evm.eth_txes.evm_chain_id = $2
`, address, chainID.String())
		if err != nil {
			return pkgerrors.Wrap(err, "getInProgressEthTxAttempts failed to load evm.eth_tx_attempts")
		}
		attempts = dbEthTxAttemptsToEthTxAttempts(dbAttempts)
		err = o.PreloadTxes(attempts, pg.WithQueryer(tx))
		return pkgerrors.Wrap(err, "getInProgressEthTxAttempts failed to load evm.eth_txes")
	}, pg.OptReadOnlyTx())
...
func (o *evmTxStore) FindReceiptsPendingConfirmation(ctx context.Context, blockNum int64, chainID *big.Int) (receiptsPlus []ReceiptPlus, err error) {
	...
	SELECT pipeline_task_runs.id, evm.eth_receipts.receipt, COALESCE((evm.eth_txes.meta->>'FailOnRevert')::boolean, false) "FailOnRevert" FROM pipeline_task_runs
	INNER JOIN pipeline_runs ON pipeline_runs.id = pipeline_task_runs.pipeline_run_id
	INNER JOIN evm.eth_txes ON evm.eth_txes.pipeline_task_run_id = pipeline_task_runs.id
	INNER JOIN evm.eth_tx_attempts ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id
	INNER JOIN evm.eth_receipts ON evm.eth_tx_attempts.hash = evm.eth_receipts.tx_hash
	WHERE pipeline_runs.state = 'suspended' AND evm.eth_receipts.block_number <= ($1 - evm.eth_txes.min_confirmations) AND evm.eth_txes.evm_chain_id = $2
	`, blockNum, chainID.String())
...
func (o *evmTxStore) FindTxWithIdempotencyKey(idempotencyKey string, chainID *big.Int) (etx *Tx, err error) {
	var dbEtx DbEthTx
	err = o.q.Get(&dbEtx, `SELECT * FROM evm.eth_txes WHERE idempotency_key = $1 and evm_chain_id = $2`, idempotencyKey, chainID.String())
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, nil
		}
		return nil, pkgerrors.Wrap(err, "FindTxWithIdempotencyKey failed to load evm.eth_txes")
	}
	etx = new(Tx)
	DbEthTxToEthTx(dbEtx, etx)
	return
...
func (o *evmTxStore) FindTxWithSequence(fromAddress common.Address, nonce evmtypes.Nonce) (etx *Tx, err error) {
	...
	err = o.q.Transaction(func(tx pg.Queryer) error {
		var dbEtx DbEthTx
		err = tx.Get(&dbEtx, `
SELECT * FROM evm.eth_txes WHERE from_address = $1 AND nonce = $2 AND state IN ('confirmed', 'confirmed_missing_receipt', 'unconfirmed')
`, fromAddress, nonce.Int64())
		if err != nil {
			return pkgerrors.Wrap(err, "FindEthTxWithNonce failed to load evm.eth_txes")
		}
		DbEthTxToEthTx(dbEtx, etx)
		err = o.LoadTxAttempts(etx, pg.WithQueryer(tx))
		return pkgerrors.Wrap(err, "FindEthTxWithNonce failed to load evm.eth_tx_attempts")
	}, pg.OptReadOnlyTx())
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
...
func updateEthTxAttemptUnbroadcast(q pg.Queryer, attempt TxAttempt) error {
	...
	_, err := q.Exec(`UPDATE evm.eth_tx_attempts SET broadcast_before_block_num = NULL, state = 'in_progress' WHERE id = $1`, attempt.ID)
...
func updateEthTxUnconfirm(q pg.Queryer, etx Tx) error {
	...
	_, err := q.Exec(`UPDATE evm.eth_txes SET state = 'unconfirmed' WHERE id = $1`, etx.ID)
	return pkgerrors.Wrap(err, "updateEthTxUnconfirm failed")
}

func deleteEthReceipts(q pg.Queryer, etxID int64) (err error) {
	_, err = q.Exec(`
DELETE FROM evm.eth_receipts
USING evm.eth_tx_attempts
WHERE evm.eth_receipts.tx_hash = evm.eth_tx_attempts.hash
AND evm.eth_tx_attempts.eth_tx_id = $1
	`, etxID)
...
SELECT DISTINCT evm.eth_txes.* FROM evm.eth_txes
INNER JOIN evm.eth_tx_attempts ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id AND evm.eth_tx_attempts.state = 'broadcast'
INNER JOIN evm.eth_receipts ON evm.eth_receipts.tx_hash = evm.eth_tx_attempts.hash
WHERE evm.eth_txes.state IN ('confirmed', 'confirmed_missing_receipt') AND block_number BETWEEN $1 AND $2 AND evm_chain_id = $3
ORDER BY nonce ASC
`, lowBlockNumber, highBlockNumber, chainID.String())
		if err != nil {
			return pkgerrors.Wrap(err, "FindTransactionsConfirmedInBlockRange failed to load evm.eth_txes")
		}
		etxs = make([]*Tx, len(dbEtxs))
		dbEthTxsToEvmEthTxPtrs(dbEtxs, etxs)
		if err = o.LoadTxesAttempts(etxs, pg.WithQueryer(tx)); err != nil {
			return pkgerrors.Wrap(err, "FindTransactionsConfirmedInBlockRange failed to load evm.eth_tx_attempts")
		}
		err = loadEthTxesAttemptsReceipts(tx, etxs)
		return pkgerrors.Wrap(err, "FindTransactionsConfirmedInBlockRange failed to load evm.eth_receipts")
	}, pg.OptReadOnlyTx())
...
func saveAttemptWithNewState(q pg.Queryer, timeout time.Duration, logger logger.Logger, attempt TxAttempt, broadcastAt time.Time) error {
	...
	return pg.SqlxTransaction(ctx, q, logger, func(tx pg.Queryer) error {
		...
		if _, err := tx.Exec(`UPDATE evm.eth_txes SET broadcast_at = $1 WHERE id = $2 AND broadcast_at < $1`, broadcastAt, dbAttempt.EthTxID); err != nil {
			return pkgerrors.Wrap(err, "saveAttemptWithNewState failed to update evm.eth_txes")
		}
		_, err := tx.Exec(`UPDATE evm.eth_tx_attempts SET state=$1 WHERE id=$2`, dbAttempt.State, dbAttempt.ID)
		return pkgerrors.Wrap(err, "saveAttemptWithNewState failed to update evm.eth_tx_attempts")
	})
}
...
func (o *evmTxStore) SaveConfirmedMissingReceiptAttempt(ctx context.Context, timeout time.Duration, attempt *TxAttempt, broadcastAt time.Time) error {
	...
	err := qq.Transaction(func(tx pg.Queryer) error {
		...
		if _, err := tx.Exec(`UPDATE evm.eth_txes SET state = 'confirmed_missing_receipt' WHERE id = $1`, attempt.TxID); err != nil {
			return pkgerrors.Wrap(err, "failed to update evm.eth_txes")
...
func (o *evmTxStore) DeleteInProgressAttempt(ctx context.Context, attempt TxAttempt) error {
	...
	_, err := qq.Exec(`DELETE FROM evm.eth_tx_attempts WHERE id = $1`, attempt.ID)
...
func (o *evmTxStore) SaveInProgressAttempt(attempt *TxAttempt) error {
	...
	if attempt.ID == 0 {
		...
		DbEthTxAttemptToEthTxAttempt(dbAttempt, attempt)
		return pkgerrors.Wrap(e, "SaveInProgressAttempt failed to insert into evm.eth_tx_attempts")
	}
	// Update only applies to case of insufficient eth and simply changes the state to in_progress
	res, err := o.q.Exec(`UPDATE evm.eth_tx_attempts SET state=$1, broadcast_before_block_num=$2 WHERE id=$3`, dbAttempt.State, dbAttempt.BroadcastBeforeBlockNum, dbAttempt.ID)
	if err != nil {
		return pkgerrors.Wrap(err, "SaveInProgressAttempt failed to update evm.eth_tx_attempts")
	}
	rowsAffected, err := res.RowsAffected()
	if err != nil {
	...
	if rowsAffected == 0 {
		return pkgerrors.Wrapf(sql.ErrNoRows, "SaveInProgressAttempt tried to update evm.eth_tx_attempts but no rows matched id %d", attempt.ID)
	}
	return nil
}
...
//
// It also returns evm.eth_txes that are unconfirmed with no evm.eth_tx_attempts
func (o *evmTxStore) FindTxsRequiringGasBump(ctx context.Context, address common.Address, blockNum, gasBumpThreshold, depth int64, chainID *big.Int) (etxs []*Tx, err error) {
	...
	err = qq.Transaction(func(tx pg.Queryer) error {
		stmt := `
SELECT evm.eth_txes.* FROM evm.eth_txes
LEFT JOIN evm.eth_tx_attempts ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id AND (broadcast_before_block_num > $4 OR broadcast_before_block_num IS NULL OR evm.eth_tx_attempts.state != 'broadcast')
WHERE evm.eth_txes.state = 'unconfirmed' AND evm.eth_tx_attempts.id IS NULL AND evm.eth_txes.from_address = $1 AND evm.eth_txes.evm_chain_id = $2
	AND (($3 = 0) OR (evm.eth_txes.id IN (SELECT id FROM evm.eth_txes WHERE state = 'unconfirmed' AND from_address = $1 ORDER BY nonce ASC LIMIT $3)))
ORDER BY nonce ASC
`
		var dbEtxs []DbEthTx
		if err = tx.Select(&dbEtxs, stmt, address, chainID.String(), depth, blockNum-gasBumpThreshold); err != nil {
			return pkgerrors.Wrap(err, "FindEthTxsRequiringGasBump failed to load evm.eth_txes")
		}
		etxs = make([]*Tx, len(dbEtxs))
		dbEthTxsToEvmEthTxPtrs(dbEtxs, etxs)
		err = o.LoadTxesAttempts(etxs, pg.WithQueryer(tx))
		return pkgerrors.Wrap(err, "FindEthTxsRequiringGasBump failed to load evm.eth_tx_attempts")
	}, pg.OptReadOnlyTx())
	return
}
...
func (o *evmTxStore) FindTxsRequiringResubmissionDueToInsufficientFunds(address common.Address, chainID *big.Int, qopts ...pg.QOpt) (etxs []*Tx, err error) {
	...
	err = qq.Transaction(func(tx pg.Queryer) error {
		...
		err = tx.Select(&dbEtxs, `
SELECT DISTINCT evm.eth_txes.* FROM evm.eth_txes
INNER JOIN evm.eth_tx_attempts ON evm.eth_txes.id = evm.eth_tx_attempts.eth_tx_id AND evm.eth_tx_attempts.state = 'insufficient_eth'
WHERE evm.eth_txes.from_address = $1 AND evm.eth_txes.state = 'unconfirmed' AND evm.eth_txes.evm_chain_id = $2
ORDER BY nonce ASC
`, address, chainID.String())
		if err != nil {
			return pkgerrors.Wrap(err, "FindEthTxsRequiringResubmissionDueToInsufficientEth failed to load evm.eth_txes")
		}
		etxs = make([]*Tx, len(dbEtxs))
		dbEthTxsToEvmEthTxPtrs(dbEtxs, etxs)
		err = o.LoadTxesAttempts(etxs, pg.WithQueryer(tx))
		return pkgerrors.Wrap(err, "FindEthTxsRequiringResubmissionDueToInsufficientEth failed to load evm.eth_tx_attempts")
	}, pg.OptReadOnlyTx())
	return
}
...
FROM (
	SELECT e1.id, e1.nonce, e1.from_address FROM evm.eth_txes AS e1 WHERE id IN (
		SELECT e2.id FROM evm.eth_txes AS e2
		INNER JOIN evm.eth_tx_attempts ON e2.id = evm.eth_tx_attempts.eth_tx_id
		WHERE e2.state = 'confirmed_missing_receipt'
		AND e2.evm_chain_id = $3
		GROUP BY e2.id
		HAVING max(evm.eth_tx_attempts.broadcast_before_block_num) < $2
	)
	FOR UPDATE OF e1
) e0
WHERE e0.id = evm.eth_txes.id
...
FROM evm.eth_txes e
INNER JOIN evm.eth_tx_attempts a ON e.id = a.eth_tx_id
WHERE e.id = ANY($1)
GROUP BY e.id
`, etxIDs)
...
func (o *evmTxStore) SaveReplacementInProgressAttempt(oldAttempt TxAttempt, replacementAttempt *TxAttempt, qopts ...pg.QOpt) error {
	...
	return qq.Transaction(func(tx pg.Queryer) error {
		if _, err := tx.Exec(`DELETE FROM evm.eth_tx_attempts WHERE id=$1`, oldAttempt.ID); err != nil {
			return pkgerrors.Wrap(err, "saveReplacementInProgressAttempt failed to delete from evm.eth_tx_attempts")
		}
...
func (o *evmTxStore) FindNextUnstartedTransactionFromAddress(etx *Tx, fromAddress common.Address, chainID *big.Int, qopts ...pg.QOpt) error {
	qq := o.q.WithOpts(qopts...)
	var dbEtx DbEthTx
	err := qq.Get(&dbEtx, `SELECT * FROM evm.eth_txes WHERE from_address = $1 AND state = 'unstarted' AND evm_chain_id = $2 ORDER BY value ASC, created_at ASC, id ASC`, fromAddress, chainID.String())
	DbEthTxToEthTx(dbEtx, etx)
...
func (o *evmTxStore) UpdateTxFatalError(etx *Tx, qopts ...pg.QOpt) error {
	...
	return qq.Transaction(func(tx pg.Queryer) error {
		if _, err := tx.Exec(`DELETE FROM evm.eth_tx_attempts WHERE eth_tx_id = $1`, etx.ID); err != nil {
		...
		err := pkgerrors.Wrap(tx.Get(&dbEtx, `UPDATE evm.eth_txes SET state=$1, error=$2, broadcast_at=NULL, initial_broadcast_at=NULL, nonce=NULL WHERE id=$3 RETURNING *`, etx.State, etx.Error, etx.ID), "saveFatallyErroredTransaction failed to save eth_tx")
		DbEthTxToEthTx(dbEtx, etx)
		return err
	})
}
...
func (o *evmTxStore) UpdateTxAttemptInProgressToBroadcast(etx *Tx, attempt TxAttempt, NewAttemptState txmgrtypes.TxAttemptState, incrNextNonceCallback txmgrtypes.QueryerFunc, qopts ...pg.QOpt) error {
	...
	return qq.Transaction(func(tx pg.Queryer) error {
		...
		dbEtx := DbEthTxFromEthTx(etx)
		if err := tx.Get(&dbEtx, `UPDATE evm.eth_txes SET state=$1, error=$2, broadcast_at=$3, initial_broadcast_at=$4 WHERE id = $5 RETURNING *`, dbEtx.State, dbEtx.Error, dbEtx.BroadcastAt, dbEtx.InitialBroadcastAt, dbEtx.ID); err != nil {
			return pkgerrors.Wrap(err, "SaveEthTxAttempt failed to save eth_tx")
		}
		DbEthTxToEthTx(dbEtx, etx)
		dbAttempt := DbEthTxAttemptFromEthTxAttempt(&attempt)
		if err := tx.Get(&dbAttempt, `UPDATE evm.eth_tx_attempts SET state = $1 WHERE id = $2 RETURNING *`, dbAttempt.State, dbAttempt.ID); err != nil {
...
func (o *evmTxStore) UpdateTxUnstartedToInProgress(etx *Tx, attempt *TxAttempt, qopts ...pg.QOpt) error {
	...
	return qq.Transaction(func(tx pg.Queryer) error {
		...
		// In this case, we must remove the abandoned attempt from evm.eth_tx_attempts before replacing it with a new one.  In any other
		// case, we uphold the constraint, leaving the original tx attempt as-is and returning the constraint violation error.
		//
		// Note:  the record of the original abandoned transaction will remain in evm.eth_txes, only the attempt is replaced.  (Any receipt
		...
		// can be abandoned.)
		_, err := tx.Exec(`DELETE FROM evm.eth_tx_attempts a USING evm.eth_txes t
		...
		DbEthTxAttemptToEthTxAttempt(dbAttempt, attempt)
		dbEtx := DbEthTxFromEthTx(etx)
		err = tx.Get(&dbEtx, `UPDATE evm.eth_txes SET nonce=$1, state=$2, broadcast_at=$3, initial_broadcast_at=$4 WHERE id=$5 RETURNING *`, etx.Sequence, etx.State, etx.BroadcastAt, etx.InitialBroadcastAt, etx.ID)
		DbEthTxToEthTx(dbEtx, etx)
...
func (o *evmTxStore) GetTxInProgress(fromAddress common.Address, qopts ...pg.QOpt) (etx *Tx, err error) {
	...
	err = qq.Transaction(func(tx pg.Queryer) error {
		var dbEtx DbEthTx
		err = tx.Get(&dbEtx, `SELECT * FROM evm.eth_txes WHERE from_address = $1 and state = 'in_progress'`, fromAddress)
		if errors.Is(err, sql.ErrNoRows) {
			etx = nil
			return nil
		} else if err != nil {
...
func (o *evmTxStore) HasInProgressTransaction(account common.Address, chainID *big.Int, qopts ...pg.QOpt) (exists bool, err error) {
	qq := o.q.WithOpts(qopts...)
	err = qq.Get(&exists, `SELECT EXISTS(SELECT 1 FROM evm.eth_txes WHERE state = 'in_progress' AND from_address = $1 AND evm_chain_id = $2)`, account, chainID.String())
...
func (o *evmTxStore) countTransactionsWithState(fromAddress common.Address, state txmgrtypes.TxState, chainID *big.Int, qopts ...pg.QOpt) (count uint32, err error) {
	qq := o.q.WithOpts(qopts...)
	err = qq.Get(&count, `SELECT count(*) FROM evm.eth_txes WHERE from_address = $1 AND state = $2 AND evm_chain_id = $3`,
...
func (o *evmTxStore) CheckTxQueueCapacity(fromAddress common.Address, maxQueuedTransactions uint64, chainID *big.Int, qopts ...pg.QOpt) (err error) {
	...
	var count uint64
	err = qq.Get(&count, `SELECT count(*) FROM evm.eth_txes WHERE from_address = $1 AND state = 'unstarted' AND evm_chain_id = $2`, fromAddress, chainID.String())
	if err != nil {
...
func (o *evmTxStore) CreateTransaction(txRequest TxRequest, chainID *big.Int, qopts ...pg.QOpt) (tx Tx, err error) {
	...
	err = qq.Transaction(func(tx pg.Queryer) error {
		if txRequest.PipelineTaskRunID != nil {
			...
			err = tx.Get(&dbEtx, `SELECT * FROM evm.eth_txes WHERE pipeline_task_run_id = $1 AND evm_chain_id = $2`, txRequest.PipelineTaskRunID, chainID.String())
		...
		err = tx.Get(&dbEtx, `
INSERT INTO evm.eth_txes (from_address, to_address, encoded_payload, value, gas_limit, state, created_at, meta, subject, evm_chain_id, min_confirmations, pipeline_task_run_id, transmit_checker, idempotency_key)
VALUES (
...
`, txRequest.FromAddress, txRequest.ToAddress, txRequest.EncodedPayload, assets.Eth(txRequest.Value), txRequest.FeeLimit, txRequest.Meta, txRequest.Strategy.Subject(), chainID.String(), txRequest.MinConfirmations, txRequest.PipelineTaskRunID, txRequest.Checker, txRequest.IdempotencyKey)
		...
		if err != nil {
			return pkgerrors.Wrap(err, "CreateEthTransaction failed to prune evm.eth_txes")
		}
		if pruned > 0 {
...
func (o *evmTxStore) ReapTxHistory(minBlockNumberToKeep int64, timeThreshold time.Time, chainID *big.Int) error {
	// Delete old confirmed evm.eth_txes
	// NOTE that this relies on foreign key triggers automatically removing
	// the evm.eth_tx_attempts and evm.eth_receipts linked to every eth_tx
	err := pg.Batch(func(_, limit uint) (count uint, err error) {
		res, err := o.q.Exec(`
WITH old_enough_receipts AS (
	SELECT tx_hash FROM evm.eth_receipts
	WHERE block_number < $1
	ORDER BY block_number ASC, id ASC
	LIMIT $2
)
DELETE FROM evm.eth_txes
USING old_enough_receipts, evm.eth_tx_attempts
WHERE evm.eth_tx_attempts.eth_tx_id = evm.eth_txes.id
AND evm.eth_tx_attempts.hash = old_enough_receipts.tx_hash
AND evm.eth_txes.created_at < $3
AND evm.eth_txes.state = 'confirmed'
AND evm_chain_id = $4`, minBlockNumberToKeep, limit, timeThreshold, chainID.String())
		if err != nil {
			return count, pkgerrors.Wrap(err, "ReapTxes failed to delete old confirmed evm.eth_txes")
		}
		rowsAffected, err := res.RowsAffected()
		if err != nil {
	...
	if err != nil {
		return pkgerrors.Wrap(err, "TxmReaper#reapEthTxes batch delete of confirmed evm.eth_txes failed")
	}
	// Delete old 'fatal_error' evm.eth_txes
	err = pg.Batch(func(_, limit uint) (count uint, err error) {
		res, err := o.q.Exec(`
DELETE FROM evm.eth_txes
WHERE created_at < $1
...
AND evm_chain_id = $2`, timeThreshold, chainID.String())
		if err != nil {
			return count, pkgerrors.Wrap(err, "ReapTxes failed to delete old fatally errored evm.eth_txes")
		}
		rowsAffected, err := res.RowsAffected()
		if err != nil {
			return count, pkgerrors.Wrap(err, "ReapTxes failed to get rows affected")
	...
	if err != nil {
		return pkgerrors.Wrap(err, "TxmReaper#reapEthTxes batch delete of fatally errored evm.eth_txes failed")
	}
...
func (o *evmTxStore) Abandon(chainID *big.Int, addr common.Address) error {
	_, err := o.q.Exec(`UPDATE evm.eth_txes SET state='fatal_error', nonce = NULL, error = 'abandoned' WHERE state IN ('unconfirmed', 'in_progress', 'unstarted') AND evm_chain_id = $1 AND from_address = $2`, chainID.String(), addr)
	return err
}
