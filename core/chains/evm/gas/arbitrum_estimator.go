...
func (a *arbitrumEstimator) GetLegacyGas(calldata []byte, l2GasLimit uint64, maxGasPriceWei *big.Int, opts ...Opt) (gasPrice *big.Int, chainSpecificGasLimit uint64, err error) {
	...
	ok := a.IfStarted(func() {
		...
		chainSpecificGasLimit = l2GasLimit + perL2Tx + uint64(len(calldata))*perL1CalldataUnit
...
func (a *arbitrumEstimator) getPricesInArbGas() (perL2Tx uint64, perL1CalldataUnit uint64) {
...
func (a *arbitrumEstimator) callGetPricesInArbGas() (perL2Tx uint64, perL1CalldataUnit uint64, err error) {
