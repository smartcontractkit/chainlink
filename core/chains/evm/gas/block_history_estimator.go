...
func (b *BlockHistoryEstimator) checkConnectivity(attempts []EvmPriorAttempt) error {
	...
	for _, attempt := range attempts {
		if attempt.GetBroadcastBeforeBlockNum() == nil {
			...
			return errors.Errorf("BroadcastBeforeBlockNum was unexpectedly nil for attempt %s", attempt.GetHash().String())
		}
		...
		b.logger.Debugw(fmt.Sprintf("transaction %s has been pending inclusion for %d blocks which equals or exceeds expected specified check inclusion blocks of %d", attempt.GetHash().String(), blocksSinceBroadcast, expectInclusionWithinBlocks), "broadcastBeforeBlockNum", broadcastBeforeBlockNum, "latestBlockNum", *latestBlockNum)
		...
		if err != nil {
			if errors.Is(err, ErrNoSuitableTransactions) {
				b.logger.Warnf("no suitable transactions found to verify if transaction %s has been included within expected inclusion blocks of %d", attempt.GetHash().String(), expectInclusionWithinBlocks)
				return nil
			}
