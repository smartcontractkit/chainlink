...
// GetPrices provides a mock function with given fields: ctx, maxGasPriceWei, opts
func (_m *EvmFeeEstimator) GetPrices(ctx context.Context, maxGasPriceWei *assets.Wei, opts ...types.Opt) ([]gas.PriceComponent, error) {
	_va := make([]interface{}, len(opts))
	for _i := range opts {
		_va[_i] = opts[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, ctx, maxGasPriceWei)
	_ca = append(_ca, _va...)
	ret := _m.Called(_ca...)

	var r0 []gas.PriceComponent
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, *assets.Wei, ...types.Opt) ([]gas.PriceComponent, error)); ok {
		return rf(ctx, maxGasPriceWei, opts...)
	}
	if rf, ok := ret.Get(0).(func(context.Context, *assets.Wei, ...types.Opt) []gas.PriceComponent); ok {
		r0 = rf(ctx, maxGasPriceWei, opts...)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]gas.PriceComponent)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, *assets.Wei, ...types.Opt) error); ok {
		r1 = rf(ctx, maxGasPriceWei, opts...)
	} else {
		r1 = ret.Error(1)
	}
