...
func TestORM(t *testing.T) {
	...
	// Read
	unstarted, err := o.SelectMsgsWithState(Unstarted, 5)
	require.NoError(t, err)
	...
	// Limit
	unstarted, err = o.SelectMsgsWithState(Unstarted, 0)
	assert.Error(t, err)
	assert.Empty(t, unstarted)
	unstarted, err = o.SelectMsgsWithState(Unstarted, -1)
	assert.Error(t, err)
	assert.Empty(t, unstarted)
	...
	assert.Equal(t, "hello", string(unstarted[0].Raw))
	assert.Equal(t, chainID, unstarted[0].ChainID)
	unstarted, err = o.SelectMsgsWithState(Unstarted, 2)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	broadcasted, err := o.SelectMsgsWithState(Broadcasted, 5)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	confirmed, err := o.SelectMsgsWithState(Confirmed, 5)
	require.NoError(t, err)
