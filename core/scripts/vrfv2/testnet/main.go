...
func main() {
	...
	case "batch-coordinatorv2-fulfill":
		...
		preSeedSlice := helpers.ParseIntSlice(*preSeeds)
		bhSlice := helpers.ParseHashSlice(*blockHashes)
		blockNumSlice := helpers.ParseIntSlice(*blockNums)
		subIDSlice := helpers.ParseIntSlice(*subIDs)
		cbLimitsSlice := helpers.ParseIntSlice(*cbGasLimits)
		numWordsSlice := helpers.ParseIntSlice(*numWordses)
	...
	case "batch-bhs-store":
		...
		helpers.PanicErr(err)
		blockNumbers := helpers.ParseIntSlice(*blockNumbersArg)
		helpers.PanicErr(err)
