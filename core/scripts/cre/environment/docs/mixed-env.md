...
- **CI:** automatically on CRE-affecting PRs, via `.github/workflows/cre-system-tests.yaml` run in **mixed-env mode** (`mixed_env: true`) — the same workflow that runs the normal CRE system tests, with a curated matrix, per-node 2-2 image resolution, and a non-determinism gate switched on by that flag. It runs the OCR3/DON2DON-heavy tests — `Test_CRE_V2_Suite_Bucket_A`, `Test_CRE_V2_Suite_Bucket_B`, and the `Test_CRE_V2_EVM_Read_*` suite — under mixed-env. The PR image and the develop image are both already built (per-PR and nightly), so no extra image builds are added. A dedicated **Check for non-determinism** step scans the live node containers after the suite and fails the job on any marker — kept separate from the auto-quarantined test step so the failure can't be swallowed.
- **Nightly full sweep:** `.github/workflows/cre-mixed-env-nightly.yaml` calls the same dedicated workflow with `full_matrix: true` — running the *entire* matrix under mixed-env (EVM base + Solana/Aptos/Stellar/sharded/cache-test/multi-gateway variants), comparing the just-built develop nightly against a previous one to catch non-determinism introduced into develop between nightlies. It's triggered from `post-docker-build.yml` (the same fan-out the other nightly suites use), so it fires right after Docker Build publishes the nightly image — no cron/timing race. A nightly run has no PR to block, so on failure it posts a `:rotating_light:` alert to the **#cre-platform-operations** Slack channel (with a link to the run) — that's the signal to watch, not a CI check.
