...
#
#   1. stops the env
#   2. starts env + observability (builds node + capabilities on-the-fly)
#   3. compiles + deploys the workflow
#   4. watches results until Ctrl-C
#
...
echo "==> [1/4] Stopping local env (if running)"
...
# --- 2. start the env with local builds and observability -------------------
echo "==> [2/4] Starting local CRE (observability) with local source builds"
...
# --- 3. compile + deploy the workflow ---------------------------------------
echo "==> [3/4] Compiling + deploying workflow '$WORKFLOW_NAME'"
...
# --- 4. watch workflow results until killed ---------------------------------
echo ""
echo "==> [4/4] Watching '$WORKFLOW_NAME' results on $WATCH_NODE (Ctrl-C to stop)"
