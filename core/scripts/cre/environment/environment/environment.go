...
// localDepsImageTag is the tag used when we pre-build the chainlink node image for local-deps.
const localDepsImageTag = "chainlink:local-deps"
...
func ensureLocalDepsDockerignore(parentDir string) (cleanup func(), err error) {
	ignorePath := filepath.Join(parentDir, ".dockerignore")
	existing, err := os.ReadFile(ignorePath)
	if err != nil && !os.IsNotExist(err) {
		return nil, fmt.Errorf("read existing .dockerignore: %w", err)
	}
	restore := len(existing) > 0
	content := `# CRE local-deps: restrict context to required repos only; exclude heavy dirs for smaller context.
*
!chainlink
!chainlink/**
!chainlink-common
...
!chainlink-aptos
!chainlink-aptos/**
# Exclude .git, node_modules, state, cache inside each repo
chainlink/.git
chainlink/.git/**
chainlink/**/node_modules
chainlink/**/node_modules/**
chainlink/**/state
chainlink/**/state/**
chainlink/.cre-docker-cache
chainlink-common/.git
chainlink-common/.git/**
chainlink-common/**/node_modules
chainlink-common/**/node_modules/**
capabilities/.git
capabilities/.git/**
capabilities/**/node_modules
capabilities/**/node_modules/**
chainlink-aptos/.git
chainlink-aptos/.git/**
chainlink-aptos/**/node_modules
chainlink-aptos/**/node_modules/**
# Exclude dirs not needed to build chainlink binary (separate modules / test-only)
chainlink/system-tests
chainlink/system-tests/**
...
chainlink/deployment/**
# Exclude chainlink-aptos build artifacts and large contract outputs
chainlink-aptos/contracts/**/build
chainlink-aptos/contracts/**/build/**
chainlink-aptos/contracts/**/target
chainlink-aptos/contracts/**/target/**
chainlink-aptos/**/*.hex
chainlink-aptos/**/*.zip
# Exclude testdata only where not required: chainlink and chainlink-common need testdata for go mod tidy and build (imported by core/capabilities, logevent, etc.)
capabilities/**/testdata
capabilities/**/testdata/**
chainlink-aptos/**/testdata
chainlink-aptos/**/testdata/**
chainlink/.github
chainlink/.github/**
...
`
	...
	cleanup = func() {
		if restore {
			_ = os.WriteFile(ignorePath, existing, 0o644)
		} else {
...
// Image on those specs and clear DockerFilePath/DockerContext so the framework does not build.
func ensureLocalDepsNodeImage(ctx context.Context, in *envconfig.Config, relativePathToRepoRoot string) error {
	cwd, err := os.Getwd()
	if err != nil {
		return err
	}
	chainlinkRepoRoot, err := filepath.Abs(filepath.Join(cwd, relativePathToRepoRoot))
	if err != nil {
		return err
	}
	// See if any node spec requests the local-deps build (docker_ctx ".." and docker_file for local-deps).
	var needBuild bool
	var buildArgs map[string]string
	for setIdx := range in.NodeSets {
		for nodeIdx := range in.NodeSets[setIdx].NodeSpecs {
			n := in.NodeSets[setIdx].NodeSpecs[nodeIdx].Node
			if n == nil || n.DockerFilePath == "" {
				continue
			}
			if n.DockerContext == ".." && strings.Contains(n.DockerFilePath, "chainlink-local-deps") {
				needBuild = true
				if n.DockerBuildArgs != nil {
					buildArgs = n.DockerBuildArgs
				}
				break
			}
		}
	...
	dockerfilePath := filepath.Join(chainlinkRepoRoot, "core", "chainlink-local-deps.Dockerfile")
	parentDir, err := filepath.Abs(filepath.Join(chainlinkRepoRoot, ".."))
	if err != nil {
		return fmt.Errorf("resolve local-deps context path: %w", err)
	}
	// Restrict build context to chainlink, chainlink-common, capabilities, chainlink-aptos via .dockerignore
	// so Docker does not send the entire parent dir (e.g. 22GB+ and I/O errors).
	cleanupDockerignore, err := ensureLocalDepsDockerignore(parentDir)
	if err != nil {
		return err
	}
	defer cleanupDockerignore()
	capabilitiesDir := filepath.Join(parentDir, "capabilities")
	if err := os.MkdirAll(capabilitiesDir, 0o755); err != nil {
		return fmt.Errorf("ensure capabilities dir in parent: %w", err)
	}
	// Use only the Docker daemon's build cache (no local .cre-docker-cache). Rebuilds are cold after daemon restart or docker builder prune.
	args := []string{"build", "-t", localDepsImageTag, "-f", dockerfilePath, parentDir}
	for k, v := range buildArgs {
		args = append(args, "--build-arg", k+"="+v)
	}
	useLocalCapabilities := false
	if _, err := os.Stat(filepath.Join(capabilitiesDir, "go.mod")); err == nil {
		args = append(args, "--build-arg", "CL_USE_LOCAL_CAPABILITIES=true")
		useLocalCapabilities = true
	}
	// Optional: use local chainlink-aptos when present (sibling of chainlink) for write-target iteration.
	if _, err := os.Stat(filepath.Join(parentDir, "chainlink-aptos", "go.mod")); err == nil {
		args = append(args, "--build-arg", "CL_USE_LOCAL_APTOS=true")
	}
	// Secret mount is required by the Dockerfile. Use GIT_AUTH_TOKEN env var if set (for private repo access during plugin install); otherwise a dummy when using local capabilities.
	if token := os.Getenv("GIT_AUTH_TOKEN"); token != "" {
		secretFile, err := os.CreateTemp("", "cre-git-auth-*")
		if err != nil {
			return fmt.Errorf("create GIT_AUTH_TOKEN secret file: %w", err)
		}
		if _, err := secretFile.WriteString(token); err != nil {
			_ = secretFile.Close()
			_ = os.Remove(secretFile.Name())
			return fmt.Errorf("write GIT_AUTH_TOKEN secret file: %w", err)
		}
		if err := secretFile.Close(); err != nil {
			_ = os.Remove(secretFile.Name())
			return fmt.Errorf("close GIT_AUTH_TOKEN secret file: %w", err)
		}
		defer os.Remove(secretFile.Name())
		args = append(args, "--secret", "id=GIT_AUTH_TOKEN,src="+secretFile.Name())
	} else if useLocalCapabilities {
		emptySecret, err := os.CreateTemp("", "cre-git-auth-dummy-*")
		if err != nil {
			return fmt.Errorf("create dummy secret file for local-deps build: %w", err)
		}
		_ = emptySecret.Close()
		defer os.Remove(emptySecret.Name())
		args = append(args, "--secret", "id=GIT_AUTH_TOKEN,src="+emptySecret.Name())
	}
	cmd := exec.CommandContext(ctx, "docker", args...)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	cmd.Env = append(os.Environ(), "DOCKER_BUILDKIT=1")
	framework.L.Info().Str("cmd", cmd.String()).Msg("Building chainlink node image for local-deps")
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("build chainlink local-deps image: %w", err)
	}
	// Point all specs that used local-deps build to the image we just built and clear build fields.
	for setIdx := range in.NodeSets {
		for nodeIdx := range in.NodeSets[setIdx].NodeSpecs {
			n := in.NodeSets[setIdx].NodeSpecs[nodeIdx].Node
			if n == nil || n.DockerFilePath == "" {
			...
			if n.DockerContext == ".." && strings.Contains(n.DockerFilePath, "chainlink-local-deps") {
				in.NodeSets[setIdx].NodeSpecs[nodeIdx].Node.Image = localDepsImageTag
...
func StartCLIEnvironment(
	...
	// Pre-build chainlink node image when config uses local-deps Dockerfile; then use that image
	// so the framework never runs its (path-corrupting) build.
	if err := ensureLocalDepsNodeImage(cmdContext, in, relativePathToRepoRoot); err != nil {
		return nil, fmt.Errorf("failed to ensure local-deps node image: %w", err)
	}
