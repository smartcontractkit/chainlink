...
func swapCapability(ctx context.Context, capabilityFlag, binaryPath string, forceFlag bool) error {
	...
	creEnvironment, donTopology, loadErr := creenv.BuildFromSavedState(ctx, cldLogger, config, envArtifact)
	if loadErr != nil {
	...
	for donIdx := range donIdxToNodeIDToProposalIDs {
		pattern := ns.NodeNamePrefix(donTopology.Dons.List()[donIdx].Name)
