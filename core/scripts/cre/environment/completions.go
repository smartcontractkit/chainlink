...
func buildCommandTree() *CompletionNode {
	...
	workflowNode.Children["deploy"] = &CompletionNode{
		Flags: []prompt.Suggest{
			...
			{Text: "--container-name-pattern", Description: "Pattern to match Docker containers for workflow artifact copy (e.g. 'workflow-node' or 'feeds-zone-a-node')"},
			{Text: "--workflow-don-name", Description: "Workflow DON name from local CRE state (required for multi-DON topologies; e.g. feeds-zone-a)"},
