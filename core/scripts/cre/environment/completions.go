...
func buildCommandTree() *CompletionNode {
	...
	workflowNode.Children["deploy"] = &CompletionNode{
		Flags: []prompt.Suggest{
			...
			{Text: "--container-name-pattern", Description: "Substring match for docker cp (defaults to {workflow-don-name}-node from local CRE state)"},
			{Text: "--workflow-don-name", Description: "Workflow DON nodesets.name (optional when --don-family uniquely identifies the DON)"},
			{Text: "--don-family", Description: "DON family for registry registration (resolves workflow DON when unique)"},
			{Text: "--shard-index", Description: "Shard index when multiple shard DONs share a don_family"},
