// Package crereconciler provides a reconciliation tool for configuring Chainlink nodes
...
import (
	...
	"github.com/smartcontractkit/chainlink/core/scripts/cre/crereconciler/internal/domain"
	"github.com/smartcontractkit/chainlink/core/scripts/cre/crereconciler/internal/ui"
)

// Version identifies the build. Overridden at build time via:
//
//	go build -ldflags "-X github.com/smartcontractkit/chainlink/core/scripts/cre/crereconciler.Version=$(git rev-parse --short HEAD)-$(date -u +%Y%m%d%H%M%S)"
//
...
Usage:
  cre-reconciler apply --desired cre/desired.toml --state cre/state.toml
  cre-reconciler status --state cre/state.toml
  cre-reconciler diff --desired cre/desired.toml --state cre/state.toml`,
}
...
func runApply(ctx context.Context) error {
	...
	log.Info().Str("version", Version).Msg("cre-reconciler starting")
