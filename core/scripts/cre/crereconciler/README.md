...
cd core/scripts/cre/crereconciler

# Build
go build -o bin/cre-reconciler ./cmd
...
# Run reconciliation
./bin/cre-reconciler apply --desired cre/desired.toml --state cre/state.toml
...
```bash
./bin/cre-reconciler serve --desired cre/desired.toml --state cre/state.toml --chart-dir deploy/config/my-repo
```
