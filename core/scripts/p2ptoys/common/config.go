...
type Config struct {
	...
	BootstrapperPeerIDs  []ragep2ptypes.PeerID
...
func ParseConfigFromFile(fileName string) (*Config, error) {
	...
	for _, hexKey := range config.Bootstrappers {
		...
		config.BootstrapperPeerIDs = append(config.BootstrapperPeerIDs, peerID)
	}

	locators := []commontypes.BootstrapperLocator{}
	for id, peerID := range config.BootstrapperPeerIDs {
		locators = append(locators, commontypes.BootstrapperLocator{
			PeerID: peerID.String(),
			Addrs:  []string{fmt.Sprintf("127.0.0.1:%d", BootstrapStartPort+id)},
		})
	}
