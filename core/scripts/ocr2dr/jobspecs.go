...
func generateJobSpecs(cfg *config, nodes []Node) {
	...
	outputPath := filepath.Join(artefactsDir, removeHttpsPrefix(nodes[0].Host)+".toml")
	...
	for i := 1; i < len(nodes); i++ {
		...
		outputPath := filepath.Join(artefactsDir, removeHttpsPrefix(nodes[i].Host)+".toml")
