			...
			before: func(ctx context.Context, f *gqlTestFramework) {
				...
				f.Mocks.jobORM.On("FindJobWithoutSpecErrors", mock.Anything, id).Return(job.Job{
					...
					OCROracleSpec: &job.OCROracleSpec{
						BlockchainTimeout:                      types.Interval(1 * time.Minute),
						ContractAddress:                        contractAddress,
						...
						DatabaseTimeout:                        types.NewInterval(3 * time.Second),
						ObservationGracePeriod:                 types.NewInterval(4 * time.Second),
						ContractTransmitterTransmitTimeout:     types.NewInterval(555 * time.Millisecond),
						CreatedAt:                              f.Timestamp(),
						EVMChainID:                             ubig.NewI(42),
						IsBootstrapPeer:                        false,
						EncryptedOCRKeyBundleID:                &keyBundleID,
						ObservationTimeout:                     types.Interval(2 * time.Minute),
			...
			before: func(ctx context.Context, f *gqlTestFramework) {
				...
				f.Mocks.jobORM.On("FindJobWithoutSpecErrors", mock.Anything, id).Return(job.Job{
					...
					BootstrapSpec: &job.BootstrapSpec{
						...
						BlockchainTimeout:                 types.Interval(2 * time.Minute),
						ContractConfigTrackerPollInterval: types.Interval(2 * time.Minute),
