...
func (cc *EVMForwardersController) Delete(c *gin.Context) {
	...
	filterCleanup := func(tx sqlutil.DataSource, evmChainID int64, addr common.Address) error {
		...
		if err2 != nil {
			cc.App.GetLogger().Info(fmt.Sprintf("DeleteForwarder chain err2 %s ", err2.Error()))
		...
		if chain.LogPoller() == logpoller.LogPollerDisabled {
			cc.App.GetLogger().Info(fmt.Sprintf("DeleteForwarder Lp disabled "))
		...
		cc.App.GetLogger().Info(fmt.Sprintf("DeleteForwarder LogPoller().UnregisterFilter  unregErr %s ", unregErr))
		return unregErr
	}
	...
	cc.App.GetLogger().Info(fmt.Sprintf("DeleteForwarder result? %s", err))
