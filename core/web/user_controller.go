...
func (c *UserController) UpdatePassword(ctx *gin.Context) {
	...
	if !utils.CheckPasswordHash(request.OldPassword, user.HashedPassword) {
		c.App.GetAuditLogger().Audit(audit.PasswordResetAttemptFailedMismatch, map[string]any{"user": user.Email})
	...
	c.App.GetAuditLogger().Audit(audit.PasswordResetSuccess, map[string]any{"user": user.Email})
	jsonAPIResponse(ctx, presenters.NewUserResource(user), "user")
}
...
func (c *UserController) NewAPIToken(ctx *gin.Context) {
	...
	if !utils.CheckPasswordHash(request.Password, user.HashedPassword) {
		c.App.GetAuditLogger().Audit(audit.APITokenCreateAttemptPasswordMismatch, map[string]any{"user": user.Email})
	...
	c.App.GetAuditLogger().Audit(audit.APITokenCreated, map[string]any{"user": user.Email})
...
func (c *UserController) DeleteAPIToken(ctx *gin.Context) {
	...
	if !utils.CheckPasswordHash(request.Password, user.HashedPassword) {
		c.App.GetAuditLogger().Audit(audit.APITokenDeleteAttemptPasswordMismatch, map[string]any{"user": user.Email})
		jsonAPIError(ctx, http.StatusUnauthorized, errors.New("incorrect password"))
		return
	}
	...
	{
		c.App.GetAuditLogger().Audit(audit.APITokenDeleted, map[string]any{"user": user.Email})
