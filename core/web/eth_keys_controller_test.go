...
func TestETHKeysController_ChainSuccess_ResetWithAbandon(t *testing.T) {
	...
	var count int
	err = app.GetSqlxDB().Get(&count, `SELECT count(*) FROM evm.eth_txes WHERE from_address = $1 AND state = 'fatal_error'`, addr)
	require.NoError(t, err)
	...
	var s string
	err = app.GetSqlxDB().Get(&s, `SELECT error FROM evm.eth_txes WHERE from_address = $1 AND state = 'fatal_error'`, addr)
	require.NoError(t, err)
