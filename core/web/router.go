...
func v2Routes(app chainlink.Application, r *gin.RouterGroup) {
	...
	))
	{
		...
		authv2.Use(auth.RequiresAdminRole).GET("/users", uc.Index)
		authv2.Use(auth.RequiresAdminRole).POST("/users", uc.Create)
		authv2.Use(auth.RequiresAdminRole).PATCH("/users", uc.Update)
		authv2.Use(auth.RequiresAdminRole).DELETE("/users/:email", uc.Delete)
		...
		authv2.Use(auth.RequiresEditRole).POST("/external_initiators", eia.Create)
		authv2.Use(auth.RequiresEditRole).DELETE("/external_initiators/:Name", eia.Destroy)

		bt := BridgeTypesController{app}
		authv2.GET("/bridge_types", paginatedRequest(bt.Index))
		authv2.Use(auth.RequiresEditRole).POST("/bridge_types", bt.Create)
		authv2.GET("/bridge_types/:BridgeName", bt.Show)
		authv2.Use(auth.RequiresEditRole).PATCH("/bridge_types/:BridgeName", bt.Update)
		authv2.Use(auth.RequiresEditRole).DELETE("/bridge_types/:BridgeName", bt.Destroy)

		ets := EVMTransfersController{app}
		authv2.Use(auth.RequiresAdminRole).POST("/transfers", ets.Create)
		authv2.Use(auth.RequiresAdminRole).POST("/transfers/evm", ets.Create)
		tts := TerraTransfersController{app}
		authv2.Use(auth.RequiresAdminRole).POST("/transfers/terra", tts.Create)
		sts := SolanaTransfersController{app}
		authv2.Use(auth.RequiresAdminRole).POST("/transfers/solana", sts.Create)

		cc := ConfigController{app}
		authv2.GET("/config", cc.Show)
		authv2.Use(auth.RequiresAdminRole).PATCH("/config", cc.Patch)
		authv2.Use(auth.RequiresAdminRole).GET("/config/v2", cc.Dump)
		...
		authv2.Use(auth.RequiresRunRole).POST("/replay_from_block/:number", rc.ReplayFromBlock)

		csakc := CSAKeysController{app}
		authv2.GET("/keys/csa", csakc.Index)
		authv2.Use(auth.RequiresEditRole).POST("/keys/csa", csakc.Create)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/csa/import", csakc.Import)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/csa/export/:ID", csakc.Export)

		ekc := ETHKeysController{app}
		authv2.GET("/keys/eth", ekc.Index)
		authv2.Use(auth.RequiresEditRole).POST("/keys/eth", ekc.Create)
		authv2.Use(auth.RequiresAdminRole).PUT("/keys/eth/:keyID", ekc.Update)
		authv2.Use(auth.RequiresAdminRole).DELETE("/keys/eth/:keyID", ekc.Delete)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/eth/import", ekc.Import)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/eth/export/:address", ekc.Export)

		ocrkc := OCRKeysController{app}
		authv2.GET("/keys/ocr", ocrkc.Index)
		authv2.Use(auth.RequiresEditRole).POST("/keys/ocr", ocrkc.Create)
		authv2.Use(auth.RequiresAdminRole).DELETE("/keys/ocr/:keyID", ocrkc.Delete)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/ocr/import", ocrkc.Import)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/ocr/export/:ID", ocrkc.Export)

		ocr2kc := OCR2KeysController{app}
		authv2.GET("/keys/ocr2", ocr2kc.Index)
		authv2.Use(auth.RequiresEditRole).POST("/keys/ocr2/:chainType", ocr2kc.Create)
		authv2.Use(auth.RequiresAdminRole).DELETE("/keys/ocr2/:keyID", ocr2kc.Delete)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/ocr2/import", ocr2kc.Import)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/ocr2/export/:ID", ocr2kc.Export)

		p2pkc := P2PKeysController{app}
		authv2.GET("/keys/p2p", p2pkc.Index)
		authv2.Use(auth.RequiresEditRole).POST("/keys/p2p", p2pkc.Create)
		authv2.Use(auth.RequiresAdminRole).DELETE("/keys/p2p/:keyID", p2pkc.Delete)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/p2p/import", p2pkc.Import)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/p2p/export/:ID", p2pkc.Export)
		...
		for _, keys := range []struct {
			...
			authv2.Use(auth.RequiresEditRole).POST("/keys/"+keys.path, keys.kc.Create)
			authv2.Use(auth.RequiresAdminRole).DELETE("/keys/"+keys.path+"/:keyID", keys.kc.Delete)
			authv2.Use(auth.RequiresAdminRole).POST("/keys/"+keys.path+"/import", keys.kc.Import)
			authv2.Use(auth.RequiresAdminRole).POST("/keys/"+keys.path+"/export/:ID", keys.kc.Export)
		}

		vrfkc := VRFKeysController{app}
		authv2.GET("/keys/vrf", vrfkc.Index)
		authv2.Use(auth.RequiresEditRole).POST("/keys/vrf", vrfkc.Create)
		authv2.Use(auth.RequiresAdminRole).DELETE("/keys/vrf/:keyID", vrfkc.Delete)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/vrf/import", vrfkc.Import)
		authv2.Use(auth.RequiresAdminRole).POST("/keys/vrf/export/:keyID", vrfkc.Export)

		jc := JobsController{app}
		authv2.GET("/jobs", paginatedRequest(jc.Index))
		authv2.GET("/jobs/:ID", jc.Show)
		authv2.Use(auth.RequiresEditRole).POST("/jobs", jc.Create)
		authv2.Use(auth.RequiresEditRole).DELETE("/jobs/:ID", jc.Delete)
		...
		authv2.Use(auth.RequiresEditRole).DELETE("/pipeline/job_spec_errors/:ID", psec.Destroy)

		lgc := LogController{app}
		authv2.GET("/log", lgc.Get)
		authv2.Use(auth.RequiresAdminRole).PATCH("/log", lgc.Patch)
		...
		for _, chain := range []struct {
			...
			chains.Use(auth.RequiresEditRole).POST(chain.path, chain.cc.Create)
			chains.GET(chain.path+"/:ID", chain.cc.Show)
			chains.Use(auth.RequiresEditRole).PATCH(chain.path+"/:ID", chain.cc.Update)
			chains.Use(auth.RequiresEditRole).DELETE(chain.path+"/:ID", chain.cc.Delete)
		}
		...
		for _, chain := range []struct {
			...
			if chain.path == "evm" {
				...
				nodes.Use(auth.RequiresEditRole).POST("", chain.nc.Create)
				nodes.Use(auth.RequiresEditRole).DELETE("/:ID", chain.nc.Delete)
			}
			nodes.GET(chain.path, paginatedRequest(chain.nc.Index))
			chains.GET(chain.path+"/:ID/nodes", paginatedRequest(chain.nc.Index))
			nodes.Use(auth.RequiresEditRole).POST(chain.path, chain.nc.Create)
			nodes.Use(auth.RequiresEditRole).DELETE(chain.path+"/:ID", chain.nc.Delete)
		}

		efc := EVMForwardersController{app}
		authv2.GET("/nodes/evm/forwarders", paginatedRequest(efc.Index))
		authv2.Use(auth.RequiresEditRole).POST("/nodes/evm/forwarders", efc.Create)
		authv2.Use(auth.RequiresEditRole).DELETE("/nodes/evm/forwarders/:fwdID", efc.Delete)
	...
	userOrEI.Use(auth.RequiresRunRole).POST("/jobs/:ID/runs", prc.Create)
}
