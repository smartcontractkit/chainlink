...
// GetTestBinaryPath returns a path to a temp file containing the raw WASM binary for the
// given package path. The binary is built/cached via GetTestBinary.
func GetTestBinaryPath(tb testing.TB, outputPath string) string {
	tb.Helper()
	binary := GetTestBinary(tb, outputPath, false)
	dir := tb.TempDir()
	path := filepath.Join(dir, "test.wasm")
	require.NoError(tb, os.WriteFile(path, binary, 0o600))
	return path
}
...
func GetTestBinary(tb testing.TB, outputPath string, compress bool) []byte {
	...
	binary, err := wasmbuild.Build(context.Background(), wasmbuild.Config{
