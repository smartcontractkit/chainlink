...
func randomiseAndRestartSequences(db sdlDBCommon) error {
	...
	for seqRows.Next() {
		...
		if err = seqRows.Scan(&sequenceSchema, &sequenceName); err != nil {
			return fmt.Errorf("%s: failed to setup pgtest, error scanning sequence rows: %s", failedToSetupTestPGError{}, err)
		}
		if _, err = db.Exec(fmt.Sprintf("ALTER SEQUENCE %s.%s RESTART WITH %d", sequenceSchema, sequenceName, rand.Intn(10000))); err != nil {
			return fmt.Errorf("%s: failed to setup pgtest, failed to alter %s sequence: %w", failedToSetupTestPGError{}, sequenceName, err)
		}
	}
