...
// RunWebhookJobV2 provides a mock function with given fields: jobUUID, pipelineInputs, meta
func (_m *Application) RunWebhookJobV2(jobUUID common.Hash, pipelineInputs []pipeline.Result, meta pipeline.JSONSerializable) (int64, error) {
	ret := _m.Called(jobUUID, pipelineInputs, meta)

	var r0 int64
	if rf, ok := ret.Get(0).(func(common.Hash, []pipeline.Result, pipeline.JSONSerializable) int64); ok {
		r0 = rf(jobUUID, pipelineInputs, meta)
	} else {
		r0 = ret.Get(0).(int64)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(common.Hash, []pipeline.Result, pipeline.JSONSerializable) error); ok {
		r1 = rf(jobUUID, pipelineInputs, meta)
	} else {
		r1 = ret.Error(1)
	}
