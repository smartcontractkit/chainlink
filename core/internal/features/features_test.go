...
observationSource   = """
    ...
    parse -> submit
"""
    `, jobUUID, eiName, testcommon.MustJSONMarshal(t, eiSpec))

		_, err := webhook.ValidatedWebhookSpec(ctx, tomlSpec, app.GetExternalInitiatorManager())
		require.NoError(t, err)
		job := cltest.CreateJobViaWeb(t, app, []byte(testcommon.MustJSONMarshal(t, web.CreateJobRequest{TOML: tomlSpec})))
		jobID = job.ID
	...
	t.Run("calling webhook_spec with non-matching external_initiator_id returns unauthorized", func(t *testing.T) {
		...
		body := testcommon.MustJSONMarshal(t, eiRequest)
	...
	t.Run("calling webhook_spec with matching external_initiator_id works", func(t *testing.T) {
		...
		_ = cltest.CreateJobRunViaExternalInitiatorV2(t, app, jobUUID, *eia, testcommon.MustJSONMarshal(t, eiRequest))
...
func TestIntegration_DirectRequest(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			j := cltest.CreateJobViaWeb(t, app, []byte(testcommon.MustJSONMarshal(t, web.CreateJobRequest{TOML: spec})))
			...
			jobSingleWord := cltest.CreateJobViaWeb(t, app, []byte(testcommon.MustJSONMarshal(t, web.CreateJobRequest{TOML: singleWordSpec})))
...
`
		...
		j := cltest.CreateJobViaWeb(t, app, []byte(testcommon.MustJSONMarshal(t, web.CreateJobRequest{TOML: tomlSpec})))
