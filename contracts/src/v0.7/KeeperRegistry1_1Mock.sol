...
contract KeeperRegistry1_1Mock {
    event ConfigSet(uint32 paymentPremiumPPB, uint24 blockCountPerTurn, uint32 checkGasLimit, uint24 stalenessSeconds, uint16 gasCeilingMultiplier, uint256 fallbackGasPrice, uint256 fallbackLinkPrice);
    ...
    function emitConfigSet(uint32 paymentPremiumPPB, uint24 blockCountPerTurn, uint32 checkGasLimit, uint24 stalenessSeconds, uint16 gasCeilingMultiplier, uint256 fallbackGasPrice, uint256 fallbackLinkPrice) public {
        emit ConfigSet(paymentPremiumPPB, blockCountPerTurn, checkGasLimit, stalenessSeconds, gasCeilingMultiplier, fallbackGasPrice, fallbackLinkPrice);
    }
    ...
    function emitUpkeepPerformed(uint256 id, bool success, address from, uint96 payment, bytes memory performData) public {
    ...
    function setUpkeep(uint256 id, address _target, uint32 _executeGas, uint96 _balance, address _admin, uint64 _maxValidBlocknumber, address _lastKeeper, bytes memory _checkData) external {
    ...
    function setCheckUpkeepData(uint256 id, bytes memory performData, uint256 maxLinkPayment, uint256 gasLimit, uint256 adjustedGasWei, uint256 linkEth) external {
