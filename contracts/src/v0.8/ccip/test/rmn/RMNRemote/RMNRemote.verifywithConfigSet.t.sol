...
contract RMNRemote_verify_withConfigSet is RMNRemoteSetup {
  function setUp() public override {
    ...
    RMNRemote.Config memory config =
      RMNRemote.Config({rmnHomeContractConfigDigest: _randomBytes32(), signers: s_signers, fObserve: 3});
  ...
  function test_verify_RevertWhen_ThresholdNotMet() public {
    RMNRemote.Config memory config =
      RMNRemote.Config({rmnHomeContractConfigDigest: _randomBytes32(), signers: s_signers, fObserve: 2}); // 3 = f+1 sigs required
    s_rmnRemote.setConfig(config);
