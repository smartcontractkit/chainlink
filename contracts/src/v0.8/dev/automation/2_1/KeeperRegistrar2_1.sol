...
import "../../interfaces/automation/2_1/AutomationRegistryInterface2_1.sol";
 ...
 */
contract KeeperRegistrar2_1 is TypeAndVersionInterface, ConfirmedOwner, ERC677ReceiverInterface {
  /**
  ...
  /**
   * @notice versions:
   * - KeeperRegistrar 2.1.0: Update interface to work with registry v2.1
   ...
   */
  string public constant override typeAndVersion = "KeeperRegistrar 2.1.0";
  ...
  /**
   * @notice Allows external users to register upkeeps; assumes amount is approved for transfer by the LINK contract
   * @param requestParams struct of all possible registration parameters
   */
  function registerUpkeep(RegistrationParams calldata requestParams) external returns (uint256, address) {
    ...
    returns (uint256 upkeepId, address forwarder)
  {
    ...
    if (_shouldAutoApprove(config, sender)) {
      ...
      (upkeepId, forwarder) = _approve(params, hash);
    } else {
    ...
    return (upkeepId, forwarder);
  }

  /**
   * @dev register upkeep on KeeperRegistry contract and emit RegistrationApproved event
   */
  function _approve(RegistrationParams memory params, bytes32 hash) private returns (uint256, address) {
    AutomationRegistryBaseInterface keeperRegistry = s_config.keeperRegistry;

    // register upkeep
    (uint256 upkeepId, address forwarder) = keeperRegistry.registerUpkeep(
      params.upkeepContract,
      params.gasLimit,
      params.adminAddress,
      params.checkData,
