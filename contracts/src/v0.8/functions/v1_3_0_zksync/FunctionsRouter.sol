...
contract FunctionsRouter is IFunctionsRouter, FunctionsSubscriptions, Pausable, ITypeAndVersion, ConfirmedOwner {
  ...
  string public constant override typeAndVersion = "Functions Router v1.3.0";
  ...
  function _callback(
    ...
    (success, returnData) = GAS_BOUND_CALLER.delegatecall{gas: callbackGasLimit}(
      abi.encodeWithSelector(IGasBoundCaller.gasBoundCall.selector, client, callbackGasLimit, encodedCallback)
    );
