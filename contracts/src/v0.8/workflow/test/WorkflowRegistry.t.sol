...
contract WorkflowRegistryTest is Test {
  ...
  function _allowAccessAndRegisterWorkflow(
    ...
    s_registry.registerWorkflow(
      workflowName, workflowID, s_donID, initialStatus, s_testBinaryURL, s_testConfigURL, s_testSecretsURL
    );
  }
  ...
  function testLockRegistry() public {
    ...
    _allowAccessAndRegisterWorkflow(
      s_authorizedUser, s_workflowName1, s_workflowID1, WorkflowRegistry.WorkflowStatus.ACTIVE
    );
  ...
  function testRegisterWorkflowFailsForNotAuthorizedAddressOrForNotAllowedDONId() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(DONAccessControl.AddressNotAuthorized.selector, s_donID, s_unauthorizedUser));
    s_registry.registerWorkflow(
      s_workflowName1,
      s_workflowID1,
      s_donID,
  ...
  function testActivateWorkflow() public {
    // create a new workflow
    _allowAccessAndRegisterWorkflow(
      s_authorizedUser, s_workflowName1, s_workflowID1, WorkflowRegistry.WorkflowStatus.PAUSED
    );
  ...
  function testGetWorkflowMetadataListByOwner() public {
    ...
    _allowAccessAndRegisterWorkflow(
      s_authorizedUser, s_workflowName2, s_workflowID2, WorkflowRegistry.WorkflowStatus.PAUSED
    );
    _allowAccessAndRegisterWorkflow(
      s_authorizedUser, "workflow3", keccak256(abi.encodePacked("workflow3")), WorkflowRegistry.WorkflowStatus.ACTIVE
    );
    _allowAccessAndRegisterWorkflow(
      s_authorizedUser, "workflow4", keccak256(abi.encodePacked("workflow4")), WorkflowRegistry.WorkflowStatus.ACTIVE
    );
    _allowAccessAndRegisterWorkflow(
      s_authorizedUser, "workflow5", keccak256(abi.encodePacked("workflow5")), WorkflowRegistry.WorkflowStatus.ACTIVE
    );

    // Retrieve the list of workflows for the s_owner
    WorkflowRegistry.WorkflowMetadata[] memory workflows =
      s_registry.getWorkflowMetadataListByOwner(s_authorizedUser, 0, 10);
    ...
    WorkflowRegistry.WorkflowMetadata[] memory firstPage =
      s_registry.getWorkflowMetadataListByOwner(s_authorizedUser, 0, 2);
    ...
    WorkflowRegistry.WorkflowMetadata[] memory secondPage =
      s_registry.getWorkflowMetadataListByOwner(s_authorizedUser, 2, 2);
    ...
    WorkflowRegistry.WorkflowMetadata[] memory lastPage =
      s_registry.getWorkflowMetadataListByOwner(s_authorizedUser, 4, 2);
    assertEq(lastPage.length, 1);
    assertEq(lastPage[0].workflowName, "workflow5");

    // Pagination: Request page beyond available items
    WorkflowRegistry.WorkflowMetadata[] memory emptyPage =
      s_registry.getWorkflowMetadataListByOwner(s_authorizedUser, 6, 2);
    assertEq(emptyPage.length, 0);

    // Pagination: Request all items at once
    WorkflowRegistry.WorkflowMetadata[] memory allItems =
      s_registry.getWorkflowMetadataListByOwner(s_authorizedUser, 0, 10);
  ...
  function testGetWorkflowMetadataListByDON() public {
    ...
    _allowAccessAndRegisterWorkflow(
      s_authorizedUser, "workflow4", keccak256(abi.encodePacked("workflow4")), WorkflowRegistry.WorkflowStatus.PAUSED
    );
    _allowAccessAndRegisterWorkflow(
      s_authorizedUser, "workflow5", keccak256(abi.encodePacked("workflow5")), WorkflowRegistry.WorkflowStatus.ACTIVE
    );
    ...
    WorkflowRegistry.WorkflowMetadata[] memory emptyDON =
      s_registry.getWorkflowMetadataListByDON(nonExistentDonID, 0, 10);
