 ...
 * in `getiPoRAddressList()`.
 */
interface iPoRAddressList {
  /// @notice Get total number of addresses in the list.
  function getiPoRAddressListLength() external view returns (uint256);
   ...
   * @param endIndex The index of the last address in the batch. If `endIndex > getiPoRAddressListLength()-1`,
   * endIndex need to default to `getiPoRAddressListLength()-1`.
   * @return Array of addresses as strings.
   */
  function getiPoRAddressList(uint256 startIndex, uint256 endIndex) external view returns (string[] memory);
}
