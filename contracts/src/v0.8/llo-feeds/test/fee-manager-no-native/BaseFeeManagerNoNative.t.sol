...
contract BaseFeeManagerNoNativeTest is Test {
  ...
  function processFee(bytes memory payload, address subscriber, address feeAddress) public {
    //record the current address and switch to the recipient
    address originalAddr = msg.sender;
    changePrank(subscriber);

    //process the fee
    feeManagerProxy.processFee(payload, abi.encode(feeAddress));
  ...
  function processFee(bytes[] memory payloads, address subscriber, address feeAddress) public {
    ...
    //process the fee
    feeManagerProxy.processFeeBulk(payloads, abi.encode(feeAddress));
