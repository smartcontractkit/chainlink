...
//        );
    ...
    /// @inheritdoc IChannelVerifier
    function latestConfigDigestAndEpoch() external view override returns (bool scanLogs, bytes32 configDigest, uint32 epoch) {
        return (false, s_feedVerifierState.latestConfigDigest, s_feedVerifierState.latestEpoch);
    }

    /// @inheritdoc IChannelVerifier
    function latestConfigDetails() external view override returns (uint32 configCount, uint32 blockNumber, bytes32 configDigest) {
        return (
            s_feedVerifierState.configCount,
            s_feedVerifierState.latestConfigBlockNumber,
