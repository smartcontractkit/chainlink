...
contract KeystoneFeedsPermissionHandlerTestSetup is Test {
  ...
  function _createPermissions() internal pure returns (KeystoneFeedsPermissionHandler.Permission[] memory) {
    ...
    permissions[0] = KeystoneFeedsPermissionHandler.Permission(FORWARDER_1, WORKFLOW_NAME_1, WORKFLOW_OWNER_1, true);
    permissions[1] = KeystoneFeedsPermissionHandler.Permission(FORWARDER_2, WORKFLOW_NAME_2, WORKFLOW_OWNER_2, true);
    return permissions;
  }
...
contract KeystoneFeeds_validatePermissions is KeystoneFeedsPermissionHandlerTestSetup {
  function test__validatePermissions() public {
    ...
    permissionHandlerHelper.validateReportPermission(permissions[0].workflowOwner, permissions[0].workflowName);

    vm.prank(FORWARDER_2);
    permissionHandlerHelper.validateReportPermission(permissions[1].workflowOwner, permissions[1].workflowName);
  }
