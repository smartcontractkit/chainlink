...
import {IGasModule} from "./interfaces/IGasModule.sol";

contract VRFCoordinatorV2_5Optimism is VRFCoordinatorV2_5 {

  IGasModule public s_optimismGasModule;
  constructor(address blockhashStore, address module) VRFCoordinatorV2_5(blockhashStore) {
    s_optimismGasModule = IGasModule(module);
  }

  function setOptimismGasModule(address module) external onlyOwner {
    s_optimismGasModule = IGasModule(module);
  }
  ...
  function getBlockhash(uint64 blockNum) internal view override returns (bytes32) {
    return blockhash(blockNum);
  }

  function getTxL1GasFees(bytes calldata data) internal view override returns (uint256) {
    return s_optimismGasModule.getTxL1GasFees(data);
  }
}
...
//contract VRFCoordinatorV2_5Optimism is VRF, SubscriptionAPI, IVRFCoordinatorV2Plus {
...
//  IOptimismHelper public s_optimismHelper;

//  event FallbackWeiPerUnitLinkUsed(uint256 requestId, int256 fallbackWeiPerUnitLink);

//  constructor(address blockhashStore, IOptimismHelper helper) SubscriptionAPI() {
//    BLOCKHASH_STORE = BlockhashStoreInterface(blockhashStore);
//    s_optimismHelper = helper;
//  }
...
//    uint256 l1CostWei = s_optimismHelper.getTxL1GasFees(msg.data);
