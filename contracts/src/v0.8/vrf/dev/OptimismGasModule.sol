...
contract OptimismGasModule is IGasModule, ConfirmedOwner {
  ...
  constructor(uint8 l1_gas_calculation_mode) ConfirmedOwner(msg.sender) {
    s_l1_gas_calculation_mode = l1_gas_calculation_mode;
  }

  function setL1GasCalculationMode(uint8 l1_gas_calculation_mode) external onlyOwner {
    if (l1_gas_calculation_mode > 2) {
      revert InvalidMode();
    }
    s_l1_gas_calculation_mode = l1_gas_calculation_mode;
  }
