...
contract AutomationRegistry2_2 is AutomationRegistryBase2_2, OCR2Abstract, Chainable, IERC677Receiver {
  ...
  /**
   * @notice versions:
   * - AutomationRegistry 2.1.0: introduces support for log, cron, and ready triggers
                           : removes the need for "wrapped perform data"
   * - AutomationRegistry 2.0.2: pass revert bytes as performData when target contract reverts
   *                       : fixes issue with arbitrum block number
   *                       : does an early return in case of stale report instead of revert
   * - AutomationRegistry 2.0.1: implements workaround for buggy migrate function in 1.X
   * - AutomationRegistry 2.0.0: implement OCR interface
   * - AutomationRegistry 1.3.0: split contract into Proxy and Logic
   *                       : account for Arbitrum and Optimism L1 gas fee
   *                       : allow users to configure upkeeps
   * - AutomationRegistry 1.2.0: allow funding within performUpkeep
   *                       : allow configurable registry maxPerformGas
   *                       : add function to let admin change upkeep gas limit
   *                       : add minUpkeepSpend requirement
   *                       : upgrade to solidity v0.8
   * - AutomationRegistry 1.1.0: added flatFeeMicroLink
   * - AutomationRegistry 1.0.0: initial release
   */
  string public constant override typeAndVersion = "AutomationRegistry 2.1.0";
