...
describe("UpkeepRegistrationRequests", () => {
  ...
  describe("#approve", () => {
    ...
    it("reverts if not called by the owner", async () => {
      const tx = registrar.connect(stranger).approve(upkeepName, mock.address, executeGas, await admin.getAddress(), emptyBytes, hash)
      await evmRevert(tx, "Only callable by owner");
    })

    it("reverts if the hash does not exist", async () => {
      const tx = registrar.connect(registrarOwner).approve(upkeepName, mock.address, executeGas, await admin.getAddress(), emptyBytes, "0x000000000000000000000000322813fd9a801c5507c9de605d63cea4f2ce6c44")
      await evmRevert(tx, errorMsgs.requestNotFound);
    })

    it("reverts if any member of the payload changes", async () => {
      let tx = registrar.connect(registrarOwner).approve(upkeepName, ethers.Wallet.createRandom().address, executeGas, await admin.getAddress(), emptyBytes, hash)
      await evmRevert(tx, errorMsgs.hashPayload);
      tx = registrar.connect(registrarOwner).approve(upkeepName, mock.address, 10000, await admin.getAddress(), emptyBytes, hash)
      await evmRevert(tx, errorMsgs.hashPayload);
      tx = registrar.connect(registrarOwner).approve(upkeepName, mock.address, executeGas, ethers.Wallet.createRandom().address, emptyBytes, hash)
      await evmRevert(tx, errorMsgs.hashPayload);
      tx = registrar.connect(registrarOwner).approve(upkeepName, mock.address, executeGas, await admin.getAddress(), "0x1234", hash)
      await evmRevert(tx, errorMsgs.hashPayload);
    })

    it("approves an existing registration request", async () => {
      const tx = await registrar.connect(registrarOwner).approve(upkeepName, mock.address, executeGas, await admin.getAddress(), emptyBytes, hash)
      await expect(tx).to.emit(registrar, "RegistrationApproved")
    })

    it("deletes the request afterwards / reverts if the request DNE", async () => {
      await registrar.connect(registrarOwner).approve(upkeepName, mock.address, executeGas, await admin.getAddress(), emptyBytes, hash)
      const tx = registrar.connect(registrarOwner).approve(upkeepName, mock.address, executeGas, await admin.getAddress(), emptyBytes, hash)
      await evmRevert(tx, errorMsgs.requestNotFound);
    })
  })
  ...
  describe("#cancel", () => {
    ...
    it("reverts if the hash does not exist", async () => {
      const tx = registrar.connect(registrarOwner).cancel("0x000000000000000000000000322813fd9a801c5507c9de605d63cea4f2ce6c44")
    ...
    it("deletes the request hash", async () => {
      ...
      await evmRevert(tx, errorMsgs.requestNotFound);
      tx = registrar.connect(registrarOwner).approve(upkeepName, mock.address, executeGas, await admin.getAddress(), emptyBytes, hash)
      await evmRevert(tx, errorMsgs.requestNotFound);
    })
  })
});
