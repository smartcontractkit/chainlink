...
before(async () => {
  ...
  controllerFactory = await ethers.getContractFactory("SimpleWriteAccessController", personas.Carol);
});
...
describe("SimpleWriteAccessController", () => {
  ...
  describe("#hasAccess", () => {
    ...
    it("blocks unauthorized calls originating from different accounts", async () => {
      assert.isFalse(await controller.connect(personas.Carol).hasAccess(await personas.Eddy.getAddress(), "0x00"));
      assert.isFalse(await controller.connect(personas.Eddy).hasAccess(await personas.Carol.getAddress(), "0x00"));
    });
  });
  ...
  describe("#addAccess", () => {
    describe("when called by a non-owner", () => {
      it("reverts", async () => {
        await expect(controller.connect(personas.Eddy).addAccess(await personas.Eddy.getAddress())).to.be.revertedWith(
    ...
    describe("when called by the owner", () => {
      let tx: Transaction;
      beforeEach(async () => {
        assert.isFalse(await controller.hasAccess(await personas.Eddy.getAddress(), "0x00"));
        tx = await controller.addAccess(await personas.Eddy.getAddress());
      });

      it("adds the address to the controller", async () => {
        assert.isTrue(await controller.hasAccess(await personas.Eddy.getAddress(), "0x00"));
      });
  ...
  describe("#removeAccess", () => {
    ...
    describe("when called by a non-owner", () => {
      it("reverts", async () => {
        await expect(
          controller.connect(personas.Eddy).removeAccess(await personas.Eddy.getAddress()),
  ...
  describe("#disableAccessCheck", () => {
    describe("when called by a non-owner", () => {
      it("reverts", async () => {
        await expect(controller.connect(personas.Eddy).disableAccessCheck()).to.be.revertedWith(
          "Only callable by owner",
        );
    ...
    describe("when called by the owner", () => {
      ...
      it("allows users without access", async () => {
        assert.isTrue(await controller.hasAccess(await personas.Ned.getAddress(), "0x00"));
      });
  ...
  describe("#enableAccessCheck", () => {
    describe("when called by a non-owner", () => {
      it("reverts", async () => {
        await expect(controller.connect(personas.Eddy).enableAccessCheck()).to.be.revertedWith(
          "Only callable by owner",
        );
      });
    });
    ...
    describe("when called by the owner", () => {
      ...
      it("does not allow users without access", async () => {
        assert.isFalse(await controller.hasAccess(await personas.Ned.getAddress(), "0x00"));
      });
