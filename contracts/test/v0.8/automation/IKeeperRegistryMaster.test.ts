 ...
 */
function assertSatisfiesInterface(contractABI: any[], interfaceABI: any[]) {
  const contractEntries = new Set()
  for (const entry of contractABI) {
    if (entry.type != 'function') continue
    contractEntries.add(entryID(entry))
  }
  for (const entry of interfaceABI) {
    if (entry.type != 'function') continue
    const id = entryID(entry)
    assert(
      contractEntries.has(id),
      `contract is missing interface entry: ${JSON.stringify(entry)}`,
    )
  }
}
