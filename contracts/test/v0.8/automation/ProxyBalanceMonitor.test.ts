...
import { ProxyBalanceMonitorExposed, LinkToken } from '../../../typechain'
...
let bm: ProxyBalanceMonitorExposed
let lt: LinkToken
...
describe('ProxyBalanceMonitor', () => {
  ...
  describe('setWatchList() / addToWatchList() / getWatchList() / getAccountInfo()', () => {
    it('Should allow owner to set the watchlist', async () => {
      ...
      tx = await bm
        .connect(owner)
        .addToWatchList([watchAddress2], [oneLINK], [twoLINK])
      await tx.wait()
      watchList = await bm.getWatchList()
    ...
    it('Should not allow a topUpLevel less than or equal to minBalance in the watchlist', async () => {
      let tx = bm
        .connect(owner)
        .setWatchList(
          [watchAddress1, watchAddress2, watchAddress1],
    ...
    it('Should not allow larger than maximum watchlist size', async () => {
      ...
      tx = bm
        .connect(owner)
        .addToWatchList(watchlist[0], watchlist[1], watchlist[2])
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
    })
    ...
    it('Should not allow strangers to set the watchlist', async () => {
      ...
      const addTxStranger = bm
        .connect(stranger)
        .addToWatchList([watchAddress1], [oneLINK], [twoLINK])
    ...
    it('Should revert if the list lengths differ', async () => {
      ...
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
      tx = bm.connect(owner).addToWatchList([watchAddress1], [], [twoLINK])
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
      tx = bm.connect(owner).addToWatchList([watchAddress1], [oneLINK], [])
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
      tx = bm.connect(owner).addToWatchList([], [oneLINK], [twoLINK])
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
    })
