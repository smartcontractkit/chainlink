...
import {FunctionsDON} from "../../../../src/v0.8/dev/functions/FunctionsDON.sol";
...
contract FunctionsDONSetup is BaseTest {
  bytes constant DATA = abi.encode("bytes");
  address registryAddress = makeAddr("Registry");

  FunctionsDON s_oracle;

  function setUp() public virtual override {
    BaseTest.setUp();

    s_oracle = new FunctionsDON();
  }
}

contract FunctionsDON_typeAndVersion is FunctionsDONSetup {
  function testTypeAndVersionSuccess() public {
    assertEq(s_oracle.typeAndVersion(), "FunctionsDON 0.0.0");
  }
}

contract FunctionsDON_setDONPublicKey is FunctionsDONSetup {
  ...
  function testEmptyPublicKeyReverts() public {
    ...
    vm.expectRevert(FunctionsDON.EmptyPublicKey.selector);
    s_oracle.setDONPublicKey(donPublicKey);
  }
...
contract FunctionsDON_setRegistry is FunctionsDONSetup {
  ...
  function testEmptyPublicKeyReverts() public {
    ...
    vm.expectRevert(FunctionsDON.EmptyBillingRegistry.selector);
    s_oracle.setRegistry(registryAddress);
  }
...
contract FunctionsDON_sendRequest is FunctionsDONSetup {
  ...
  function setUp() public virtual override {
    ...
    s_registry = new FunctionsBillingRegistry(makeAddr("Link Token"), makeAddr("Link Eth"));
    s_oracle.setRegistry(address(s_registry));
    s_oracle.deactivateAuthorizedReceiver();
  }

  event DONRequest(bytes32 requestId, bytes data);
  ...
  function testEmptyRequestDataReverts() public {
    ...
    vm.expectRevert(FunctionsDON.EmptyRequestData.selector);
