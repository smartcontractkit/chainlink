...
contract BaseTest is Test {
    ...
    function _getSigners(uint256 numSigners)
        internal
        view
    ...
    function _getSignerAddresses(Signer[] memory signers)
        internal
        view
        ...
        )
    {
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(keccak256(abi.encode(report)), reportContext)
        );

        for (uint256 i = 0; i < signers.length; i++) {
            (uint8 v, bytes32 r, bytes32 s) = vm.sign(
                signers[i].mockPrivateKey,
                hash
            );
            rs[i] = r;
            ss[i] = s;
    ...
    function _generateEncodedBlob(
        ...
        (
            ...
            bytes32[] memory ss,
            bytes32 rawVs
        ) = _generateSignerSignatures(report, reportContext, signers);
