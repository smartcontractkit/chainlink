...
contract VerifierSetConfigTest is BaseTest {
    ...
    function test_revertsIfSetWithTooManySigners() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Verifier.ExcessSigners.selector,
                signers.length,
                MAX_ORACLES
            )
        );
    ...
    function test_revertsIfFaultToleranceIsZero() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                Verifier.FaultToleranceMustBePositive.selector
            )
        );
        Signer[] memory signers = _getSigners(MAX_ORACLES);
        s_verifier.setConfig(
    ...
    function test_revertsIfNotEnoughSigners() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Verifier.InsufficientSigners.selector,
                signers.length,
                FAULT_TOLERANCE * 3 + 1
            )
        );
    ...
    function test_correctlyUpdatesTheConfig() public {
        ...
        ) = s_verifier.latestConfigDetails(FEED_ID);
        assertEq(configCount, 1);
        assertEq(blockNumber, block.number);
        assertEq(configDigest, expectedConfigDigest);

        (bool scanLogs, bytes32 configDigestTwo, uint32 epoch) = s_verifier
            .latestConfigDigestAndEpoch(FEED_ID);
...
contract VerifierSetConfigWhenThereAreMultipleDigestsTest is
