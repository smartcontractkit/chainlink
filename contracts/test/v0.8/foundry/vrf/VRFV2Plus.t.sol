...
contract VRFV2Plus is BaseTest {
  ...
  function setUp() public override {
    ...
    // 1 LINK == 10 USD
    s_linkUsdFeed = new MockV3Aggregator(8, 10e8);
    // 1 ETH == 1000 USD
    s_ethUsdFeed = new MockV3Aggregator(8, 1000e8);
    s_registry = new VRFV2PlusPriceRegistry(address(s_linkEthFeed), address(s_linkUsdFeed), address(s_ethUsdFeed));
    // function setConfig(uint32 stalenessSeconds, uint32 gasAfterPaymentCalculation,
    // int256 fallbackWeiPerUnitLink, int256 fallbackUSDPerUnitEth, int256 fallbackUSDPerUnitLink,
    // uint40 fulfillmentFlatFeeLinkUSD, uint40 fulfillmentFlatFeeEthUSD)
    s_registry.setConfig(
      30 * 24 * 60 * 60, // 30 days
      35_000, // gasAfterPaymentCalculation
      1e17, // fallbackWeiPerUnitLink
      1000e8, // fallbackUSDPerUnitEth
      10e8, // fallbackUSDPerUnitLink
      0, // fulfillmentFlatFeeLinkUSD
      0 // fulfillmentFlatFeeEthUSD
    );
  ...
  function testRequestAndFulfillRandomWordsNative() public {
    ...
    /*
       go run . generate-proof-v2-plus \
        ...
        -pre-seed 31200903121010798535219165048625353630703136274779830065573178128585886566241 \
    ...
    VRF.Proof memory proof = VRF.Proof({
      ...
      gamma: [
        63973143266433164955039661811601935533678330920314736450091156112128975681283,
        3330225865100436372999120447699722202923082123325509718726283167366591347443
      ],
      c: 60702863572285806626512350066433518132454756566574305971412675143394430036892,
      s: 98951919331174466741712692596833476017102384606258177548884164834535187008848,
      seed: 31200903121010798535219165048625353630703136274779830065573178128585886566241,
      uWitness: 0xA158912CF9FD197B84c6a467287c5418d3bAaE38,
      cGammaWitness: [
        69637319982917939312132600268750109585602010842015118650819322031676808525505,
        20744775735057409738102200839884027194211758513553341743582159736526494047493
      ],
      sHashWitness: [
        113904814701759783470539051163744082252479388386130071640441241159913593050738,
        1179648389339511951720059457509162637650959253462756044431460247900874679748
      ],
      zInv: 60166743419064786579997015973046865218693030350368400566341796971041955509299
    });
    ...
    assertApproxEqAbs(ethBalanceAfter, ethBalanceBefore - 120_000, 20_000);
  }
  ...
  function testRequestAndFulfillRandomWordsLINK() public {
    ...
    /*
        go run . generate-proof-v2-plus \
        -key-hash 0x9f2353bde94264dbc3d554a94cceba2d7d2b4fdce4304d3e09a1fea9fbeb1528 \
        -pre-seed 44286195792738184726776539593469714361356614696379778388164352292003492132421 \
    ...
    */
    VRF.Proof memory proof = VRF.Proof({
      ...
      gamma: [
        20987581850453999220873699963804999713056919763262632513787934443697226626288,
        54966307414754059586320689381225420871591994778300914604818375058022014906427
      ],
      c: 89061452938482507916960316629823603496184815836131040096924549179760611446163,
      s: 83795195309949985751629897813308344155472859395239285492481267581027377379354,
      seed: 44286195792738184726776539593469714361356614696379778388164352292003492132421,
      uWitness: 0x8955E83aE440808AF635d10671c3Ca20A47feEAf,
      cGammaWitness: [
        78144744549629617990172662000985783604471507946953200314010948162686902357323,
        66151613324909271213133663538453484826380633076514118105651132322310836673522
      ],
      sHashWitness: [
        90594848722948348875928972916896688803901769347282026922899060504454809364235,
        34816550232574512073831162959332028098378046545028184636468847033682205021523
      ],
      zInv: 38847007285889730891485401036038301805348834750242529992698998206415135691177
    });
    VRFCoordinatorV2Plus.RequestCommitment memory rc = VRFCoordinatorV2Plus.RequestCommitment({
      blockNum: requestBlock,
      subId: subId,
    ...
    // ...
    // billed_fee = paymentNoFee + fulfillmentFlatFeeLinkUSDInJuels
