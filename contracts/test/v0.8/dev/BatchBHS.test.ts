...
describe('BatchBHS', () => {
	...
	describe('#store', () => {
		...
		it('reverts if block number is too far back for blockhash instruction', async () => {
			...
			expect(batchBHS.connect(owner).store([1, 2, 3])).to.be.revertedWith('blockhash(n) failed')
		})
	})
	...
	describe('#getBlockhashes', () => {
		it('fetches blockhashes of a batch of block numbers', async () => {
			...
			const actualBlockhashes = await batchBHS.connect(owner).getBlockhashes(range(numBlocks, bottomBlock))
			let expectedBlockhashes = []
			for (let i = bottomBlock; i < bottomBlock + numBlocks; i++) {
				const block = await ethers.provider.send('eth_getBlockByNumber', ['0x' + i.toString(16), false])
	...
	describe('#storeVerifyHeader', () => {
		it('stores batches of blocknumbers using storeVerifyHeader', async () => {
			...
			const blockNumbers = range(numBlocks + 1, startBlock - numBlocks).reverse()
			let blockHeaders = []
			let expectedBlockhashes = []
			for (let i of blockNumbers) {
				const block = await ethers.provider.send('eth_getBlockByNumber', ['0x' + (i + 1).toString(16), false])
				...
				expectedBlockhashes.push(
					(await ethers.provider.send('eth_getBlockByNumber', ['0x' + i.toString(16), false])).hash
				)
			}
			await batchBHS.connect(owner).storeVerifyHeader(blockNumbers, blockHeaders)

			// fetch blocks that were just stored and assert correctness
			const actualBlockhashes = await batchBHS.connect(owner).getBlockhashes(blockNumbers)
			assert.deepEqual(actualBlockhashes, expectedBlockhashes)
		})
