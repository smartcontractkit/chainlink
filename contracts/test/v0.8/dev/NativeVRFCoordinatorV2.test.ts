...
describe('VRFCoordinatorV2', () => {
  let nativeVRFCoordinatorV2: Contract
  ...
  beforeEach(async () => {
    ...
    const nativeVRFCoordinatorV2Factory = await ethers.getContractFactory(
      'src/v0.8/NativeVRFCoordinatorV2.sol:NativeVRFCoordinatorV2',
      accounts[0],
    )
    nativeVRFCoordinatorV2 = await nativeVRFCoordinatorV2Factory.deploy(
      linkToken.address,
  ...
  it('has a limited public interface [ @skip-coverage ]', async () => {
    publicAbi(nativeVRFCoordinatorV2, [
      // Public constants
      'MAX_CONSUMERS',
      'MAX_NUM_WORDS',
  ...
  describe('#setConfig', async function () {
    it('only owner can set', async function () {
      ...
      const resp = await nativeVRFCoordinatorV2.connect(random).getConfig()
  ...
  async function createSubscription(): Promise<number> {
    ...
    const tx = await nativeVRFCoordinatorV2.connect(subOwner).createSubscription()
  ...
  async function createSubscriptionWithConsumers(
    ...
    for (let i = 0; i < consumers.length; i++) {
      await nativeVRFCoordinatorV2.connect(subOwner).addConsumer(subId, consumers[i])
    }
    return subId
  }
  ...
  describe('#createSubscription', async function () {
    it('can create a subscription', async function () {
      await expect(nativeVRFCoordinatorV2.connect(subOwner).createSubscription())
        .to.emit(nativeVRFCoordinatorV2, 'SubscriptionCreated')
        .withArgs(1, subOwnerAddress)
      const s = await nativeVRFCoordinatorV2.getSubscription(1)
  ...
  describe('#requestSubscriptionOwnerTransfer', async function () {
    ...
    it('owner can request transfer', async function () {
      await expect(
        ...
        .to.emit(nativeVRFCoordinatorV2, 'SubscriptionOwnerTransferRequested')
      ...
      await expect(
        nativeVRFCoordinatorV2
          .connect(subOwner)
          .requestSubscriptionOwnerTransfer(subId, randomAddress),
      ).to.not.emit(nativeVRFCoordinatorV2, 'SubscriptionOwnerTransferRequested')
    })
  })
  ...
  describe('#acceptSubscriptionOwnerTransfer', async function () {
    ...
    it('requested owner can accept', async function () {
      await expect(
        ...
        .withArgs(subId, subOwnerAddress, randomAddress)
      await expect(
        nativeVRFCoordinatorV2.connect(random).acceptSubscriptionOwnerTransfer(subId),
      )
        .to.emit(nativeVRFCoordinatorV2, 'SubscriptionOwnerTransferred')
        .withArgs(subId, subOwnerAddress, randomAddress)
    })
  })
  ...
  describe('#addConsumer', async function () {
    ...
    it('must be owner', async function () {
      await expect(
        nativeVRFCoordinatorV2.connect(random).addConsumer(subId, randomAddress),
      ).to.be.revertedWith(`MustBeSubOwner("${subOwnerAddress}")`)
    })
    it('add is idempotent', async function () {
      await nativeVRFCoordinatorV2.connect(subOwner).addConsumer(subId, randomAddress)
      await nativeVRFCoordinatorV2.connect(subOwner).addConsumer(subId, randomAddress)
    })
    it('cannot add more than maximum', async function () {
      ...
      await expect(
        nativeVRFCoordinatorV2.connect(subOwner).addConsumer(subId, randomAddress),
      ).to.be.revertedWith(`TooManyConsumers()`)
    ...
    it('owner can update', async function () {
      await expect(
        ...
        .to.emit(nativeVRFCoordinatorV2, 'SubscriptionConsumerAdded')
  ...
  describe('#removeConsumer', async function () {
    ...
    it('must be owner', async function () {
      await expect(
        nativeVRFCoordinatorV2.connect(random).removeConsumer(subId, randomAddress),
      ).to.be.revertedWith(`MustBeSubOwner("${subOwnerAddress}")`)
    })
    it('owner can update', async function () {
      const subBefore = await nativeVRFCoordinatorV2.getSubscription(subId)
      await nativeVRFCoordinatorV2.connect(subOwner).addConsumer(subId, randomAddress)
      await expect(
        nativeVRFCoordinatorV2.connect(subOwner).removeConsumer(subId, randomAddress),
      )
        .to.emit(nativeVRFCoordinatorV2, 'SubscriptionConsumerRemoved')
        .withArgs(subId, randomAddress)
      const subAfter = await nativeVRFCoordinatorV2.getSubscription(subId)
  ...
  describe('#cancelSubscription', async function () {
    ...
    it('can cancel', async function () {
      await linkToken
        .connect(subOwner)
        .transferAndCall(
          nativeVRFCoordinatorV2.address,
          BigNumber.from('1000'),
      ...
      await expect(
        ...
        .to.emit(nativeVRFCoordinatorV2, 'SubscriptionCanceled')
      ...
      await expect(
        nativeVRFCoordinatorV2.connect(subOwner).getSubscription(subId),
      ).to.be.revertedWith('InvalidSubscription')
    })
    ...
    it('cannot cancel with pending req', async function () {
      ...
      await nativeVRFCoordinatorV2.registerProvingKey(subOwnerAddress, testKey)
      await nativeVRFCoordinatorV2.connect(owner).reg
      const kh = await nativeVRFCoordinatorV2.hashOfKey(testKey)
      await nativeVRFCoordinatorV2.connect(consumer).requestRandomWords(
        kh, // keyhash
        subId, // subId
        1, // minReqConf
      ...
      await expect(
        nativeVRFCoordinatorV2.connect(owner).ownerCancelSubscription(subId),
      )
        .to.emit(nativeVRFCoordinatorV2, 'SubscriptionCanceled')
  ...
  describe('#recoverFunds', async function () {
    ...
    it('function that should change internal balance do', async function () {
      ...
      for (const [fn, expectedBalanceChange] of balanceChangingFns) {
        const startingBalance = await nativeVRFCoordinatorV2.getTotalBalance()
        await fn()
        const endingBalance = await nativeVRFCoordinatorV2.getTotalBalance()
        assert(
    ...
    it('only owner can recover', async function () {
      await expect(
        nativeVRFCoordinatorV2.connect(subOwner).recoverFunds(randomAddress),
      ).to.be.revertedWith(`Only callable by owner`)
    })
    ...
    it('owner can recover link transferred', async function () {
      ...
      await linkToken
        .connect(subOwner)
        .transferAndCall(nativeVRFCoordinatorV2.address, BigNumber.from('1000'), s)
      // Circumvent internal balance
      await linkToken
        .connect(subOwner)
        .transfer(nativeVRFCoordinatorV2.address, BigNumber.from('1000'))
      // Should recover this 1000
      await expect(nativeVRFCoordinatorV2.connect(owner).recoverFunds(randomAddress))
        .to.emit(nativeVRFCoordinatorV2, 'FundsRecovered')
  ...
  it('subscription lifecycle', async function () {
    ...
    await expect(
      ...
      .to.emit(nativeVRFCoordinatorV2, 'SubscriptionFunded')
    ...
    await expect(
      nativeVRFCoordinatorV2.connect(subOwner).acceptSubscriptionOwnerTransfer(subId),
    ).to.be.revertedWith(`MustBeRequestedOwner("${randomAddress}")`)
    ...
    await expect(
      nativeVRFCoordinatorV2.connect(random).cancelSubscription(subId, randomAddress),
    ).to.be.revertedWith(`MustBeSubOwner("${subOwnerAddress}")`)
  ...
  describe('#requestRandomWords', async function () {
    ...
    beforeEach(async () => {
      subId = await createSubscription()
      const testKey = [BigNumber.from('1'), BigNumber.from('2')]
      kh = await nativeVRFCoordinatorV2.hashOfKey(testKey)
    })
    it('invalid subId', async function () {
      await expect(
        nativeVRFCoordinatorV2.connect(random).requestRandomWords(
          kh, // keyhash
          12301928312, // subId
          1, // minReqConf
    ...
    it('invalid req confs', async function () {
      await expect(
        nativeVRFCoordinatorV2.connect(consumer).requestRandomWords(
          kh, // keyhash
          subId, // subId
          0, // minReqConf
          1000, // callbackGasLimit
    ...
    it('nonce increments', async function () {
      ...
      const r1 = await nativeVRFCoordinatorV2.connect(consumer).requestRandomWords(
        kh, // keyhash
        subId, // subId
        1, // minReqConf
        1000000, // callbackGasLimit
      ...
      const r2 = await nativeVRFCoordinatorV2.connect(consumer).requestRandomWords(
        kh, // keyhash
        subId, // subId
        1, // minReqConf
        1000000, // callbackGasLimit
    ...
    it('emits correct log', async function () {
      ...
      const reqTx = await nativeVRFCoordinatorV2.connect(consumer).requestRandomWords(
        kh, // keyhash
        subId, // subId
        1, // minReqConf
        1000, // callbackGasLimit
  ...
  describe('#keyRegistration', async function () {
    it('register key emits log', async function () {
      ...
      const kh = await nativeVRFCoordinatorV2.hashOfKey(testKey)
      await expect(
        nativeVRFCoordinatorV2.registerProvingKey(subOwnerAddress, testKey),
      )
        .to.emit(nativeVRFCoordinatorV2, 'ProvingKeyRegistered')
        .withArgs(kh, subOwnerAddress)
      const reqConfig = await nativeVRFCoordinatorV2.getRequestConfig()
    ...
    it('deregister key emits log', async function () {
      ...
      const kh = await nativeVRFCoordinatorV2.hashOfKey(testKey)
      await nativeVRFCoordinatorV2.registerProvingKey(subOwnerAddress, testKey)
      await expect(nativeVRFCoordinatorV2.deregisterProvingKey(testKey))
        .to.emit(nativeVRFCoordinatorV2, 'ProvingKeyDeregistered')
        .withArgs(kh, subOwnerAddress)
      const reqConfig = await nativeVRFCoordinatorV2.getRequestConfig()
    ...
    it('cannot deregister unregistered key', async function () {
      ...
      const kh = await nativeVRFCoordinatorV2.hashOfKey(testKey)
      await expect(
        nativeVRFCoordinatorV2.deregisterProvingKey(testKey),
      ).to.be.revertedWith(`NoSuchProvingKey("${kh}")`)
    })
    it('can register after deregister', async function () {
      const testKey = [BigNumber.from('1'), BigNumber.from('2')]
      await nativeVRFCoordinatorV2.registerProvingKey(subOwnerAddress, testKey)
      await nativeVRFCoordinatorV2.deregisterProvingKey(testKey)
      await nativeVRFCoordinatorV2.registerProvingKey(randomAddress, testKey)
    })
  })
  ...
  describe('#fulfillRandomWords', async function () {
    ...
    it('unregistered key should fail', async function () {
      ...
      await expect(
        nativeVRFCoordinatorV2.connect(oracle).fulfillRandomWords(proof, rc),
      ).to.be.revertedWith(
    ...
    it('incorrect commitment wrong blocknum', async function () {
      ...
      const testKey = [BigNumber.from('1'), BigNumber.from('2')]
      const kh = await nativeVRFCoordinatorV2.hashOfKey(testKey)
      const tx = await nativeVRFCoordinatorV2.connect(consumer).requestRandomWords(
        kh, // keyhash
        subId, // subId
        1, // minReqConf
        1000, // callbackGasLimit
  ...
  describe('#getFeeTier', async function () {
    ...
    it('tier1', async function () {
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(0)) == 10000)
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(5)) == 10000)
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(10)) == 10000)
    })
    it('tier2', async function () {
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(11)) == 1000)
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(12)) == 1000)
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(20)) == 1000)
    })
    it('tier3', async function () {
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(21)) == 100)
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(30)) == 100)
    })
    it('tier4', async function () {
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(31)) == 10)
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(40)) == 10)
    })
    it('tier5', async function () {
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(41)) == 1)
      assert((await nativeVRFCoordinatorV2.connect(random).getFeeTier(123102)) == 1)
    })
  })
