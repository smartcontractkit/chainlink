...
describe('OptimismValidator', () => {
  ...
  beforeEach(async () => {
    ...
    const mockOptimismL1CrossDomainMessengerFactory = await ethers.getContractFactory(
      'src/v0.8/tests/MockOptimismL1CrossDomainMessenger.sol:MockOptimismL1CrossDomainMessenger',
    )
    mockOptimismL1CrossDomainMessenger = await mockOptimismL1CrossDomainMessengerFactory.deploy()
  ...
  describe('#withdrawFunds', () => {
    it('reverts if called by non owner', async () => {
      ...
      await expect(optimismValidator.connect(eoaValidator).withdrawFundsTo(refundAddr)).to.be.revertedWith("Only callable by owner")
    })
  ...
  describe('#withdrawFundsFromL2', () => {
    ...
    it('reverts if called by non owner', async () => {
      const refundAddr = deployer.address
      await expect(optimismValidator.connect(eoaValidator).withdrawFundsFromL2(amountToWithdraw, refundAddr)).to.be.revertedWith("Only callable by owner")
    })

    it('successfully withdraws funds from L2', async () => {
      const refundAddr = deployer.address
      await expect(optimismValidator.connect(deployer).withdrawFundsFromL2(amountToWithdraw, refundAddr))
        .to.emit(
          optimismValidator,
  ...
  describe('#validate', () => {
    ...
    it('reverts if called by account with no access', async () => {
      await expect(optimismValidator.connect(eoaValidator).validate(0, 0, 1, 1)).to.be.revertedWith("No access")
    })
    ...
    it('post sequencer offline', async () => {
      ...
      const arbitrumSequencerStatusRecorderCallData =
        optimismUptimeFeedFactory.interface.encodeFunctionData(
          'updateStatus',
          [true, now],
        )
