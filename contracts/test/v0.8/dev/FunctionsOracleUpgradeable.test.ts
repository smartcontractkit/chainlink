...
before(async () => {
  ...
  functionsOracleOriginalFactory = await ethers.getContractFactory(
    'src/v0.8/functions/test/testhelpers/FunctionsOracleOriginalHelper.sol:FunctionsOracleOriginalHelper',
  ...
  describe('Upgrades', () => {
    ...
    it('can be upgraded to a new implementation', async () => {
      const upgradedOracle = await migrateAndCheck(
        'src/v0.8/functions/test/testhelpers/FunctionsOracleMigrationHelper.sol:FunctionsOracleMigrationHelper',
      )
    ...
    it('can be upgraded to the latest implementation', async () => {
      await migrateAndCheck(
        'src/v0.8/functions/test/testhelpers/FunctionsOracleUpgradeableHelper.sol:FunctionsOracleUpgradeableHelper',
      )
    })
  })
})
