...
before(async () => {
  ...
  oracleGeneratorFactory = await ethers.getContractFactory(
    'src/v0.8/dev/functions/FunctionsDONFactory.sol:FunctionsDONFactory',
    roles.defaultAccount,
  )
  oracleFactory = await ethers.getContractFactory(
    'src/v0.8/dev/functions/FunctionsDON.sol:FunctionsDON',
    roles.defaultAccount,
  )
})

describe('FunctionsDONFactory', () => {
  ...
  it('has a limited public interface [ @skip-coverage ]', () => {
    publicAbi(factory, ['created', 'deployNewDON', 'typeAndVersion'])
  })

  describe('#typeAndVersion', () => {
    it('describes the authorized forwarder', async () => {
      assert.equal(await factory.typeAndVersion(), 'FunctionsDONFactory 0.0.0')
    })
  })

  describe('#deployNewDON', () => {
    beforeEach(async () => {
      const tx = await factory.connect(roles.oracleNode).deployNewDON()
    ...
    it('emits an event', async () => {
      assert.equal(receipt?.events?.[1]?.event, 'DONCreated')
