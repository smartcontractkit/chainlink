...
#            slither --fail-none --checklist --markdown-root ${{ github.server_url }}/${{ github.repository }}/blob/${{ github.sha }}/ "$file" >> slither-results.md
    ...
    source ./scripts/select_solc_version.sh "$sanitized_file"
    exit_code=$?
    if [ $exit_code -ne 0 ]; then
      echo "Error: Failed to select the correct solc version for $sanitized_file."
      slither_errors=$((slither_errors+1))
      continue
    fi
      slither --fail-none --checklist "$sanitized_file" >> slither-results.md  || slither_errors=$((slither_errors+1))
