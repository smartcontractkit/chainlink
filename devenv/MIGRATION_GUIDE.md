...
- `nodes` count depends on the product (1 for single-node products like cron/DR, 5 for multi-node like OCR2/flux)
- The product-specific TOML section (e.g. `[[vrfv2]]`) key must match the `toml` struct tag on the `Configurator.Config` field you create in Step 2
...
- [products/vrfv2/configuration.go](products/vrfv2/configuration.go) -- single-node product with coordinator v2 and jobs
...
- Example: a VRF devenv product may reimplement proving-key / job-ID encoding as local helpers in `configuration.go` (see [products/vrfv2/core.go](products/vrfv2/core.go) for patterns)
...
- [tests/vrfv2/smoke_test.go](tests/vrfv2/smoke_test.go) -- VRF v2 smoke (contracts, subscriptions, async assertions)
...
2. **TOML key mismatch** -- The `toml` struct tag on `Configurator.Config` must exactly match the TOML section name (e.g. `toml:"vrfv2"` matches `[[vrfv2]]`). A mismatch means the config silently loads as empty.
...
7. **Package name** -- The test file's `package` declaration should match the directory name (e.g. `package vrfv2` in `tests/vrfv2/`).
