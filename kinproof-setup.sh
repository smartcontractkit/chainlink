...
echo "=== KinProof Sovereign x402 Identity Layer - Clean E2E Setup ==="
...
func (h *SovereignHD) DeriveEphemeral(index uint32) (string, string, error) {
	...
	priv := hex.EncodeToString(k.Key)
	return deriveAddr(k), priv, nil
}

func deriveAddr(k *bip32.Key) string {
	pub := k.Key.PublicKey()
	h := sha256.Sum256(pub)
...
# Intent + Proof + Attribution (shortened but functional)
...
func main() {
	...
	fmt.Println("\n✓ Sovereign transaction intent created with proof-bound attribution.")
}
MAIN
...
echo "=== KinProof Setup Complete ==="
echo "Run with: cd kinproof && ./run.sh"
