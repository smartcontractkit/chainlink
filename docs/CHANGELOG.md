...
- New optional Audit Log Forwarder added
  - The core node now supports emitting and forwarding audit logs of various actions to a logging service via HTTP. To opt in to sending audit log events to a custom service, set the `AUDIT_LOGS_HEC_FORWARDER_URL` environment variable accordingly. See the configuration-variables documentation for details.
