...
## Mixed and Remote Mode Test Runs

For mixed-mode execution (some components local, some remote), use a topology with explicit `placement` values and verify startup prints `Runtime Placement Matrix`.

Remote agent resolution during these runs follows:
...
1. start with a mixed placement config
2. confirm startup output includes `Runtime Placement Matrix`
3. deploy a workflow and verify remote artifact delivery succeeds
4. run `go run . env remote stop` and verify the stop summary reports requested/stopped counts
5. run `go run . env stop-all` and verify local containers and state are cleaned up

For architecture details and ownership boundaries, see [Remote Execution Architecture](../reference/remote-execution-architecture.md).
