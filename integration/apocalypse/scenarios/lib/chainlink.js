...
async function chainlinkLogin(nodeURL, tmpdir) {
    let resp = await run(`chainlink -j admin login -f ${__dirname}/../../chainlink/apicredentials`, { CLIENT_NODE_URL: nodeURL, ROOT: tmpdir })
    console.log(resp)
}

async function addJobSpec(nodeURL, jobSpec, tmpdir) {
    let jobSpecJSON = JSON.stringify(jobSpec)
    let resp = await run(['chainlink', '-j', 'jobs', 'create', jobSpecJSON], { CLIENT_NODE_URL: nodeURL, ROOT: tmpdir })
    try {
