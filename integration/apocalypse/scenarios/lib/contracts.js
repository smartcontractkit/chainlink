...
async function deployFluxMonitorContracts(carol, oracles) {
    ...
    let resp = await (await linkToken.transfer(fluxAggregator.address, '100000000000000000000')).wait()
    ...
    let resp3 = await (await fluxAggregator.addOracles(oracles, oracles, 1, 2, 1)).wait()
    assert(resp3.status === 1)

    console.log('Calling .updateFutureRounds...')
    let resp2 = await (await fluxAggregator.updateFutureRounds('1000000000000000000', 1, 2, 1, 3000)).wait()
